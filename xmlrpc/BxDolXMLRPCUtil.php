...
class BxDolXMLRPCUtil
{
    ...
    function getUserLocation($aProfileInfo)
    {
        if (!empty($aProfileInfo['Country']) && !empty($aProfileInfo['City'])) {
            return _t('_sys_city_country_format', $aProfileInfo['City'],
                _t($GLOBALS['aPreValues']['Country'][$aProfileInfo['Country']]['LKey']));
    ...
    public static function getUserInfo($iId, $iIdViewer = 0, $isCountData = true)
    {
        ...
        bx_import('BxDolAlerts');
        $oZ = new BxDolAlerts('mobile', 'user_info', $iId, $iIdViewer,
            array('profile' => &$aSexSql, 'return_data' => &$aRet));
        $oZ->alert();
    ...
    public static function fillProfileArray($a, $sImage = 'icon', $iIdViewer = 0)
    {
        ...
        bx_import('BxDolAlerts');
        $oZ = new BxDolAlerts('mobile', 'user_info2', $a['ID'], $iIdViewer,
            array('profile' => &$a, 'return_data' => &$aRet));
        $oZ->alert();
    ...
    public static function getMenu($sMenu, $aMarkersReplace = array())
    {
        ...
        $aMenu    = array();
        $aRecords = $GLOBALS['MySQL']->getAll("SELECT * FROM `sys_menu_mobile` WHERE `page` = ? AND `active` = 1 ORDER BY `order`",
            [$sMenu]);

        bx_import('BxDolAlerts');
        $oZ = new BxDolAlerts('mobile', 'menu', 0, 0,
            array('menu' => $sMenu, 'data' => &$aRecords, 'markers_replace' => &$aMarkersReplace));
        $oZ->alert();
