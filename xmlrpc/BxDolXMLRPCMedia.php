...
class BxDolXMLRPCMedia
{
    ...
    {
        $aAlbums = (new BxDolXMLRPCMedia)->_getMediaAlbumsArray($sType, $iIdProfile, $iIdProfileViewer,
            $isShowEmptyAlbums);
    ...
    function _getMediaAlbumsArray($sType, $iIdProfile, $iIdProfileViewer, $isShowEmptyAlbums = false)
    {
        ...
        $aRet  = array();
        foreach ($aList as $r) {
            if (!BxDolService::call($sModuleName, 'get_album_privacy', array((int)$r['ID'], $iIdProfileViewer),
                'Search')
            ) {
                continue;
            }
            ...
            if ($isSetAvatarFromDefaultAlbumOnly) {
                bx_import('BxDolAlbums');
                $isDefaulAlbum = $r['Uri'] == BxDolAlbums::getAbumUri(getParam($sType . '_profile_album_name'),
                    $iIdProfile) ? 1 : 0;
            } else {
                $isDefaulAlbum = 1;
            }
    ...
    function _getFilesInAlbum(
        ...
        $a = BxDolService::call($sModuleName, 'get_files_in_album',
            array((int)$iAlbumId, $iIdProfileViewer != $iIdProfile, $iIdProfileViewer, array('per_page' => 100)),
            'Search');
        if (!$a) {
        ...
        foreach ($a as $k => $aRow) {
            ...
            $a        = array(
                ...
                'desc'       => new xmlrpcval(BxDolService::call($sModuleName, 'get_length', array($aRow['size']),
                    'Search')),
    ...
    function _isMembershipEnabledFor($iProfileId, $sMembershipActionConstant, $isPerformAction = false)
    {
        ...
        $aCheck = checkAction($iProfileId ? $iProfileId : (int)$_COOKIE['memberID'],
            constant($sMembershipActionConstant), $isPerformAction);
    ...
    function _uploadFile($sType, $sUser, $sPwd, $sAlbum, $binData, $iDataLength, $sTitle, $sTags, $sDesc, $sExt)
    {
        ...
        if ('photo' == $sType && BxDolService::call($sModuleName, $sService, array($sTmpFilename, $aFileInfo, 0, $iId),
                'Uploader')
        ) {
            return new xmlrpcval ("ok");
        } elseif ('photo' != $sType && ($iFileId = BxDolService::call($sModuleName, $sService,
                array($sTmpFilename, $aFileInfo, true), 'Uploader'))
        ) {
