...
class BxDolXMLRPCUser
{
    ...
    function login4($sUser, $sPwdClear)
    {
        ...
        if ($aProfileInfo && ((32 == strlen($sPwdClear) || 40 == strlen($sPwdClear)) && BxDolXMLRPCUtil::checkLogin($sUser,
                    $sPwdClear))
        ) {
            $iId = $aProfileInfo['ID'];
        } elseif ($aProfileInfo && getParam('enable_dolphin_footer') != 'on' && check_password($aProfileInfo['ID'],
                $sPwdClear, BX_DOL_ROLE_MEMBER, false)
        ) {
            $iId = $aProfileInfo['ID'];
        }
    ...
    function updateUserLocation($sUser, $sPwd, $sLat, $sLng, $sZoom, $sMapType)
    {
        ...
        $iRet = BxDolService::call('wmap', 'update_location_manually',
            array('profiles', $iId, (float)$sLat, (float)$sLng, (int)$sZoom, $sMapType)) ? '1' : '0';
    ...
    function _checkUserPrivacy($iId, $iIdProfile)
    {
        ...
        $oZ = new BxDolAlerts('mobile', 'view_profile', $iIdProfile, $iId,
            array('access_denied' => &$mixedAccessDenied));
        $oZ->alert();
