    ...
    switch ($_POST['direction']) {
        case 'subscribe':
            if (isset($_POST['user_id']) && (int)$_POST['user_id'] != 0) {
                ...
                if (isset($_POST['user_name']) && isset($_POST['user_email'])) {
                    $aResult = $oSubscription->subscribeVisitor($_POST['user_name'], $_POST['user_email'], $sUnit,
                        $sAction, $iObjectId);
                }
            }
            break;
        ...
        case 'unsubscribe':
            if (isset($_POST['user_id']) && (int)$_POST['user_id'] != 0) {
                ...
                if (isset($_POST['user_name']) && isset($_POST['user_email'])) {
                    $aResult = $oSubscription->unsubscribeVisitor($_POST['user_name'], $_POST['user_email'], $sUnit,
                        $sAction, $iObjectId);
                }
            }
            break;
