    ...
    function __construct()
    {
        ...
        switch ($_REQUEST['search_mode']) {
            ...
            case 'adv':
                ...
                break;
            default:
                $iPFArea = 9; // simple search (default)
        }
        ...
        for ($i = ($aPaginate['page'] - 1) * $aPaginate['per_page'];
            ...
            if ($aProfile['Couple']) {
                ...
                if (!($iIndex % 2)) {
                    ...
                    $sOutputHtml .= $oSearchProfile->PrintSearhResult($aProfile, $aCoupleInfo, $aExtendedCss,
                        $sTemplateName);
                }
            } else {
                if (!($iIndex % 2)) {
    ...
    function getBlockCode_Results($iBlockID)
    {
        ...
        switch ($_REQUEST['show']) {
            ...
            default:
                ...
                list($sResults, $aDBTopMenu, $sPagination, $sTopFilter) = $oProfile->GenSearchResultBlock($this->oPF->aBlocks,
                    $aRequestParams, $this->aFilterSortSettings, 'search.php');
                break;
        }
