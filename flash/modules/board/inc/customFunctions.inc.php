...
function save($sSavedId, $sFilePath, $sTitle)
{
    ...
    $aFileInfo = array(
        'medTitle'   => stripslashes($sTitle),
        'medDesc'    => stripslashes($sTitle),
        'medTags'    => BX_BOARD_PHOTOS_TAG,
        'Categories' => array(BX_BOARD_PHOTOS_CAT),
        'album'      => str_replace('{nickname}', $aUser["NickName"], getParam('bx_photos_profile_album_name'))
    );

    if ($sSavedId > 0) {
        $iRet = BxDolService::call('photos', 'perform_photo_replace', array($sFilePath, $aFileInfo, false, $sSavedId),
            'Uploader');
        if ($iRet) {
