...
switch ($sAction) {
    ...
    case 'userAuthorize':
        if (loginUser($sId, $sPassword) == TRUE_VAL) {
            ...
            $sContents .= parseXml($aXmlTemplates['user'], $sId, USER_STATUS_NEW, $aUser['nick'], $aUser['sex'],
                $aUser['age'], $aUser['photo'], $aUser['profile'], $aUser['desc']);
        } else {
    ...
    case 'createBoard':
        ...
        if (empty($iBoardId)) {
            $sContents = parseXml($aXmlTemplates['result'], "msgErrorCreatingBoard", FAILED_VAL);
        } else {
