    ...
    switch ($sMode) {
        case 'update':
            ...
            while ($aFile = $rFiles->fetch()) {
                @unlink($sFilesPath . $aFile['ID'] . $sFileExtension);
            }
            ...
            while ($aBoard = $rResult->fetch())
                switch ($aBoard['Status']) {
                    ...
                    case BOARD_STATUS_NEW:
                        $sBoards .= parseXml($aXmlTemplates['board'], $aBoard['ID'], BOARD_STATUS_NORMAL,
                            $aBoard['OwnerID'], empty($aBoard['Password']) ? FALSE_VAL : TRUE_VAL,
                            stripslashes($aBoard['Name']));
                        break;
                }
        ...
        case 'all':
            ...
            $rResult = getResult($sSql);
            while ($aBoard = $rResult->fetch()) {
                $sBoards .= parseXml($aXmlTemplates['board'], $aBoard['BoardID'], BOARD_STATUS_NORMAL,
                    $aBoard['OwnerID'], empty($aBoard['Password']) ? FALSE_VAL : TRUE_VAL, $aBoard['In'],
                    stripslashes($aBoard['Name']));
            }
            if ($rResult->rowCount() === 0) {
                ...
                switch ($aUser['Status']) {
                    case USER_STATUS_NEW:
                        $sContent .= parseXml($aXmlTemplates['user'], $aUser['ID'], $aUser['Status'], $aUser['Nick'],
                            $aUser['Sex'], $aUser['Age'], $aUser['Photo'], $aUser['Profile'], $aUser['Desc']);
                        break;
