...
switch ($sAction) {
            ...
            if ($rDirHandle = opendir($sModulesPath . $sModule . $sFolder)) {
                while (false !== ($sPlugin = readdir($rDirHandle))) {
                    if (strpos($sPlugin, ".swf") === strlen($sPlugin) - 4) {
                        $sContents .= parseXml(array(1 => '<plugin><![CDATA[#1#]]></plugin>'),
                            $sModulesUrl . $sModule . $sFolder . $sPlugin);
                    }
                }
            }
            closedir($rDirHandle);
    ...
    case 'RayzFontGet':
        ...
        $sContents = parseXml($aSettings, $_COOKIE["RayzFontbold"], $_COOKIE["RayzFontitalic"],
            $_COOKIE["RayzFontunderline"], $_COOKIE["RayzFontcolor"], $_COOKIE["RayzFontfont"],
            $_COOKIE["RayzFontsize"], $_COOKIE["RayzFontvolume"], $_COOKIE["RayzFontmuted"]);
        break;
    ...
    case 'userAuthorize':
        if (loginAdmin($sId, $sPassword) == TRUE_VAL) {
            ...
            $aUser     = array('id'      => $aUserInfo['id'],
                               'nick'    => $aUserInfo['nick'],
                               'sex'     => $aUserInfo['sex'],
                               'age'     => $aUserInfo['age'],
                               'desc'    => $aUserInfo['desc'],
                               'photo'   => $aUserInfo['photo'],
                               'profile' => $aUserInfo['profile'],
            ...
            $sContents = parseXml($aXmlTemplates['result'], $bBanned ? "msgBanned" : "msgUserAuthenticationFailure",
                FAILED_VAL);
            break;
        }
        $aUser     = initUser($aUser);
        $sContents = parseXml($aXmlTemplates['result'], "", SUCCESS_VAL);
        $sContents .= parseXml($aXmlTemplates['user'], $aUser['id'], USER_STATUS_NEW, $aUser['nick'], $aUser['sex'],
            $aUser['age'], $aUser['desc'], $aUser['photo'], $aUser['profile'], $aUser['type'], USER_STATUS_ONLINE);
        break;
    ...
    case 'searchUser':
        ...
        $sContents = parseXml($aXmlTemplates['result'], "", SUCCESS_VAL);
        $sContents .= parseXml($aXmlTemplates['user'], $sUserId, $aUser['nick'], $aUser['sex'], $aUser['age'],
            $aUser['photo'], $aUser['profile'], $aProfile['Banned'], $aProfile['Type']);
        break;
    ...
    case 'getSmilesets':
        ...
        for ($i = 0; $i < count($aSmilesets); $i++) {
            $sName = getSettingValue(GLOBAL_MODULE, "name", "config", false,
                $sDataDir . $sSmilesetsDir . $aSmilesets[$i]);
            $sData .= parseXml($aXmlTemplates['smileset'], $aSmilesets[$i] . "/", $sConfigFile,
                empty($sName) ? $aSmilesets[$i] : $sName);
        }
    ...
    case 'createRoom':
        $iRoomId = doRoom('insert', $sId, 0, $sRoom, $sPassword, $sDesc,
            process_db_input($_REQUEST['temp']) == TRUE_VAL);
        if (empty($iRoomId)) {
    ...
    case 'getOnlineUsers':
        ...
        if ($rResult->rowCount() == 0) {
            getResult("TRUNCATE TABLE `" . MODULE_DB_PREFIX . "Messages`");
        }
    ...
    case 'update':
        ...
        while ($aMsg = $res->fetch()) {
            ...
            $sMsgs .= parseXml($aXmlTemplates['message'], $aMsg['ID'], stripslashes($aMsg['Message']), $aMsg['Room'],
                $aMsg['Sender'], $aMsg['Recipient'], $aMsg['Whisper'], $aStyle['color'], $aStyle['bold'],
                $aStyle['underline'], $aStyle['italic'], $aStyle['size'], $aStyle['font'], $aStyle['smileset'],
                $aMsg['When']);
        }
    ...
    case 'newMessage':
        ...
        if (!useServer()) {
            ...
            $sStyle     = serialize(array('color'     => $sColor,
                                          'bold'      => $sBold,
    ...
    case 'getHistory':
        ...
        for ($i = 0; $i < $iRoomsCount; $i++) {
            ...
            if ($aMsg['Room'] != $iRoom) {
                if (!empty($sRoom) && !empty($sMsgs)) {
                    $sRooms .= parseXml($aXmlTemplates['history']['room'], $iRoom, $sRoom,
                            $iCount) . $sMsgs . "</room>";
                    $sMsgs  = "";
                    $iCount = 0;
                }
            ...
            $iCount++;
            $sMsgs .= parseXml($aXmlTemplates['history']['msg'], $aMsg['ID'], $aMsg['Sender'], $aMsg['Recipient'],
                $aMsg['Message']);
        }
        if (!empty($sRoom) && !empty($sMsgs)) {
        ...
        for ($i = 0; $i < $iMsgsCount; $i++) {
            ...
            if ($aMsg['SndRcp'] != $sSndRcp) {
                if (!empty($sMsgs)) {
                    $sPrivate .= parseXml($aXmlTemplates['history']['private'], $aMsg['Sender'], $aMsg['Recipient'],
                            $iCount) . $sMsgs . "</private>";
                    $sMsgs  = "";
                    $iCount = 0;
                }
                $sSndRcp = $aMsg['SndRcp'];
