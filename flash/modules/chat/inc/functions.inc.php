...
function rzGetMembershipSettings($bAdmin = false)
{
    ...
    for ($i = 0; $i < $iCount; $i++) {
        $aSetting = $rResult->fetch();
        if ($bAdmin) {
            $sSettings .= parseXml($aSettingTmpls, $aSetting['ID'], $aSetting['Name'], $aSetting['Type'],
                $aSetting['Default'], $aSetting['Range'], $aSetting['Caption']);
        } else {
            $sSettings .= parseXml($aSettingTmpls, $aSetting['ID'], $aSetting['Name'], $aSetting['Error']);
        }
    }
    ...
    switch ($sMode) {
        case 'update': //--- Return new and deleted rooms.
            ...
            while ($aRoom = $rResult->fetch())
                switch ($aRoom['Status']) {
                    ...
                    default:
                        $sRooms .= parseXml($aXmlTemplates['room'], $aRoom['ID'], ROOM_STATUS_NORMAL, $aRoom['OwnerID'],
                            empty($aRoom['Password']) ? FALSE_VAL : TRUE_VAL, stripslashes($aRoom['Name']),
                            stripslashes($aRoom['Desc']));
                        break;
                }
            break;
        ...
        case 'all':
            ...
            if ($rResult->rowCount() == 0) {
                getResult("TRUNCATE TABLE `" . MODULE_DB_PREFIX . "RoomsUsers`");
            }
...
function refreshUsersInfo($sId = "", $sMode = 'all')
{
    ...
    switch ($sMode) {
            ...
            while ($aUser = $rRes->fetch()) {
                ...
                switch ($aUser['Status']) {
                    case USER_STATUS_NEW:
                        $sContent .= parseXml($aXmlTemplates['user'], $aUser['ID'], $aUser['Status'], $aUser['Nick'],
                            $aUser['Sex'], $aUser['Age'], stripslashes($aUser['Desc']), $aUser['Photo'],
                            $aUser['Profile'], $aUser['Type'], $aUser['Online']);
                        break;
        ...
        case 'all':
            ...
            while ($aUser = $rRes->fetch()) {
                $sContent .= parseXml($aXmlTemplates['user'], $aUser['ID'], USER_STATUS_NEW, $aUser['Nick'],
                    $aUser['Sex'], $aUser['Age'], stripslashes($aUser['Desc']), $aUser['Photo'], $aUser['Profile'],
                    $aUser['Type'], $aUser['Online'], $aUser['Time']);
            }
            break;
    }
