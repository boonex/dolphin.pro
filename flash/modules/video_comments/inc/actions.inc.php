...
switch ($sAction) {
    ...
    case 'getFile':
        ...
        if ($sStatus == SUCCESS_VAL) {
            $sContents .= parseXml($aXmlTemplates['file'], $sId, $sGetFile, $sGetFile, $sImageFile, $aFile['Time'],
                $sSaveName);
        }
        break;
    ...
    case 'processFile':
        ...
        if (file_exists($sTempFile) && filesize($sTempFile) > 0 && _grabImages($sTempFile,
                $sFilesPath . $sTempFileName)
        ) {
            $sContents = parseXml($aXmlTemplates['result'], "", SUCCESS_VAL);
            $sContents .= parseXml($aXmlTemplates['file'], "0",
                $GLOBALS['sFilesDir'] . $sTempFileName . VC_FLV_EXTENSION, "",
                $GLOBALS['sFilesDir'] . $sTempFileName . VC_IMAGE_EXTENSION, 0);
        } else {
            _deleteTempFiles($sId);
        }
        break;
