...
switch ($sAction) {
    ...
    case 'processFile':
        ...
        if (file_exists($sTempFile) && filesize($sTempFile) > 0 && grabImages($sTempFile,
                $sFilesPath . $sTempFileName)
        ) {
            $sContents = parseXml($aXmlTemplates['result'], "", SUCCESS_VAL);
            $sContents .= parseXml($aXmlTemplates['file'], "0", $GLOBALS['sFilesDir'] . $sTempFileName . FLV_EXTENSION,
                "", $GLOBALS['sFilesDir'] . $sTempFileName . IMAGE_EXTENSION, 0);
        } else {
            deleteTempFiles($sId);
        }
        break;
