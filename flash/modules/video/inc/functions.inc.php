...
function uploadVideo(
    ...
    if (file_exists($sFilePath) && filesize($sFilePath) > 0) {
        ...
        if (file_exists($sTempFileName) && filesize($sTempFileName) > 0) {
            ...
            foreach ($aFilesConfig as $a) {
                if (isset($a['image']) && $a['image']) {
                    @rename($sFilesPath . $sUserId . TEMP_FILE_NAME . $a['postfix'],
                        $sFilesPath . $sFileId . $a['postfix']);
                }
            }
...
function getVideoSize($sInputFile)
{
    ...
    $sTmpl = prepareCommand($GLOBALS['aConvertTmpls']['image'],
...
function getConverterTmpl($sInputFile, $sSize, $bSound = true, $bRecorded = false)
{
    ...
    return prepareCommand($GLOBALS['aConvertTmpls'][$bUsex264 ? 'playX264' : 'play'], array(
        "input"         => $sInputFile,
        "bitrate"       => getVideoBitrate(),
        "size"          => getVideoSize($sSize),
...
function convertVideo($sId)
{
    ...
    $bUseX264 = usex264();
    $sTmpl    = prepareCommand($GLOBALS['aConvertTmpls'][$bUseX264 ? "playX264" : "play"], array(
        "bitrate"       => getVideoBitrate(),
...
function getGrabImageTmpl($sInputFile, $sOutputFile, $sSize = "", $iSecond = 0)
{
    ...
    return prepareCommand($aConvertTmpls["image"], array(
        "input"  => $sInputFile,
        "second" => $iSecond,
