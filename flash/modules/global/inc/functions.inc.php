...
function recompileIntegrator($sWidget)
{
    ...
    if (!file_exists($sFileName)) {
        return parseXml($aXmlTemplates['result'], getError($aErrorCodes[1], $sFile), FAILED_VAL);
    }
    ...
    foreach ($aModules as $sAppName => $aModule) {
        ...
        if (empty($iWidth)) {
            $iWidth = $aModule['layout']['width'];
        }
        if (!is_numeric($iWidth)) {
            $iWidth = $aModule['minSize']['width'];
        }
        $iHeight = getSettingValue($sWidget, $sAppName . "_height");
        if (empty($iHeight)) {
            $iHeight = $aModule['layout']['height'];
        }
        if (!is_numeric($iHeight)) {
            $iHeight = $aModule['minSize']['height'];
        }
...
function refreshExtraFile($sWidget, $sCase, $bReset = false, $sDefaultFile = "", $aEnabledFiles = array())
{
    ...
    $sCurrent        = isset($aContents[FILE_DEFAULT_KEY]) && in_array($aContents[FILE_DEFAULT_KEY],
        $aFiles["files"]) ? $aContents[FILE_DEFAULT_KEY] : $aFiles["current"];
...
function getFileContents($sWidget, $sFile, $bArray = false)
{
    ...
    if (!file_exists($sFileName)) {
        return array(
            'value'    => getError($aErrorCodes[1], $sFile),
            'status'   => FAILED_VAL,
...
function getFileExtension($sWidget, $sFolder)
{
    ...
    if ($rDirHandle = opendir($sFolderPath)) {
        while (false !== ($sFile = readdir($rDirHandle))) {
            ...
            if (is_file($sFolderPath . $sFile) && $sFile != "." && $sFile != ".." && in_array($aPathInfo['extension'],
                    $aRightExtensions)
            ) {
