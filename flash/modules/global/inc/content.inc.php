...
{
    if (getSettingValue(GLOBAL_MODULE, "license", "main") == $sLicense) {
        return;
    }
...
function getApplicationContent(
    ...
    ?>
    <div id="<?= $sDivId ?>" <?= $sDivName . $sDivStyle ?>>
        <div id="<?= $sInnerDivId ?>"></div>
    </div>
    <script type="text/javascript" language="javascript">
        <?php
        foreach ($aConfig['flashVars'] as $sKey => $sValue) {
...
function getRedirectForm($sModule, $sApp, $aRequest)
{
    ob_start();
    ?>
    <form style="margin:0; padding:0;" name="<?= $sModule . "-" . $sApp; ?>" method="POST"
        ...
        foreach ($aRequest as $sKey => $sValue) {
            ?>
            <input type="hidden" name="<?= process_db_input($sKey, BX_TAGS_SPECIAL_CHARS) ?>"
                   value="<?= process_db_input($sValue, BX_TAGS_SPECIAL_CHARS) ?>" />
            <?php
        }
        ?>
    </form>
