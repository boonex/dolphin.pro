...
switch ($sAction) {
    ...
    case 'startInstall':
        ...
        if (file_exists($sFileName)) {
            ...
            if (!isset($aItems["xml/main.xml"])) {
                $sContents = parseXml($aXmlTemplates['result'], getError($aErrorCodes[1], $sWidget . "/xml/main.xml"),
                    FAILED_VAL);
            } else {
            ...
            $sText    = parseXml($aXmlTemplates['text'],
    ...
    case 'setSettingValue':
        ...
        if ($aResult['status'] == SUCCESS_VAL && (strpos($sSettingKey, "_width") > 0 || strpos($sSettingKey,
                    "_height") > 0)
        ) {
            if (isset($aModules)) {
                unset($aModules);
    ...
    case 'updates':
        ...
        if ($aResult['status'] == SUCCESS_VAL) {
            ...
            $sContents .= parseXml($aXmlTemplates['result'], SUCCESS_VAL, $aContents['updated'],
                $aContents['updateLast'], $aContents['updateUrl']);
        } else {
            $sContents = parseXml($aXmlTemplates['result'], $aResult['value'], FAILED_VAL);
        }
        break;
    ...
    case 'getMyWidgets':
        ...
        while (($sInner = readdir($rDirHandler)) !== false) {
            if (is_dir($sModulesPath . $sInner) && substr($sInner, 0, 1) != '.' && $sInner != 'global') {
                ...
                $aContents[] = parseXml($aXmlTemplates['widget'], $sInner, $sVersion, $sTitle, $sAuthor, $sAuthorUrl,
                    $sImageUrl, $sStatus, $sAdminUrl);
                $aTitles[]   = $sTitle;
    ...
    case 'getWidgetAds':
        ...
        if ($bPaid) {
            ...
            $sContents = parseXml($aXmlTemplates['ads'], $sEnabled, $sBannerUrl, $sBannerLink, $sBannerTarget,
                $iBannerAlpha);
        } else {
