...
switch ($sAction) {
    ...
    case 'senderAuthorize':
        if (loginUser($sSndId, $sSndPassword) == TRUE_VAL) {
            ...
            $sContents .= parseXml($aXmlTemplates['user'], $sSndId, $aUser['nick'], $aUser['sex'], $aUser['age'],
                $aUser['desc'], $aUser['photo'], $aUser['profile'], USER_STATUS_ONLINE);
        } else {
    ...
    case 'recipientAuthorize':
        ...
        if (!empty($sSearchResult)) {
            ...
            $aUser['sex'] = $aUser['sex'] == 'female' ? "F" : "M";
            $sContents .= parseXml($aXmlTemplates['user'], $sRspId, $aUser['nick'], $aUser['sex'], $aUser['age'],
                $aUser['desc'], $aUser['photo'], $aUser['profile'], getUserOnlineStatus($sRspId, $sSndId));
        } else {
    ...
    case 'getPend':
        ...
        while ($aMsg = $rResult->fetch()) {
            $sMsgs .= parseXml($aXmlTemplates['message'], $aMsg['Message']);
        }
    ...
    case 'updateInvite':
        ...
        while ($aMsg = $rResult->fetch()) {
            ...
            $sContents .= parseXml($aXmlTemplates['message'], $aMsg['SenderID'], $aUserInfo['nick'],
                $aUserInfo['profile'], $aMsg['Message']);
        }
    ...
    case 'refreshStatus':
        ...
        if (empty($iContactId)) {
            getResult("INSERT INTO `" . MODULE_DB_PREFIX . "Contacts`(`SenderID`, `RecipientID`, `When`) VALUES ('" . $sSndId . "', '" . $sRspId . "', '" . time() . "')");
        }
    ...
    case 'recipientUpdate':
        ...
        if (empty($iId)) {
            getResult("INSERT INTO `" . MODULE_DB_PREFIX . "Contacts`(`SenderID`, `RecipientID`, `Online`, `When`) VALUES ('" . $sSndId . "', '" . $sRspId . "', '" . USER_STATUS_ONLINE . "', '" . time() . "')");
        }
        ...
        while ($aMsg = $res->fetch()) {
            ...
            $sMsgs .= parseXml($aXmlTemplates['message'], $aMsg['ID'], stripslashes($aMsg['Message']), $aStyle['color'],
                $aStyle['bold'], $aStyle['underline'], $aStyle['italic'], $aStyle['size'], $aStyle['font'],
                $aStyle['smileset']);
        }
        $sContents .= makeGroup($sMsgs, "messages");
