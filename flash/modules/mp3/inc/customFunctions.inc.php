...
if (!defined("SCREENSHOT_WIDTH")) {
    define("SCREENSHOT_WIDTH", 453);
}
if (!defined("SCREENSHOT_HEIGHT")) {
    define("SCREENSHOT_HEIGHT", 330);
}
...
function postMusic($sUploadedFile, $aFileInfo)
{
    ...
    $aResult = initFile($sId, $aFileInfo['category'], addslashes($aFileInfo['title']), addslashes($aFileInfo['tags']),
        addslashes($aFileInfo['description']));
...
function mp3_getList($sId)
{
    ...
    for ($i = 0; $i < count($aData); $i++) {
        ...
        $sResult .= parseXml($aXmlTemplates['file'], $sImageFile, $aData[$i]['size'], $aData[$i]['ownerName'],
            $aData[$i]['view'], $aData[$i]['voting_rate'], $aData[$i]['date'], $aData[$i]['title'],
            BX_DOL_URL_ROOT . $aData[$i]['uri']);
    }
