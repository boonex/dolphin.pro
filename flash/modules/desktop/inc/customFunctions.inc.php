...
{
    return isModuleAvailable("chat", $sUserId,
        ACTION_ID_USE_CHAT) ? "#desktopUrl#chat.html?id=#owner#&password=#password#" : "";
}

function getUserImLink($sUserId)
{
    return isModuleAvailable("messenger", $sUserId,
        ACTION_ID_USE_MESSENGER) ? "im/sender=#owner#&password=#password#&recipient=#user#/#nick#/10,10,550,500/true" : "";
}
    ...
    for ($i = 0; $i < count($aMails); $i++) {
        ...
        if (is_array($aMediaUsers[$sSenderId])) {
            ...
            $sResult .= parseXml($aXmlTemplates["message"], $aMails[$i]['ID'], $sSenderId, $aMails[$i]['Body'],
                $aUser['nick'], $aUser['sex'], $aUser['age'], $aUser['photo'], $aUser['profile'], $aUser['music'],
                $aUser['video']);
        } else {
            $sResult .= parseXml($aXmlTemplates["message"], $aMails[$i]['ID'], $sSenderId, $aMails[$i]['Body']);
        }
...
function getAvailableStatuses()
{
    ...
    foreach ($oStatuses->aStatuses as $sKey => $aStatus) {
        $sContents .= parseXml($aXmlTemplates["status"], $sKey, getTemplateIcon($aStatus["icon"]),
            _t($aStatus["title"]));
    }
