...
function getOnlineUsersInfo($sUserId, $bInit)
{
    ...
    if ($rResult != null) {
        for ($i = 0; $i < $rResult->rowCount(); $i++) {
            ...
            $sContents .= parseXml($aXmlTemplates['user'], $aUser['ID'], $aUserInfo['nick'], $aUserInfo['sex'],
                $aUserInfo['age'], $aUserInfo['photo'], $aUserInfo['profile'], $sOnline, $sFriend, $sMusic, $sVideo,
                $aUserInfo['desc']);
        }
    }
