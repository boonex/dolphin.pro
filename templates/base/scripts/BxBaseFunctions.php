...
class BxBaseFunctions
{
    ...
    function getProfileMatch($memberID, $profileID)
    {
        ...
        return DesignProgressPos(_t("_XX match", $match_n), $GLOBALS['oTemplConfig']->iProfileViewProgressBar, 100,
            $match_n);
    }
        ...
        if (is_array($aMatches) and !empty($aMatches)) {
            ...
            foreach ($aMatches[3] as $iMarker => $sMarkerValue) {
                if (is_array($aMemberSettings) and array_key_exists($sMarkerValue,
                        $aMemberSettings) and !array_key_exists($sMarkerValue, $this->aSpecialKeys)
                ) {
                    $sTransformText = str_replace('{' . $sMarkerValue . '}', $aMemberSettings[$sMarkerValue],
                        $sTransformText);
                } else {
                    if ($sMarkerValue == 'evalResult' and $sExecuteCode) {
    ...
    function getMemberThumbnail(
        ...
        return $GLOBALS['oSysTemplate']->parseHtmlByName($bCouple ? 'thumbnail_couple' . $sTmplSfx . '.html' : 'thumbnail_single' . $sTmplSfx . '.html',
            array(
                'iProfId'                      => $iId ? $iId : 0,
    ...
    function genObjectsActions(
        ...
        if (!empty($aActionsItem)) {
            ...
            if (!empty($aKeys['ID'])) {
                $sResponceBlock = $GLOBALS['oSysTemplate']->parseHtmlByName($aUsedTemplate['ajaxy_popup'],
                    array('object_id' => $aKeys['ID']));
            }
...
EOF;
    ...
    function getSiteStatBody($aVal, $sMode = '')
    {
        ...
        $sImg  = (false === strpos($aVal['icon'],
            '.') ? '<i class="sys-icon ' . $aVal['icon'] . '"></i>' : '<img src="' . getTemplateIcon($aVal['icon']) . '" alt="" />');
        $sCode =
            '
