...
class BxBaseCmtsView extends BxDolCmts
{
        ...
        if (!$aCmts) {
            ...
            for (reset($aCmts); list ($k, $r) = each($aCmts); ++$i) {
                ...
                if ($r['cmt_rated'] == -1 || $r['cmt_rate'] < $this->_aSystem['viewing_threshold']) {
                    ...
                    $sRet .= _t('_hidden_comment',
                            getNickName($r['cmt_author_id'])) . ' ' . ($r['cmt_replies'] > 0 ? _t('_Show N replies',
                                $r['cmt_replies']) . '. ' : '') . '<a href="javascript:void(0)" onclick="javascript:' . $this->_sJsObjName . '.showReplacement(' . $r['cmt_id'] . ')">' . _t('_show_replacement') . '</a>.';
                    if ($this->isRatable()) {
    ...
    function getComment($iCmtId, $sType = 'new')
    {
        ...
        if ($r['cmt_rated'] == -1 || $r['cmt_rate'] < $this->_aSystem['viewing_threshold']) {
            $sRet .= '<li id="cmt' . $r['cmt_id'] . '-hidden" class="cmt-replacement">';
            $sRet .= _t('_hidden_comment', $r['cmt_author_name']) . ' ' . ($r['cmt_replies'] > 0 ? _t('_Show N replies',
                        $r['cmt_replies']) . '. ' : '') . '<a href="javascript:void(0)" onclick="javascript:' . $this->_sJsObjName . '.showReplacement(' . $r['cmt_id'] . ')">' . _t('_show_replacement') . '</a>.';
            if ($this->isRatable()) {
                $sRet .= $this->_getRateBox($r);
            }
    ...
    function _getRateBox(&$a)
    {
        ...
        return '
            <div class="cmt-rate' . $sClass . '">
                <div class="cmt-points">' . _t((1 == $a['cmt_rate'] || -1 == $a['cmt_rate']) ? '_N point' : '_N points',
            $a['cmt_rate']) . '</div>
    ...
    function _getActionsBox(&$a, $isJustPosted)
    {
        ...
        if ((($isJustPosted && $n) || $isEditAllowedPermanently) && strpos($a['cmt_text'],
    ...
    {
        if ($sType == 'comment') {
            $sSwitcher = '<a class="cmt-post-reply-text inactive" href="javascript:void(0)" onclick="javascript:' . $this->_sJsObjName . '.toggleType(this)"><i class="sys-icon pencil"></i>&#160;' . _t('_Add Your Comment') . '</a>' . (getSettingValue("video_comments",
                    "status",
                    "main") == "enabled" ? '<a class="cmt-post-reply-video" href="javascript:void(0)" onclick="javascript:' . $this->_sJsObjName . '.toggleType(this)"><i class="sys-icon film"></i>&#160;' . _t('_Record Your Comment') . '</a>' : '');
        } else {
            if ($sType == 'reply') {
                $sSwitcher = '<a class="cmt-post-reply-text inactive" href="javascript:void(0)" onclick="javascript:' . $this->_sJsObjName . '.toggleType(this)"><i class="sys-icon pencil"></i>&#160;' . _t('_Reply as text') . '</a>' . (getSettingValue("video_comments",
                        "status",
                        "main") == "enabled" ? '<a class="cmt-post-reply-video" href="javascript:void(0)" onclick="javascript:' . $this->_sJsObjName . '.toggleType(this)"><i class="sys-icon film"></i>&#160;' . _t('_Reply as video') . '</a>' : '');
            }
        }
    ...
    function _getFormBox($sType, $aCmt = array())
    {
        ...
        $sContent = '
			<form name="cmt-post-reply" onsubmit="' . $this->_sJsObjName . '.' . $sFunction . '; return false;">
				...
				<div class="cmt-post-reply-video">' . getApplicationContent('video_comments', 'recorder', array(
                'user'     => $this->_getAuthorId(),
                'password' => $this->_getAuthorPassword(),
                'extra'    => implode('_', array($this->_sSystem . '-' . $this->getId(), $iCmtParentId))
            ), true) . '</div>
    ...
    function _getAuthorIcon($a)
    {
        ...
        if (!$a['cmt_author_id'] || !getProfileInfo($a['cmt_author_id'])) {
            ...
            return '<div class="thumbnail_block thumbnail_block_icon" style="float:none;"><div class="thumbnail_image"><img class="thumbnail_image_file bx-def-thumbnail bx-def-shadow bx-img-retina" src="' . $oFunctions->getSexPic('',
                'small') . '" /></div></div>';
        } else {
