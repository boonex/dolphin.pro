...
class BxBaseFormView extends BxDolForm
{
    ...
    function genRowStandard(&$aInput)
    {
        ...
        $sCode .= $GLOBALS['oSysTemplate']->parseHtmlByName('form_row_standard_' . (!empty($aInput['colspan']) ? 'colspan' : 'simple') . '.html',
            array(
    ...
    function genRowSelectBox(&$aInput)
    {
        ...
        $sCode = $this->getOpenTbody();
        $sCode .= $GLOBALS['oSysTemplate']->parseHtmlByName('form_row_select_' . (!empty($aInput['colspan']) ? 'colspan' : 'simple') . '.html',
            array(
                'element_type'  => 'bx-form-element-' . $aInput['type'],
                'element_class' => $sElementClass,
                'element_attrs' => $this->convertArray2Attrs(!empty($aInput['tr_attrs']) ? $aInput['tr_attrs'] : ''),
            ...
            case 'input_set': // numeric array of inputs
                ...
                foreach ($aInput as $iKey => $aSubInput) {
                    if (!is_int($iKey) or !$aSubInput) {
                        continue;
                    } // parse only integer keys and existing values
    ...
    function genInputTextarea(&$aInput)
    {
        ...
        $aAttrs['class'] =
            ...
            ((isset($aInput['html']) and $aInput['html'] and $this->addHtmlEditor($aInput['html'],
                    $aInput)) ? ' form_input_html' : '');
    ...
    function addHtmlEditor($iViewMode, &$aInput)
    {
        ...
        $this->_sCodeAdd .= $oEditor->attachEditor((!empty($this->aFormAttrs['id']) ? '#' . $this->aFormAttrs['id'] . ' ' : '') . '[name="' . $aInput['name'] . '"]',
            $iViewMode, isset($aInput['dynamic']) ? $aInput['dynamic'] : false);
    ...
    function genInputCaptcha(&$aInput)
    {
        ...
        $sCaptcha .= $this->getInput('input',
            $this->convertArray2Attrs(array('type' => 'hidden', 'name' => $aInput['name'])));
