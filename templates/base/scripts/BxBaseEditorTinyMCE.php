...
class BxBaseEditorTinyMCE extends BxDolEditor
{
    ...
    public function attachEditor($sSelector, $iViewMode = BX_EDITOR_STANDARD, $bDynamicMode = false)
    {
        ...
        $o = new BxDolAlerts('system', 'attach_editor', 0, 0, array(
            'markers'      => &$aMarkers,
            'modules'      => &$aModules,
            'view_mode'    => $iViewMode,
        ...
        if ($bDynamicMode) {
            ...
            $sScript = "<script>
                if ('undefined' == typeof(jQuery(document).tinymce)) {
                    window.tinyMCEPreInit = {base : '" . bx_js_string(BX_DOL_URL_PLUGINS . 'tinymce',
                    BX_ESCAPE_STR_APOS) . "', suffix : '.min', query : ''};
                    $.getScript('" . bx_js_string(BX_DOL_URL_PLUGINS . 'tinymce/tinymce.min.js', BX_ESCAPE_STR_APOS) . "', function(data, textStatus, jqxhr) {
                        $.getScript('" . bx_js_string(BX_DOL_URL_PLUGINS . 'tinymce/jquery.tinymce.min.js',
                    BX_ESCAPE_STR_APOS) . "', function(data, textStatus, jqxhr) {
                            $sInitEditor
                        });
                    });
                } else {
    ...
    protected function _addJsCss($bDynamicMode = false, $sInitEditor = '')
    {
        ...
        $this->_oTemplate->addInjection('injection_head_begin', 'text',
            "<script>window.tinyMCEPreInit = {base : '" . bx_js_string(BX_DOL_URL_PLUGINS . 'tinymce',
                BX_ESCAPE_STR_APOS) . "', suffix : '.min', query : ''};</script>\n");
