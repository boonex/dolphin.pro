...
class BxBaseIndexPageView extends BxDolPageView
{
    ...
    function getBlockCode_Subscribe()
    {
        ...
        $sContent      = $oSubscription->getData() . $GLOBALS['oSysTemplate']->parseHtmlByName('home_page_subscribe.html',
                array(
    ...
    {
        $a = $GLOBALS['MySQL']->fromCache('sys_box_download', 'getAll',
    ...
    function getMembers($sBlockName, $aParams = array(), $iLimit = 16, $sMode = 'last')
    {
        ...
        foreach ($aModes as $sMyMode) {
            ...
            $aDBTopMenu[$sModeTitle] = array('href'    => BX_DOL_URL_ROOT . "index.php?{$sBlockName}Mode=$sMyMode",
                                             'dynamic' => true,
                                             'active'  => ($sMyMode == $sMode)
            );
        }
