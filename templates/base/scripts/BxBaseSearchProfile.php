...
class BxBaseSearchProfile extends BxBaseSearchResultText
{
        ...
        switch ($sParamName) {
            case 'calendar':
                ...
                $this->aCurrent['restriction']['calendar-min'] = array(
                    'value'          => "'{$sParamValue}-{$sParamValue1}-{$sParamValue2} 00:00:00'",
                    'field'          => 'DateReg',
                    'operator'       => '>=',
                    'no_quote_value' => true
                );
                $this->aCurrent['restriction']['calendar-max'] = array(
                    'value'          => "'{$sParamValue}-{$sParamValue1}-{$sParamValue2} 23:59:59'",
                    'field'          => 'DateReg',
                    'operator'       => '<=',
                    'no_quote_value' => true
                );
    ...
    function PrintSearhResult(
        ...
        $sProfileMatch = $isShowMatchPercent ? $GLOBALS['oFunctions']->getProfileMatch($iVisitorID,
            $aProfileInfo['ID']) : '';
        ...
        $sProfileDesc     = $bPublic ? strmaxtextlen($aProfileInfo['DescriptionMe'],
            130) : _t('_sys_profile_private_text_title');
        ...
        if ($aCoupleInfo) {
            ...
            $sProfile2Desc   = $bPublic ? strmaxtextlen($aCoupleInfo['DescriptionMe'],
                130) : _t('_sys_profile_private_text_title');
        ...
        return ($oCustomTemplate) ? $oCustomTemplate->parseHtmlByName($sTemplateName,
            $aKeys) : $GLOBALS['oSysTemplate']->parseHtmlByName($sTemplateName, $aKeys);
    }
    ...
    function displaySearchBox($sCode, $sPaginate = '', $bAdminBox = false)
    {
        ...
        $sCode      = DesignBoxContent(_t($this->aCurrent['title']),
            '<div class="searchContentBlock">' . $sCode . $sClearBoth . '</div>' . $sPaginate, 1);
