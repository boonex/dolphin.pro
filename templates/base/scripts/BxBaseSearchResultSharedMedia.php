...
class BxBaseSearchResultSharedMedia extends BxBaseSearchResult
{
    ...
    function getLatestFile()
    {
                ...
                switch ($sKey) {
                    ...
                    case 'tag':
                        if (strlen($aValue['value']) > 0) {
                            $aWhere[] = "MATCH(`{$this->aCurrent['table']}`.`{$aValue['field']}`) AGAINST ('" . trim(process_db_input($aValue['value'],
                                    BX_TAGS_STRIP)) . "')";
                        }
                        break;
                    case 'allow_view':
        ...
        if ($iFileId != 0) {
            ...
            $aDraw = array(
                'file'       => $this->oTemplate->getFileConcept($aInfo['medID'],
                    array('ext' => $aInfo['medExt'], 'source' => $aInfo['medSource'])),
    ...
    function displaySearchBox($sCode, $sPaginate = '', $bAdminBox = false)
    {
        $sCode  = $GLOBALS['oFunctions']->centerContent($sCode,
                '.sys_file_search_unit') . '<div class="clear_both"></div>';
        $sTitle = _t($this->aCurrent['title']);
        $sFunc  = !$bAdminBox ? 'Content' : 'Admin';
        $sCode  = call_user_func('DesignBox' . $sFunc, $sTitle,
            '<div class="searchContentBlock">' . $sCode . '</div>' . $sPaginate, 1);
    ...
    function addCustomParts()
    {
        if (!$this->bCustomParts) {
            ...
            $this->oTemplate->addLocation($this->aCurrent['name'], BX_DIRECTORY_PATH_MODULES . $sModulePart,
                BX_DOL_URL_MODULES . $sModulePart);
    ...
    function getAlbumsBlock($aSectionParams = array(), $aAlbumParams = array(), $aCustom = array())
    {
        ...
        if ($this->aCurrent['paginate']['totalAlbumNum'] > 0) {
            ...
            if (empty($aCustom['menu_bottom'])) {
                ...
                $aCode['menu_bottom'] = $aCustom['simple_paginate'] ? $oPaginate->getSimplePaginate($aCustom['simple_paginate_url'],
                    -1, -1, $aCustom['simple_paginate_view_all']) : $oPaginate->getPaginate();
            } else {
    ...
    function getAlbumCovers($iAlbumId, $aParams = array())
    {
        ...
        return $this->oModule->oAlbums->getAlbumCoverFiles($iAlbumId,
            array('table' => $this->aCurrent['table'], 'field' => 'ID'),
    ...
    function serviceGetProfileAlbumsBlock($iProfileId, $sSpecUrl = '')
    {
        ...
        return $this->getAlbumsBlock(array(), array('owner' => $iProfileId, 'hide_default' => true),
            array('enable_center' => true, 'paginate_url' => $sPaginateUrl, 'simple_paginate_url' => $sSimpleUrl));
    }
