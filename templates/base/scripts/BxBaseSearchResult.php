...
class BxBaseSearchResult extends BxDolSearchResult
{
    ...
    function displaySearchBox($sCode, $sPaginate = '', $bAdminBox = false)
    {
        ...
        if (isset($this->aCurrent['rss']) && $this->aCurrent['rss']['link']) {
            ...
            $sMenu = BxDolPageView::getBlockCaptionItemCode(time(), array(
                _t('RSS') => array(
                    'href' => $this->aCurrent['rss']['link'] . (false === strpos($this->aCurrent['rss']['link'],
                            '?') ? '?' : '&') . 'rss=1',
                    'icon' => 'rss'
                )
            ));
    ...
    public static function showAdminActionsPanel(
        $sWrapperId,
        $aButtons,
        $sCheckboxName = 'entry',
        $bSelectAll = true,
    ...
    public static function showAdminFilterPanel(
        $sFilterValue,
...
EOF;
    ...
    function getBrowseBlock($aParams, $aCustom = array(), $sMainUrl = '', $bClearJoins = true)
    {
        ...
        if ($aCustom['menu_bottom']) {
            $mixedMenuBottom = $this->getBottomMenu($aCustom['menu_bottom_type'], 0,
                $this->aCurrent['restriction'][$aCustom['menu_bottom_param']]['value'], array(),
                $aCustom['simple_paginate']);
        }

        return array(
            'code'        => $sCode,
            'menu_top'    => $aMenuTop,
            'menu_bottom' => $mixedMenuBottom,
            'wrapper'     => $bWrapDisable
        );
    }
