        ...
        function _genSexSection(&$oObject, $sCurrentKey)
        {
            ...
            if (is_array($aCurrentArray) and !empty($aCurrentArray)) {
                foreach ($aCurrentArray AS $sKey => $sValue) {
                    ...
                    switch ($sKey) {
                        case 'male'   :
                            $sSexSection .= $oObject->genBrowseLink(_t('_By Male'), $sValue,
                                $oObject->genLinkLocation('sex', 'male'), 'male.png');
                            break;
                        case 'female' :
                            $sSexSection .= $oObject->genBrowseLink(_t('_By Female'), $sValue,
                                $oObject->genLinkLocation('sex', 'female'), 'female.png');
                            break;
                        case 'all' :
                            $sSexSection .= $oObject->genBrowseLink($sValue, null,
                                $oObject->genLinkLocation('sex', $sKey), 'post_featured.png');
                            break;
                        default :
                            $sSexSection .= $oObject->genBrowseLink(_t($aPreValues['Sex'][$sKey]['LKey']), $sValue,
                                $oObject->genLinkLocation('sex', $sKey), 'tux.png');
                            break;
                    }
        ...
        function _genAgeSection(&$oObject, $sCurrentKey)
        {
            ...
            if (is_array($aCurrentArray) and $iArrayCount) {
                foreach ($aCurrentArray AS $sKey => $sValue) {
                    ...
                    if ($sKey == 'all') {
                        $sAgeSection .= $oObject->genBrowseLink($sValue, null, $oObject->genLinkLocation('age', $sKey),
                            'post_featured.png');
                    } else {
                        $sAgeSection .= $oObject->genBrowseLink($sKey, $sValue, $oObject->genLinkLocation('age', $sKey),
                            'birthday.png');
                    }
        ...
        function _genCountrySection(&$oObject, $sCurrentKey)
        {
            ...
            if (is_array($aCurrentArray) and !empty($aCurrentArray)) {
                foreach ($aCurrentArray AS $sKey => $sValue) {
                    ...
                    if ($sKey == 'all') {
                        $sCountrySection .= $oObject->genBrowseLink($sValue, null,
                            $oObject->genLinkLocation('country', $sKey), 'post_featured.png');
                    } else {
                        $sCountryName = strtolower($sKey);
                        $sImagePath   = $site['flags'] . $sCountryName . '.gif';
                        $sCountrySection .= $oObject->genBrowseLink($sKey, $sValue,
                            $oObject->genLinkLocation('country', $sCountryName), $sImagePath, false);
                    }
    ...
    function getBlockCode_SearchedMembersBlock()
    {
        ...
        while (true == ($aRow = $rResult->fetch())) {
            ...
            if ($aRow['Couple']) {
                ...
                $sOutputHtml .= $this->oSearchProfileTmpl->PrintSearhResult($aRow, $aCoupleInfo,
                    ($iIndex % 2 ? $aExtendedCss : array()), $sTemplateName);
            } else {
                $sOutputHtml .= $this->oSearchProfileTmpl->PrintSearhResult($aRow, array(),
                    ($iIndex % 2 ? $aExtendedCss : array()), $sTemplateName);
            }
            $iIndex++;
        }
