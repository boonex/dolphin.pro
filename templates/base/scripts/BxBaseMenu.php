...
class BxBaseMenu extends BxDolMenu
{
    ...
    */
    function setCustomSubActions(&$aKeys, $sActionsType, $bSubMenuMode = true)
    {
        ...
        $this->sCustomActions = $GLOBALS['oFunctions']->genObjectsActions($aKeys, $sActionsType, $bSubMenuMode,
            'actions_submenu', 'action_submenu');
    }
...
EOF;
    ...
    */
    function genTopItems($aParams = array())
    {
        ...
        foreach ($this->aTopMenu as $iItemID => $aItem) {
            ...
            if ($aItem['Caption'] == "{profileNick}" && $this->aMenuInfo['profileNick'] == '') {
                continue;
            }
        ...
        foreach ($this->aTopMenu as $iItemID => $aItem) {
            ...
            if ($this->iJumpedMenuID > 0 && $bGroupInMore && $iCounter == $this->iElementsCntInLine) {
                ...
                $sCode .= $this->genTopItem(_t($aItemJmp['Caption']), $aItemJmp['Link'], $aItemJmp['Target'],
                    $aItemJmp['Onclick'], $bJumpActive, $this->iJumpedMenuID, $isBold);
            ...
            if ($this->iJumpedMenuID == 0 || $iItemID != $this->iJumpedMenuID) {
                if ($bGroupInMore && $this->iElementsCntInLine <= $iCounter) {
                    $sCode .= $this->genTopItemMore(_t($aItem['Caption']), $aItem['Link'], $aItem['Target'],
                        $aItem['Onclick'], $bActive, $iItemID);
                } else {
                    $sCode .= $this->genTopItem(_t($aItem['Caption']), $aItem['Link'], $aItem['Target'],
                        $aItem['Onclick'], $bActive, $iItemID, $isBold, $sImage);
                }
            }
    ...
    */
    ...
    function genSubItems($iTItemID = 0)
    {
        ...
        foreach ($this->aTopMenu as $iItemID => $aItem) {
            ...
            $aItem['Link']    = $this->replaceMetas($aItem['Link']);
            $aItem['Onclick'] = $this->replaceMetas($aItem['Onclick']);
            $sSubItems .= (!$bFirst ? '<div class="sys-bullet"></div>' : '') . $this->genSubItem(_t($aItem['Caption']),
                    $aItem['Link'], $aItem['Target'], $aItem['Onclick'], $iItemID == $this->aMenuInfo['currentCustom']);
    ...
    */
    ...
    {
        $sLink = (strpos($sLink, 'http://') === false && strpos($sLink,
                'https://') === false && !strlen($sOnclick)) ? $this->sSiteUrl . $sLink : $sLink;
...
EOF;
        ...
        if ($this->sCustomSubHeader == '' && !empty($this->aMenuInfo['profileID'])) {
            ...
            if ($this->aMenuInfo['memberID'] == $this->aMenuInfo['profileID']) {
                ...
                $sProfStatusMessage = $GLOBALS['oSysTemplate']->parseHtmlByName('navigation_menu_status.html',
                    $aTmplVars);
            } else {
        ...
        $sCaptionWL = $this->sCustomSubHeader != '' ? $this->sCustomSubHeader : $this->genSubHeaderCaption($this->aTopMenu[$iFirstID],
            $sCaption);
    ...
    function genSubHeaderCaptionBreadcrumb(
        $aItem,
        $sCaption,
    ...
    function getAllSubMenus($iItemID, $bActive = false)
    {
        ...
        foreach ($aTTopMenu as $iTItemID => $aTItem) {
            ...
            if ($iItemID == $aTItem['Parent']) {
                ...
                $aTItem['Link'] = str_replace("{memberID}", isset($aMenuInfo['memberID']) ? $aMenuInfo['memberID'] : '',
                    $aTItem['Link']);
                $aTItem['Link'] = str_replace("{memberNick}",
                    isset($aMenuInfo['memberNick']) ? $aMenuInfo['memberNick'] : '', $aTItem['Link']);
                $aTItem['Link'] = str_replace("{memberLink}",
                    isset($aMenuInfo['memberLink']) ? $aMenuInfo['memberLink'] : '', $aTItem['Link']);

                $aTItem['Link']    = str_replace("{profileID}",
                    isset($aMenuInfo['profileID']) ? $aMenuInfo['profileID'] : '', $aTItem['Link']);
                $aTItem['Onclick'] = str_replace("{profileID}",
                    isset($aMenuInfo['profileID']) ? $aMenuInfo['profileID'] : '', $aTItem['Onclick']);

                $aTItem['Link']    = str_replace("{profileNick}",
                    isset($aMenuInfo['profileNick']) ? $aMenuInfo['profileNick'] : '', $aTItem['Link']);
                $aTItem['Onclick'] = str_replace("{profileNick}",
                    isset($aMenuInfo['profileNick']) ? $aMenuInfo['profileNick'] : '', $aTItem['Onclick']);

                $aTItem['Link'] = str_replace("{profileLink}",
                    isset($aMenuInfo['profileLink']) ? $aMenuInfo['profileLink'] : '', $aTItem['Link']);

                $aTItem['Onclick'] = str_replace("{memberID}",
                    isset($aMenuInfo['memberID']) ? $aMenuInfo['memberID'] : '', $aTItem['Onclick']);
                $aTItem['Onclick'] = str_replace("{memberNick}",
                    isset($aMenuInfo['memberNick']) ? $aMenuInfo['memberNick'] : '', $aTItem['Onclick']);
                $aTItem['Onclick'] = str_replace("{memberPass}",
                    getPassword(isset($aMenuInfo['memberID']) ? $aMenuInfo['memberID'] : ''), $aTItem['Onclick']);

                $sElement = $this->getCustomMenuItem(_t($aTItem['Caption']), $aTItem['Link'], $aTItem['Target'],
                    $aTItem['Onclick'], ($iTItemID == $aMenuInfo['currentCustom']));
    ...
    function getCustomMenuItem($sText, $sLink, $sTarget, $sOnclick, $bActive, $bSub = false)
    {
        ...
        $sILink        = (strpos($sLink, 'http://') === false && strpos($sLink,
                'https://') === false && !strlen($sOnclick)) ? $this->sSiteUrl . $sLink : $sLink;
...
EOF;
    ...
    function getSubItems($iParentId = 0)
    {
        ...
        foreach ($this->aTopMenu as $iItemID => $aItem) {
            ...
            if (strpos($aItem['Link'], 'http://') === false && strpos($aItem['Link'],
                    'https://') === false && !strlen($aItem['Onclick'])
            ) {
