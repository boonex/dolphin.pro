...
class BxDolPEditProcessor extends BxDolPageView
{
    ...
    function getBlockCode_Privacy()
    {
        ...
        if ($oForm->isSubmittedAndValid()) {
            ...
            if ((int)db_res("UPDATE `Profiles` SET `allow_view_to`='" . $iAllowViewTo . "' WHERE `ID`='" . $iProfileId . "' LIMIT 1") > 0) {
                ...
                $oZ = new BxDolAlerts('profile', 'edit', $iProfileId, 0,
                    array('OldProfileInfo' => $aProfileInfo, 'privacy' => $iAllowViewTo));
                $oZ->alert();
            }
        }
    ...
    function getBlockCode_Membership()
    {
        ...
        if ($aMembershipCurrent['ID'] != MEMBERSHIP_ID_STANDARD) {
            $sMembershipCurrent .= ', ' . (!isset($aMembershipCurrent['DateExpires']) ? _t('_MEMBERSHIP_EXPIRES_NEVER') : _t('_MEMBERSHIP_EXPIRES',
                    defineTimeInterval($aMembershipCurrent['DateExpires'])));
        }
