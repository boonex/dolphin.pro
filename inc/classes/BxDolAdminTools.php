...
EOF;
        ...
        $aTopItems = array(
            'main_menu0' => array('href'    => 'javascript:void(0)',
                                  'onclick' => 'javascript:switchAdmPage(this)',
                                  'title'   => _t('_adm_admtools_Audit'),
                                  'active'  => 1
            ),
            'main_menu1' => array('href'    => 'javascript:void(0)',
                                  'onclick' => 'javascript:switchAdmPage(this)',
                                  'title'   => _t('_adm_admtools_Permissions'),
                                  'active'  => 0
            ),
            'main_menu2' => array('href'    => 'javascript:void(0)',
                                  'onclick' => 'javascript:switchAdmPage(this)',
                                  'title'   => _t('_adm_admtools_phpinfo'),
                                  'active'  => 0
            ),
        );
    ...
    function GenAuditPage()
    {
        ...
        $aPhpSettings = array(
            ...
            'disable_functions'    => array(
                'op'  => 'without',
                'val' => 'exec,shell_exec,popen,eval,assert,create_function,phpinfo,getenv,ini_set,mail,fsockopen,chmod,parse_ini_file'
            ),
        ...
        $aDolphinOptimizationSettings = array(
            ...
            'DB cache' => array('enabled'      => 'sys_db_cache_enable',
                                'cache_engine' => 'sys_db_cache_engine',
                                'check_accel'  => true
            ),

            'Page blocks cache' => array('enabled'      => 'sys_pb_cache_enable',
                                         'cache_engine' => 'sys_pb_cache_engine',
                                         'check_accel'  => true
            ),

            'Member menu cache' => array('enabled'      => 'always_on',
                                         'cache_engine' => 'sys_mm_cache_engine',
                                         'check_accel'  => true
            ),

            'Templates Cache' => array('enabled'      => 'sys_template_cache_enable',
                                       'cache_engine' => 'sys_template_cache_engine',
                                       'check_accel'  => true
            ),

            'CSS files cache' => array('enabled'      => 'sys_template_cache_css_enable',
                                       'cache_engine' => '',
                                       'check_accel'  => false
            ),

            'JS files cache' => array('enabled'      => 'sys_template_cache_js_enable',
                                      'cache_engine' => '',
                                      'check_accel'  => false
            ),

            'Compression for CSS/JS cache' => array('enabled'      => 'sys_template_cache_compress_enable',
                                                    'cache_engine' => '',
                                                    'check_accel'  => false
            ),
        );
                ...
                <ul>
                    ...
                    foreach ($aPhpSettings as $sName => $r) {
                        ...
                        if ($a['res']) {
                            ...
                            echo "<b class='warn'>WARNING</b> (should be {$r['op']} " . $this->format_output($r['val'],
                                    $r) . ")";
                        } else {
                            echo "<b class='fail'>FAIL</b> (must be {$r['op']} " . $this->format_output($r['val'],
                                    $r) . ")";
                        }
                        echo "</li>\n";
                    }
                ...
                <ul>
                    ...
                    if (!$bIsNginx) {
                        foreach ($aRequiredApacheModules as $sName => $sNameCompiledName) {
                            echo '<li>' . $sName . ' - ' . $this->checkApacheModule($sName,
                                    $sNameCompiledName) . '</li>';
                        }
                    }
                    ?>
                </ul>
        ...
        <p>
            Hardware requirements can not be determined automatically - <a href="#manual_audit">manual server audit</a>
            may be reqired.
        </p>
        ...
        <ul>
            ...
            <li>
                ...
                <br />
                Please <a href="javascript:void(0);" onclick="switchAdmPage($('#main_menu1'));">click here</a> to find
                out if dolphin permissions are correct.
            </li>
            <li>
                <b>ffmpeg</b>
                <pre class="code"><?php echo `{$sDolphinPath}flash/modules/global/app/ffmpeg.exe 2>&1`; ?></pre>
                if you don't know if output is correct then <a href="#manual_audit">manual server audit</a> may be
                reqired.
            </li>
            <li>
                ...
                <b>mail sending - </b>
                <span id="bx-sys-adm-audit-test-email"><a href="javascript:void(0);"
                                                          onclick="bx_sys_adm_audit_test_email()">click here</a> to send test email to <?php echo $sEmailToCkeckMailSending; ?></span>
            </li>
            <li>
                <b>cronjobs</b>
                <pre class="code"><?php echo `crontab -l 2>&1`; ?></pre>
                if you are unsure if output is correct then <a href="#manual_audit">manual server audit</a> may be
                reqired.
            </li>
            <li>
                <b>media server</b>
                <br />
                Please follow <a href="<?php echo $GLOBALS['site']['url_admin']; ?>flash.php">this link</a> to check
                media server settings. Also you can try video chat - if video chat is working then most probably that
                flash media server is working correctly, however it doesn't guarantee that all other flash media server
            ...
            <li>
                ...
                functioning properly. If it is working but '[L[' signs are displayed everywhere, then you need to <a
                    href="<?php echo BX_DOL_URL_ROOT; ?>forum/?action=goto&manage_forum=1">compile language file</a>
                (you maybe be need to compile language file separately for every language and template you have).
            </li>
        </ul>
            ...
            <li><b>MySQL</b>:
                <ul>
                    ...
                    foreach ($aMysqlSettings as $sName => $r) {
                        ...
                        echo "<li><b>$sName</b> = " . $this->format_output($a['real_val'],
                                $r) . " - " . ($a['res'] ? '<b class="ok">OK</b>' : "<b class='fail'>FAIL</b> (must be {$operation} " . $this->format_output($r['val'],
                                    $r) . ")") . "</li>\n";
                    }
                    ?>
                </ul>
            ...
            <li><b>Web-server</b>:
                <ul>
                    <li>
                        ...
                        <a href="<?php echo $this->getUrlForGooglePageSpeed('LeverageBrowserCaching'); ?>">click here to
                            check it in Google Page Speed</a>
                        <br />
                        If it is not enabled then please consider implement this optimization, since it improve
                        perceived site speed and save the bandwidth, refer to <a target="_blank"
                                                                                 href="https://www.boonex.com/trac/dolphin/wiki/HostingServerSetupRecommendations#Usersidecachingforstaticcontent">this
                    ...
                    <li>
                        <b>Server-side content compression</b> = can be checked <a href="#manual_audit">manually</a> or
                        in "Page Speed" tool build-in into browser.
                        <br />
                        If it is not enabled then please consider implement this optimization, since it improve
                        perceived site speed and save the bandwidth, refer to <a
                            href="https://www.boonex.com/trac/dolphin/wiki/HostingServerSetupRecommendations#Serversidecontentcompression">this
                            tutorial</a> on how to do this.
                        </textarea>
                        <br />
                        <?php
        ...
        <p>
            Some things can not be determined automatically, manual server audit is required to check it. If you don't
            know how to do it by yourself you can submit <a target="_blank" href="https://www.boonex.com/help/contact">BoonEx
                Server Audit Request</a>.
        </p>
    ...
    function getPhpAccelerator()
    {
        $aAccelerators = array(
            ...
            'OPcache'      => array('op'  => 'val',
                                    'val' => function_exists('opcache_get_status') && ($a = opcache_get_status(false)) ? $a['opcache_enabled'] : false
            ),
        );
    ...
    {
        $sEmailToCkeckMailSending = getParam('site_email');
        $mixedRet                 = sendMail($sEmailToCkeckMailSending, 'Audit Test Email',
