...
class BxDolFilesPageHome extends BxDolPageView
{
    ...
    {
        $this->oSearch->clearFilters(array('activeStatus', 'allow_view', 'album_status', 'albumType', 'ownerStatus'),
    ...
    function getBlockCode_Special()
    {
        ...
        $aCode                                               = $this->oSearch->getBrowseBlock(array('featured'   => 1,
                                                                                                    'allow_view' => $this->aVisible
        ), $aCustom, BX_DOL_URL_ROOT . $this->oConfig->getBaseUri() . 'home');
        if ($this->oSearch->aCurrent['paginate']['totalNum'] > 0) {
