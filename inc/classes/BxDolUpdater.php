...
class BxDolUpdater extends BxDolInstaller
{
    ...
    function update($aParams)
    {
        ...
        $aResult = array(
            'operation_title' => _t('_adm_txt_modules_operation_update', $this->_aConfig['title'],
                $this->_aConfig['version_from'], $this->_aConfig['version_to'])
        );

        //--- Check for module to update ---//
        $aModuleInfo = $MySQL->getRow("SELECT `id`, `version` FROM `sys_modules` WHERE `path`= ? AND `uri`= ? LIMIT 1",
            [$this->_aConfig['module_dir'], $this->_aConfig['module_uri']]);
        if (!$aModuleInfo) {
        ...
        //--- Check hash ---//
        $aFilesOrig = $MySQL->getAllWithKey("SELECT `file`, `hash` FROM `sys_modules_file_tracks` WHERE `module_id`= ?",
            "file", [$aModuleInfo['id']]);
        ...
        foreach ($aFiles as $aFile) {
            if (!isset($aFilesOrig[$aFile['file']]) || $aFilesOrig[$aFile['file']]['hash'] != $aFile['hash']) {
                return array_merge($aResult, array(
                    'message' => $this->_displayResult('check_module_hash', false,
    ...
    function actionUpdateFiles($bInstall = true)
    {
        ...
        return $oFtp->copy($sPath . '*',
            'modules/' . $this->_aConfig['module_dir']) ? BX_DOL_INSTALLER_SUCCESS : BX_DOL_INSTALLER_FAILED;
    }
