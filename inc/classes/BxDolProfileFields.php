...
class BxDolProfileFields extends Thing
{
            ...
            case 'TermsOfUse':
                ...
                if (2 == getParam('ipBlacklistMode') && bx_is_ip_blocked()) {
                    return _t('_Sorry, your IP been banned');
                            ...
                            'join')) || $i || $oBxDolStopForumSpam->isSpammer(array(
                        'email' => $_POST['Email'][0],
    ...
    function getFormCode($aParams = null)
    {
        ...
        $sCustomHtmlBefore = '';
        $sCustomHtmlAfter  = '';
        $oAlert            = new BxDolAlerts('profile', 'show_profile_form', 0, 0, array(
            'oProfileFields'    => $this,
            'oForm'             => $oForm,
    ...
    function getFormsSearch($aParams)
    {
        ...
        foreach ($this->aBlocks as $iBlockId => $aBlock) {
            ...
            $aInputs[] = array(
                'type'  => 'hidden',
                'name'  => 'show',
                'value' => isset($_REQUEST['show']) && in_array($_REQUEST['show'],
                    $aShowModes) ? $_REQUEST['show'] : '',
            );
                ...
                switch ($aItem['Name']) {
                    ...
                    case 'Agree':
                        $aInput           = array(
                            'type'    => 'custom',
                            'colspan' => true,
                            'content' => _t('_join_form_note',
                                    BX_DOL_URL_ROOT) . '<input type="hidden" name="Agree" />',
                        );
                        $aItem['Caption'] = '';
                        break;
                }
    ...
    function convertValues4Input($mValues, $sUseLKey = 'LKey', $sFormName = '')
    {
        ...
        if (is_array($mValues)) {
            ...
            if (isset($GLOBALS['aPreValues'][$sKey])) {
                ...
                if (!empty($sFormName) && !empty($this->aAddPleaseSelect[$sFormName]) && is_array($this->aAddPleaseSelect[$sFormName]) && in_array($sKey,
                        $this->aAddPleaseSelect[$sFormName])
                ) {
