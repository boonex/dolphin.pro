...
class BxDolFriendsPageView extends BxDolPageView
{
    ...
    function getBlockCode_Friends()
    {
        ...
        foreach ($aAllFriends as $iFriendID => $aFriendsPrm) {
            ...
            if ($aMemberInfo['Couple']) {
                ...
                $sOutputHtml .= $this->oSearchProfileTmpl->PrintSearhResult($aMemberInfo, $aCoupleInfo,
                    ($iIndex % 2 ? $aExtendedCss : array()), $sTemplateName);
            } else {
                $sOutputHtml .= $this->oSearchProfileTmpl->PrintSearhResult($aMemberInfo, array(),
                    ($iIndex % 2 ? $aExtendedCss : array()), $sTemplateName);
            }
            $iIndex++;
        }
        ...
        $sRequest .= bx_encode_url_params($_GET, array(),
                $aGetParams) . 'page={page}&per_page={per_page}&sort={sorting}';
        ...
        if ($iMemberId) {
            ...
            if ($iOnlineFriends > $iOldCount) {
                ...
                for ($i = $iOldCount; $i < $iOnlineFriends; $i++) {
                    ...
                    $aNotifyMessages[] = array(
                        'message' => $oSysTemplate->parseHtmlByName('member_menu_notify_window.html',
    ...
    public static function get_member_menu_friends_list($iMemberId = 0)
    {
        ...
        $sOutputCode = $GLOBALS['oSysTemplate']->parseHtmlByName('view_friends_member_menu_friends_list.html',
            $aTemplateKeys);
