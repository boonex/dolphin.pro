...
class BxDolTextTemplate extends BxDolModuleTemplate
{
    ...
    function displayAdminBlock($aParams)
    {
        ...
        $aResult = array(
            ...
            'filter'             => $oSearchResult->showAdminFilterPanel($this->_oDb->unescape($aParams['filter_value']),
                $sModuleUri . '-filter-txt', $sModuleUri . '-filter-chb', $sModuleUri . '-filter'),
            'content'            => $this->displayList($aParams),
            'control'            => $oSearchResult->showAdminActionsPanel($this->sCssPrefix . '-view-admin', $aButtons,
                $sModuleUri . '-ids')
        );
    ...
    function displayItem($aParams, &$aEntry)
    {
        ...
        $aTmplVars = array(
            ...
            'class'           => !in_array($sSampleType,
                array('view')) ? ' ' . $this->sCssPrefix . '-text-snippet ' : '',
