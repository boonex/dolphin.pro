...
class BxDolTextModule extends BxDolModule
{
    ...
    function getBlockAction($sUri)
    {
        ...
        $aButton       = $oSubscription->getButton($this->getUserId(), $this->_oConfig->getSubscriptionsSystemName(),
            '', $aEntry['id']);
        ...
        if ($this->_isDeleteAllowed()) {
            $this->_oTemplate->addJsTranslation(array(
                '_' . $sModuleUri . '_msg_success_delete',
                '_' . $sModuleUri . '_msg_failed_delete'
            ));
        ...
        return $oSubscription->getData() . $GLOBALS['oFunctions']->genObjectsActions($aReplacement,
            $this->_oConfig->getActionsViewSystemName());
    }
    ...
    function getCategoryContent($sCategory, $iPage = 1, $iPerPage = 0)
    {
        ...
        return array(
            $sCategoryDisplay,
            $this->getSearchContent(array('category' => $sCategory), 'category/' . title2uri($sCategoryDisplay) . '/',
                $iPage, $iPerPage)
        );
    }
    ...
    function getCalendarContent($iYear, $iMonth, $iDay, $iPage, $iPerPage)
    {
        ...
        return $this->getSearchContent(
            array(
                'calendar-min' => array(
                    'value'          => "UNIX_TIMESTAMP('" . $iYear . "-" . $iMonth . "-" . $iDay . " 00:00:00')",
                    'field'          => 'when',
                    'operator'       => '>=',
                    'no_quote_value' => true
                ),
                'calendar-max' => array(
                    'value'          => "UNIX_TIMESTAMP('" . $iYear . "-" . $iMonth . "-" . $iDay . " 23:59:59')",
                    'field'          => 'when',
                    'operator'       => '<=',
                    'no_quote_value' => true
                ),
    ...
    function getSettingsForm($mixedResult)
    {
        ...
        $iId = (int)$this->_oDb->getOne("SELECT `ID` FROM `sys_options_cats` WHERE `name`='" . ucwords(str_replace('_',
                ' ', $sUri)) . "'");
        if (empty($iId)) {
    ...
    {
        $sUri        = $this->_oConfig->getUri();
        $oCategories = new BxTemplCategoriesModule(array('type' => $this->_oConfig->getCategoriesSystemName()),
            _t('_categ_users'), BX_DOL_URL_ROOT . $this->_oConfig->getBaseUri() . 'category');
    ...
    {
        $sUri  = $this->_oConfig->getUri();
        $oTags = new BxTemplTagsModule(array('type' => $this->_oConfig->getTagsSystemName(), 'orderby' => 'popular'),
            _t('_' . $sUri . '_bcaption_all_tags'), BX_DOL_URL_ROOT . $this->_oConfig->getBaseUri() . 'tags');
    ...
    function actionGetEntries($sSampleType = 'all', $iStart = 0, $iPerPage = 0)
    {
        ...
        return $this->_oTemplate->displayList(array(
            ...
            'start'         => $iStart,
            'count'         => $iPerPage,
            'filter_value'  => isset($_POST['filter_value']) ? process_db_input($_POST['filter_value'],
                BX_TAGS_STRIP) : ''
        ));
    }
    ...
    function actionCalendar($iYear = 0, $iMonth = 0, $iDay = 0, $iStart = 1, $iPerPage = 0)
    {
        ...
        switch ($iArgs) {
            ...
            case 3:
            case 5:
                $sContent = $this->getCalendarContent((int)$iYear, (int)$iMonth, (int)$iDay, (int)$iStart,
                    (int)$iPerPage);
    ...
    function actionCategories()
    {
        $sUri        = $this->_oConfig->getUri();
        $oCategories = new BxTemplCategoriesModule(array('type' => $this->_oConfig->getCategoriesSystemName()),
            _t('_categ_users'), BX_DOL_URL_ROOT . $this->_oConfig->getBaseUri() . 'categories');
    ...
    function actionMobileLatestEntries($iPage = 1)
    {
        ...
        foreach ($aEntries as $aEntry) {
            $aVars = array(
                'content' => '<h2>' . $aEntry['caption'] . '</h2>' . getLocaleDate($aEntry['when_uts'],
                        BX_DOL_LOCALE_DATE),
    ...
    function _actFeatured($aIds, $bPositive = true)
    {
        ...
        if ($bResult) {
            foreach ($aIds as $iId) {
                ...
                bx_import('BxDolAlerts');
                $oAlert = new BxDolAlerts($this->_oConfig->getAlertsSystemName(), 'featured', $iId,
                    BxDolTextData::getAuthorId());
                $oAlert->alert();
                //--- Entry -> Featured for Alerts Engine ---//
            }
        }
    ...
    function _actPublish($aIds, $bPositive = true)
    {
        ...
        $bResult = $this->_oDb->updateEntry($aIds,
            array('status' => ($bPositive ? BX_TD_STATUS_ACTIVE : BX_TD_STATUS_INACTIVE)));
        if ($bResult) {
            foreach ($aIds as $iId) {
                //--- Entry -> Publish/Unpublish for Alerts Engine ---//
                $oAlert = new BxDolAlerts($this->_oConfig->getAlertsSystemName(),
                    ($bPositive ? 'publish' : 'unpublish'), $iId, BxDolTextData::getAuthorId());
                $oAlert->alert();
                //--- Entry -> Publish/Unpublish for Alerts Engine ---//
    ...
    function _actDelete($aIds)
    {
        ...
        if ($bResult) {
            ...
            foreach ($aIds as $iId) {
                ...
                $oAlert = new BxDolAlerts($this->_oConfig->getAlertsSystemName(), 'delete', $iId,
                    BxDolTextData::getAuthorId());
                $oAlert->alert();
                //--- Entry -> Delete for Alerts Engine ---//
