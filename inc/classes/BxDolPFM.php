...
class BxDolPFM
{
    function __construct($iArea)
    {
        $this->aColNames = array(
            1 => array('Page'         => 'Join',
                       'Order'        => 'JoinOrder',
                       'Block'        => 'JoinBlock',
                       'ShowSysItems' => 'Couple,Captcha,TermsOfUse,ProfilePhoto',
                       'EditAdd'      => array('JoinPage')
            ),
            ...
            3 => array('Page'         => 'Edit',
                       'Order'        => 'EditAdmOrder',
                       'Block'        => 'EditAdmBlock',
                       'ShowSysItems' => 'Featured,Status,Membership'
            ),
            4 => array('Page'         => 'Edit',
                       'Order'        => 'EditModOrder',
                       'Block'        => 'EditModBlock',
            ...
            5 => array('Page'         => 'View',
                       'Order'        => 'ViewAdmOrder',
                       'Block'        => 'ViewAdmBlock',
                       'ShowSysItems' => 'ID,DateReg,DateLastEdit,DateLastLogin,DateLastNav,Status,Age'
            ),
            6 => array('Page'         => 'View',
                       'Order'        => 'ViewMembOrder',
                       'Block'        => 'ViewMembBlock',
            ...
            <tr>
                ...
                <td class="value">
                    <input type="text" maxlength="255" class="input_text" name="Name"
                        ...
                        <?php if ($aField['Type'] == 'system' or !$aField['Deletable']) {
            ...
            <tr>
                ...
                <td class="value">
                    <textarea class="input_text"
                              name="Desc"><?= htmlspecialchars($this->getLangString($this->fieldDesc)); ?></textarea>
                </td>
                <td class="info">
        ...
        <table class="field_edit_tab bx-def-margin-sec-topbottom"> <!--Controls-->
            <tr>
                <td class="buttons" colspan="2">
                    <input type="submit" name="action-save" value="<?= bx_html_attribute(_t('_Save')) ?>"
                           class="bx-btn bx-btn-small" />
                    <?php

                    if ($aField['Type'] != 'system' and $aField['Deletable']) {
                        ?>
                        <input type="submit" name="action-delete" value="<?= bx_html_attribute(_t('_Delete')) ?>"
                               onclick="return confirm('<?= $sSureDeleteItemC ?>');" class="bx-btn bx-btn-small" />
                        <?php
                    }
                    ?>
                </td>
    ...
    function getFieldsOptionsList($sSelected, $sType)
    {
        ...
        switch ($sType) {
            case 'Keyword':
                $sWhere = "`Type` = 'text' OR `Type` = 'area' OR `Type` = 'html_area'";
                break;
            case 'Couple' :
                $sWhere = "`Type` != 'block' AND `Type` != 'system' AND `Deletable` = 1";
                break;
            default       :
                $sWhere = "0";
        }
    ...
    function getMatchFields($aField)
    {
        ...
        $sQuery    = "SELECT `ID`, `Name` FROM `sys_profile_fields` WHERE FIND_IN_SET( `Type`, '" . implode(',',
                $aSelectFields) . "' )";
                ...
                <tr <?= $aInput['row_id'] ? ('id="' . $aInput['row_id'] . '"') : '' ?>>
                    ...
                    <td class="value">
                        ...
                        switch ($aInput['type']) {
                            case 'textarea':
                                ?>
                                <textarea name="<?= $sInputName ?>"
                                          class="input_text"><?= htmlspecialchars($aInput['value']) ?></textarea>
                                <?php
                                break;
                            case 'checkbox':
                                ?>
                            ...
                            case 'text':
                                ?>
                                <input type="text" name="<?= $sInputName ?>"
                                       value="<?= htmlspecialchars($aInput['value']) ?>" class="input_text" />
                                <?php
                                break;
                            case 'select':
                                ?>
                                <select name="<?= $sInputName ?>" class="select_type">
                                    ...
                                    foreach ($aInput['values'] as $sKey => $sValue) {
                                        ?>
                                        <option
                                            value="<?= $sKey ?>" <?= ($sKey == $aInput['value']) ? 'selected="selected"' : '' ?>><?= $sValue ?></option>
                                        <?php
                                    }
                                    ?>
                                </select>
                            ...
                            case 'values':
                                if (substr($aInput['value'], 0, 2) == $this->sLinkPref) { //it is link
                                    ...
                                    ?>
                                    <input type="hidden" name="<?= $sInputName ?>"
                                           value="<?= htmlspecialchars($aInput['value']) ?>" />
                                    ...
                                    <a href="javascript:void(0);" onclick="activateValuesEdit( this );"
                                       title="Change link" style="margin-left:20px;">
                                    ...
                                    ?>
                                    <textarea class="input_text"
                                              name="<?= $sInputName ?>"><?= htmlspecialchars($aInput['value']) ?></textarea>
                                    <?php
                                }
                                break;
                        }
    ...
    function saveItem($aData)
    {
        ...
        if ($aItem['Type'] != 'system' and $aItem['Deletable']) { //we can change the name
            ...
            if ($sName === '') {
                ...
                $bHaveErrors = true;
            } elseif ($GLOBALS['MySQL']->getOne("SELECT COUNT(*) FROM `sys_profile_fields` WHERE `Name`='" . $sName . "' AND `ID`<>'" . $iItemID . "'") || ($sName != $aItem['Name'] && $GLOBALS['MySQL']->isFieldExists('Profiles',
                        $sName))
            ) {
            ...
            if (!$bHaveErrors) { // do not continue if have errors
                ...
                if ($sType == 'select_one' or $sType == 'select_set') {
                    ...
                    if ($sValues === '') {
                        ...
                        if (substr($sValues, 0, 2) == $this->sLinkPref and !$this->checkValuesLink(substr($sValues,
                                2))
                        ) {
                    ...
                    switch ($sType) {
                        ...
                        case 'date':
                            if ($aData['Default'] === '') {
                                ...
                                $aUpdate['Default'] = date('Y-m-d',
                                    strtotime(trim(process_pass_data($aData['Default']))));
                            }
                            break;
                        ...
                        case 'select_one':
                            ...
                            if ($sDefault === '') {
                                ...
                                if ($this->checkSelectDefault($sValues, $sDefault)) {
                                    ...
                                    $this->genSaveItemError(_t('_adm_fields_error_you_entered_incorrect_value'),
                                        'Default');
                                    $bHaveErrors = true;
                                }
                            }
    ...
    function doUpdateItem($aItem, $aUpdate)
    {
        ...
        if ( //we need alter Profiles table
            ...
            if ($aAlter['Type'] == 'select_one' or $aAlter['Type'] == 'select_set') { //insert values
                ...
                $sValuesAlter = " '" . implode("', '",
                        $aValuesAlter) . "' "; // implode values to string like 'a','b','c\'d'
    ...
    function createNewField()
    {
        ...
        if (db_res("INSERT INTO `sys_profile_fields` (`Name`, `Type` ) VALUES ('NEW_ITEM', 'text')",
                0) and $iNewID = db_last_id()
        ) {
    ...
    function __construct()
    {
        ...
        $this->aAreasProps = array(
            1 => array('Title'         => 'Join',
                       'Order'         => 'JoinOrder',
                       'Block'         => 'JoinBlock',
                       'AddSelect'     => 'Control,Extra,Min,Max,Values,Check,Unique,Mandatory,UseLKey,Default',
                       'AddBlockProps' => $aAddBlockProps['Join'],
                       'AddProps'      => $aAddProps['Join']
            ),

            2 => array('Title'         => 'Edit (Owner)',
                       'Order'         => 'EditOwnOrder',
                       'Block'         => 'EditOwnBlock',
                       'AddSelect'     => 'Control,Extra,Min,Max,Values,Check,Unique,Mandatory,UseLKey',
                       'AddBlockProps' => $aAddBlockProps['Edit'],
                       'AddProps'      => $aAddProps['Edit']
            ),
            3 => array('Title'         => 'Edit (Admin)',
                       'Order'         => 'EditAdmOrder',
                       'Block'         => 'EditAdmBlock',
                       'AddSelect'     => 'Control,Extra,Min,Max,Values,Check,Unique,Mandatory,UseLKey',
                       'AddBlockProps' => $aAddBlockProps['Edit'],
            ...
            5 => array('Title'         => 'View (Admin)',
                       'Order'         => 'ViewAdmOrder',
                       'Block'         => 'ViewAdmBlock',
                       'AddSelect'     => 'Values,UseLKey',
                       'AddBlockProps' => $aAddBlockProps['View'],
                       'AddProps'      => $aAddProps['View']
            ),
            6 => array('Title'         => 'View (Member)',
                       'Order'         => 'ViewMembOrder',
                       'Block'         => 'ViewMembBlock',
                       'AddSelect'     => 'Values,UseLKey',
                       'AddBlockProps' => $aAddBlockProps['View'],
            ...
            9   => array(
                ...
                'Order'         => 'SearchSimpleOrder',
                'Block'         => 'SearchSimpleBlock',
                'AddSelect'     => 'Control,Extra,Min,Max,Values,UseLKey',
                'AddBlockProps' => $aAddBlockProps['Search'],
                'AddProps'      => $aAddProps['Search']
            ),
            10  => array(
                'Title'         => 'Search (Quick)',
                'Order'         => 'SearchQuickOrder',
        ...
        foreach ($this->aBlocks as $iBlockID => $sBlockName) {
            ...
            foreach ($aItems as $iBlockID => $aItem) {
                ...
                foreach ($aItem as $sProp => $sValue) {
                    if ($sProp == 'ID') {
                        continue;
                    } //do not process ID it is already in key

                    $sCacheString .= "$sPrefix          '$sProp' => " . $this->processValue4CacheString($sProp, $sValue,
                            "$sPrefix          ") . ",\n";
                }
        ...
        switch ($sProp) {
            ...
            case 'Values':
                if ($sValue == '') {
                    ...
                    return '"' . $this->addSlashesDblQuot($sValue) . '"';
