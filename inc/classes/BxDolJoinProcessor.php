...
class BxDolJoinProcessor
{
    ...
    function registerMember()
    {
        ...
        if (empty($aProfile1['NickName'])) {
            $aProfile1['NickName'] = uriGenerate((empty($aProfile1['FirstName']) ? genRndPwd(10,
                false) : $aProfile1['FirstName']), 'Profiles', 'NickName');
        }
        ...
        if (isset($aProfile1['ProfilePhoto']) && !empty($aProfile1['ProfilePhoto'])) {
            ...
            if ('sys_avatar' == getParam('sys_member_info_thumb') && BxDolRequest::serviceExists('avatar',
                    'set_image_for_cropping')
            ) {
                BxDolService::call('avatar', 'set_image_for_cropping',
                    array($iId1, $GLOBALS['dir']['tmp'] . $aProfile1['ProfilePhoto']));
                ...
                $aFileInfo = array(
                    ...
                    'album'        => str_replace('{nickname}', getUsername($iId1),
                        getParam('bx_photos_profile_album_name')),
                    'albumPrivacy' => $oPrivacy->getDefaultValueModule('photos', 'album_view'),
                );
                BxDolService::call('photos', 'perform_photo_upload',
                    array($GLOBALS['dir']['tmp'] . $aProfile1['ProfilePhoto'], $aFileInfo, false), 'Uploader');
            }
        }
    ...
    function showFinishPage($iMemberId, $sStatus)
    {
        switch ($sStatus) {
            case 'Active':
                ...
                break; //activated automatically
            case 'Approval':
                $sStatusText = ('_USER_CONF_SUCCEEDED');
                break; //automatically confirmed
            case 'Unconfirmed':
                $sStatusText = ('_EMAIL_CONF_SENT');
                break; //conf mail succesfully sent
            case 'NotSent':
                $sStatusText = ('_EMAIL_CONF_NOT_SENT');
                break; //failed to send conf mail
        }

        if ('sys_avatar' == getParam('sys_member_info_thumb') && 'EXIT' == BxDolService::call('avatar', 'join',
                array($iMemberId, $sStatusText))
        ) {
            exit;
        }
