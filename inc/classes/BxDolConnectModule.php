...
class BxDolConnectModule extends BxDolModule
{
    ...
    function _actionAdministration($sOptionApiKey, $sLangSettingsTitle, $sLangInfoTitle = '', $sLangInfoText = '')
    {
        ...
        if ($sLangInfoText) {
            echo DesignBoxAdmin(_t($sLangInfoTitle),
                $GLOBALS['oSysTemplate']->parseHtmlByName('default_padding.html', array(
    ...
    function processJoinAfterPayment(&$oAlert)
    {
        ...
        header('location: ' . $this->_getRedirectUrl((int)$aResult['member_id'],
                isset($aResult['existing_profile']) && $aResult['existing_profile']));
        exit;
    }
    ...
    function _createProfileRaw($aProfileInfo, $sAlternativeName = '', $isAutoFriends = true, $isSetLoggedIn = true)
    {
        ...
        } elseif (('on' == getParam('sys_dnsbl_enable') && 'block' == getParam('sys_dnsbl_behaviour') && bx_is_ip_dns_blacklisted('',
                    'join oauth')) || $oBxDolStopForumSpam->isSpammer(array(
                'email' => $aProfileFields['Email'],
        ...
        if (!$iExistingProfileId) {
            ...
            if (!empty($aProfileInfo['email'])) {
                ...
                $aTemplate       = $oEmailTemplate->getTemplate($this->_oConfig->sEmailTemplatePasswordGenerated,
                    $iProfileId);
        ...
        return array(
            ...
            'existing_profile'    => $iExistingProfileId ? true : false
        );
    }
    ...
    function _getJoinPage($aProfileFields, $iRemoteProfileId)
    {
        ...
        $this->_oTemplate->getPage(_t('_JOIN_H'),
            $this->_oTemplate->parseHtmlByName('default_padding.html', array('content' => $oJoin->process())));
        exit;
    }
    ...
    function _getRedirectUrl($iProfileId, $isExistingProfile = false)
    {
        ...
        switch ($this->_oConfig->sRedirectPage) {
            ...
            case 'avatar':
                if (BxDolInstallerUtils::isModuleInstalled('avatar') && BxDolService::call('avatar', 'join',
                        array($iProfileId, '_Join complete'))
                ) {
                    exit;
                }
                break;
