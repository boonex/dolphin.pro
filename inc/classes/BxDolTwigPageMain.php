...
class BxDolTwigPageMain extends BxDolPageView
{
    ...
    function ajaxBrowse($sMode, $iPerPage, $aMenu = array(), $sValue = '', $isDisableRss = false, $isPublicOnly = true)
    {
        ...
        if (!$aMenu) {
            $aMenu = ($isDisableRss ? '' : array(
                _t('_RSS') => array(
                    'href' => $o->aCurrent['rss']['link'] . (false === strpos($o->aCurrent['rss']['link'],
                            '?') ? '?' : '&') . 'rss=1',
                    'icon' => 'rss'
                )
            ));
    ...
    function getBlockCode_Calendar($iBlockID, $sContent)
    {
        ...
        $oCalendar = bx_instance($this->oMain->_aModule['class_prefix'] . 'Calendar',
            array((int)$aDateParams[0], (int)$aDateParams[1], $this->oDb, $this->oConfig, $this->oTemplate));
    ...
    function getBlockCode_Categories($iBlockID, $sContent)
    {
        ...
        $oCateg = new BxTemplCategoriesModule($aParam, _t('_categ_users'),
            BX_DOL_URL_ROOT . $this->oConfig->getBaseUri() . 'categories');
