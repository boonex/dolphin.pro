...
class BxDolPrivacyQuery extends BxDolDb
{
    ...
    function getGroupsBy($aParams)
    {
        ...
        return !empty($sCacheFunction) && !empty($sCacheName) ? $this->$sCacheFunction($sCacheName, $sMethod,
            $sSql) : $this->$sMethod($sSql);
    }
    ...
    function deleteFromGroup($iGroupId, $aMemberIds)
    {
        ...
        $iCount = $this->query("DELETE FROM `sys_privacy_members` WHERE `group_id`='" . $iGroupId . "' AND `member_id` IN ('" . implode("','",
                $aMemberIds) . "')");
        $this->cleanCache($this->_sGroupMembersCache . $iGroupId);
    ...
    function getDefaultValue($iOwnerId, $sModuleUri, $sActionName)
    {
        ...
        return $this->fromMemory($this->_sActionDefaultCache . $sModuleUri . '_' . $sActionName . '_' . $iOwnerId,
            'getOne', $sSql);
    }
