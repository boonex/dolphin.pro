...
class BxDolAlertsResponceMatch extends BxDolAlertsResponse
{
    ...
    function _checkProfileMatch($iProfileId, $sAction)
    {
        ...
        if ($aProfile['Status'] == 'Active' && ($aProfile['UpdateMatch'] || $sAction == 'join')) {
            ...
            // get send mails
            $aSendMails = $oDb->getRow("SELECT `profiles_match` FROM `sys_profiles_match_mails` WHERE `profile_id` = ?",
                [$iProfileId]);
            ...
            foreach ($aProfiles as $iProfId) {
                ...
                if (!empty($aProfile) && $aProfile['Status'] == 'Active') {
                    $oDb->query("INSERT INTO `sys_sbs_queue`(`email`, `subject`, `body`) VALUES('" . $aProfile['Email'] . "', '" . process_db_input($aMessage['subject'],
                            BX_TAGS_NO_ACTION, BX_SLASHES_NO_ACTION) . "', '" . process_db_input($aMessage['body'],
