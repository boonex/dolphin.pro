...
class BxDolMenu
{
    ...
    function getMenuInfo()
    {
        ...
        foreach ($this->aTopMenu as $iItemID => $aItem) {
            ...
            foreach ($aItemUris as $sItemUri) {
                ...
                if (0 == strcasecmp($sItemUri, $this->sRequestUriFile) || 0 == strcasecmp($sItemUriPermalink,
                        $this->sRequestUriFile) || 0 == strncasecmp(rawurldecode($this->sRequestUriFile), $sItemUri,
                        strlen($sItemUri)) || 0 == strncasecmp($this->sRequestUriFile, $sItemUriPermalink,
                        strlen($sItemUriPermalink))
                ) {
    ...
    {
        $sLink = str_replace('{memberPass}',
            empty($this->aMenuInfo['memberID']) ? '' : getPassword($this->aMenuInfo['memberID']), $sLink);
