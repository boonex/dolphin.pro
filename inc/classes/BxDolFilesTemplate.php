...
class BxDolFilesTemplate extends BxDolModuleTemplate
{
    ...
    function getFileAuthor($aInfo)
    {
        ...
        return $this->parseHtmlByName('media_info.html',
            array('memberPic' => get_member_thumbnail($aInfo['medProfId'], 'none', true)));
    }
    ...
    function getBasicFileInfoForm(&$aInfo, $sUrlPref = '')
    {
        $aForm = array(
            'date' => array(
                'type'    => 'value',
                'value'   => getLocaleDate($aInfo['medDate'],
                        BX_DOL_LOCALE_DATE_SHORT) . ' (' . defineTimeInterval($aInfo['medDate'], false) . ')',
                'caption' => _t('_Date'),
            ),
    ...
    function getAlbumInfo(&$aInfo)
    {
        $aForm           = array(
            ...
            'inputs'     => array(
                ...
                'date'        => array(
                    'type'    => 'value',
                    'value'   => getLocaleDate($aInfo['Date'],
                            BX_DOL_LOCALE_DATE_SHORT) . ' (' . defineTimeInterval($aInfo['Date'], false) . ')',
                    'caption' => _t('_Date'),
                ),
            )
        );
