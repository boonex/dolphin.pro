...
class BxDolTextData
{
    ...
    function __construct(&$oModule)
    {
        ...
        $this->_aForm = array(
            ...
            'inputs'     => array(
                ...
                'categories'       => $oCategories->getGroupChooser($this->_oModule->_oConfig->getCategoriesSystemName(),
                    $this->_iOwnerId, true),
        ...
        if (!empty($this->_iOwnerId)) {
            ...
            $this->_aForm['inputs']['allow_comment_to'] = $oPrivacy->getGroupChooser($this->_iOwnerId, $sModuleUri,
                'comment');
            $this->_aForm['inputs']['allow_vote_to']    = $oPrivacy->getGroupChooser($this->_iOwnerId, $sModuleUri,
                'vote');
        }
    }
    ...
    function getEditForm($aValues, $aAddFields = array())
    {
        ...
        if (isset($this->_aForm['inputs']['categories'])) {
            ...
            $this->_aForm['inputs']['categories'] = $oCategories->getGroupChooser($this->_oModule->_oConfig->getCategoriesSystemName(),
                $this->_iOwnerId, true, $aValues['categories']);
        }
                ...
                if (array_key_exists($sKey, $this->_aForm['inputs'])) {
                    if ($this->_aForm['inputs'][$sKey]['type'] == 'checkbox') {
                        ...
                        if ($this->_aForm['inputs'][$sKey]['type'] == 'select_box' && $this->_aForm['inputs'][$sKey]['name'] == 'Categories') {
                            $aCategories                            = preg_split('/[' . $oCategories->sTagsDivider . ']/',
        ...
        if ($oForm->isSubmittedAndValid()) {
            ...
            bx_import('BxDolAlerts');
            $oAlert = new BxDolAlerts($this->_oModule->_oConfig->getAlertsSystemName(), 'edit', $aValues['id'],
                $this->_iOwnerId);
            $oAlert->alert();
            //--- 'System' -> Edit for Alerts Engine ---//
