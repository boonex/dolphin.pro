...
class BxDolPaginate extends BxDolTemplate
{
    ...
    function getSorting($aValues, $sSorting = '')
    {
        ...
        if (!empty($this->_sOnChangeSorting)) {
            ...
            $sOnChangeSorting        = "window.location='" . $this->parseHtmlByContent($this->_sPageUrl, $aReplacement,
                    array('{', '}')) . "'";
        }
        $aReplacement = array(
            'title'    => $sSortingTitle,
    ...
    function getPaginate($iStart = -1, $iPerPage = -1, $sTemplate = false)
    {
        ...
        if ($this->_iPage > 1) {
            ...
            $aReplacementLink = array_merge($aReplacement,
                array('start' => $this->_iStart - $this->_iPerPage, 'page' => $this->_iPage - 1));
            $sContent .= $this->parseHtmlByContent($this->_sButtonActiveTmpl, array(
                'icon'         => 'backward',
        ...
        if ($this->_bPageLinks) {
            for ($i = $iRangeBeg; $i <= $iRangeEnd; $i++) {
                if ($i == $this->_iPage) {
                    ...
                    $aReplacementLink = array_merge($aReplacement,
                        array('start' => ($i - 1) * $this->_iPerPage, 'page' => $i));
        ...
        if ($this->_iPage < $this->_iPages) {
            ...
            $aReplacementLink = array_merge($aReplacement,
                array('start' => $this->_iPage * $this->_iPerPage, 'page' => $this->_iPage + 1));
            $sContent .= $this->parseHtmlByContent($this->_sButtonActiveTmpl, array(
                'icon'         => 'forward',
            ...
            if ($this->_bPageLinksFirstLast) {
                $aReplacementLink = array_merge($aReplacement,
                    array('start' => ($this->_iPages - 1) * $this->_iPerPage, 'page' => $this->_iPages));
                $sContent .= $this->parseHtmlByContent($this->_sButtonActiveTmpl, array(
                    'icon'         => 'step-forward',
    ...
    {
        return !empty($this->_sOnChangePage) ? 'onclick="javascript:' . $this->parseHtmlByContent($this->_sOnChangePage,
                $aReplacement, array('{', '}')) . '; return false;"' : '';
    }
    ...
    function _getPerPageChangerFloat()
    {
        ...
        if (!empty($this->_sOnChangePerPage)) {
            ...
            $sPerPageOnChange = "window.location='" . $this->_getPageChangeUrl(array_merge($aReplacement,
                    array('page' => 1, 'per_page' => "' + this.value + '"))) . "'";
        }
