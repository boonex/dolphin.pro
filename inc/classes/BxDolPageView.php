...
class BxDolPageView
{
    ...
    function genColumnHeader($iColumn, $fColumnWidth)
    {
        ...
        if ($bColumnFull) {
            ...
            if (($iColumn == 1) || $bLastColumnWasFull) {
                $sAddClass .= 'first';
            } else {
                if ($iColumn == $iColumnsCount || ($bNextColumnIsFull)) {
                    $sAddClass .= 'last';
                } else {
                    $sAddClass .= 'middle';
                }
        ...
        if (preg_replace('/\d+/', '',
                $this->aPage['Width']) == 'px' && $GLOBALS['oTemplConfig']->PageComposeColumnCalculation == 'px'
        ) {
    ...
    */
    function genBlock($iBlockID, $aBlock, $bStatic = true, $sDynamicType = 'tab')
    {
        ...
        if (isset($GLOBALS['bx_profiler'])) {
            $GLOBALS['bx_profiler']->beginPageBlock(_t($aBlock['Caption']), $iBlockID);
        }

        if ($aBlock['Cache'] > 0 && getParam('sys_pb_cache_enable') == 'on') {
            $oCache = $this->getBlocksCacheObject();
            $sBlock = $oCache->getData($this->genBlocksCacheKey($iBlockID . $bStatic . $sDynamicType . isMember()),
                $aBlock['Cache']);
            if ($sBlock !== null) {
                if (isset($GLOBALS['bx_profiler'])) {
                    $GLOBALS['bx_profiler']->endPageBlock($iBlockID, $sBlock ? false : true, true);
                }
                $this->sCode .= $sBlock;
        ...
        if ($aBlock['Cache'] > 0 && getParam('sys_pb_cache_enable') == 'on') {
            $oCache = $this->getBlocksCacheObject();
            $oCache->setData($this->genBlocksCacheKey($iBlockID . $bStatic . $sDynamicType . isMember()), $sBlock,
                $aBlock['Cache']);
        }

        if (isset($GLOBALS['bx_profiler'])) {
            $GLOBALS['bx_profiler']->endPageBlock($iBlockID, $sBlockCode ? false : true, false);
        }
    ...
    function _getBlockCaptionCode($iBlockID, $aBlock, $aBlockCode, $bStatic = true, $sDynamicType = 'tab')
    {
        ...
        if (is_array($aBlockCode) && isset($aBlockCode[3]) && (is_array($aBlockCode[3]) || (is_string($aBlockCode[3]) && $aBlockCode[3]))) {
            ...
            if (is_array($aBlockCode[3])) {
                ...
                if (is_string($aBlockCode[3]) && $aBlockCode[3]) {
    ...
    function _getBlockTopCode($iBlockID, $aBlock, $aBlockCode, $bStatic = true, $sDynamicType = 'tab')
    {
        ...
        if (!$bStatic && $sDynamicType == 'popup') {
            ...
            if (isset($aBlockCode[1]) && is_array($aBlockCode[1])) {
    ...
    public static function getBlockCaptionItemCode($iBlockID, $aLinks)
    {
        ...
        return $GLOBALS['oSysTemplate']->parseHtmlByName('designbox_menu.html', array(
            'id'    => $iBlockID,
            'title' => $sActive,
            'popup' => $GLOBALS['oFunctions']->transBox($GLOBALS['oSysTemplate']->parseHtmlByName('designbox_menu_items_1.html',
                array(
    ...
    public static function getBlockCaptionMenu($iBlockID, $aLinks)
    {
        ...
        return $GLOBALS['oSysTemplate']->parseHtmlByName('designbox_menu.html', array(
            'id'    => '' . $iBlockID,
            'title' => $sActive,
            'popup' => $GLOBALS['oFunctions']->transBox($GLOBALS['oSysTemplate']->parseHtmlByName('designbox_menu_items_2.html',
                array(
                    'bx_repeat:items' => $aItems
                )))
        ));
