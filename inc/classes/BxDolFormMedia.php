...
class BxDolFormMedia extends BxTemplFormView
{
    ...
    function uploadVideos($sTag, $sCat, $sName = 'videos', $sTitle = 'videos_titles', $sTitleAlt = 'title')
    {
        ...
        foreach ($_FILES[$sName]['tmp_name'] as $i => $sUploadedFile) {
            ...
            if (move_uploaded_file($sUploadedFile, $sTmpFile)) {
                $iRet = BxDolService::call('videos', 'perform_video_upload', array($sTmpFile, $aFileInfo, false),
                    'Uploader');
                @unlink($sTmpFile);
                if ($iRet) {
                    $aRet[] = $iRet;
    ...
    function uploadSounds($sTag, $sCat, $sName = 'sounds', $sTitle = 'sounds_titles', $sTitleAlt = 'title')
    {
        ...
        foreach ($_FILES[$sName]['tmp_name'] as $i => $sUploadedFile) {
            ...
            if (move_uploaded_file($sUploadedFile, $sTmpFile)) {
                $iRet = BxDolService::call('sounds', 'perform_music_upload', array($sTmpFile, $aFileInfo, false),
                    'Uploader');
                @unlink($sTmpFile);
                if ($iRet) {
                    $aRet[] = $iRet;
    ...
    function processMedia($iEntryId, $iProfileId)
    {
        ...
        foreach ($this->_aMedia as $sName => $a) {
            ...
            $aFiles2Delete = array();
            $aFiles        = $this->_getFilesInEntry($a['module'], $a['service_method'], $a['post'], $sName,
                (int)$iProfileId, $iEntryId);
    ...
    function generateCustomMediaTemplates($iProfileId, $iEntryId, $iThumb = 0)
    {
        ...
        foreach ($this->_aMedia as $sName => $a) {
            ...
            $aFiles = $this->_getFilesInEntry($a['module'], $a['service_method'], $a['post'], $sName, (int)$iProfileId,
                $iEntryId);
            ...
            $aTemplates[$sName]['choice'] = $aFiles ? $this->_oMain->_oTemplate->parseHtmlByName('form_field_files_choice',
                $aVarsChoice) : '';
            ...
            if ($a['thumb']) {
                ...
                $aTemplates[$sName]['thumb_choice'] = $aFiles ? $this->_oMain->_oTemplate->parseHtmlByName('form_field_thumb_choice',
                    $aVarsThumbsChoice) : '';
            }
            ...
            $aVarsUpload                  = array(
                ...
                'bx_if:price'       => array(
                    'condition' => false,
                    'content'   => array('price' => '', 'file_price_title' => '')
                ),
                'bx_if:privacy'     => array(
                    'condition' => false,
                    'content'   => array('select' => '', 'file_permission_title' => '')
                ),
            );
            $aTemplates[$sName]['upload'] = $this->_oMain->_oTemplate->parseHtmlByName('form_field_files_upload',
                $aVarsUpload);
        }
