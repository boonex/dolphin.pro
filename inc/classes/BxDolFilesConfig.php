...
class BxDolFilesConfig extends BxDolConfig
{
    ...
    function getUploaders()
    {
        ...
        $oAlert = new BxDolAlerts($this->getMainPrefix(), 'uploaders_init', 0, getLoggedId(),
            array('uploaders' => &$this->aUploaders, 'config' => $this));
        $oAlert->alert();
    ...
    {
        $sName = array_key_exists($sPseud,
            $this->aGlParams) ? $this->aGlParams[$sPseud] : $this->getMainPrefix() . '_' . $sPseud;
    ...
    function getAlbumMainActionsArray()
    {
        ...
        return array(
            ...
            $sPref . 'organize'    => array('type'  => 'submit',
                                            'value' => _t('_' . $this->sPrefix . '_organize_objects')
            ),
    ...
    function getDefaultAlbums($bProcessed = false, $aReplacement = array())
    {
        ...
        foreach ($this->aDefaultAlbums as $sAlbum) {
            $aResult[] = str_replace(array_keys($aReplacement), array_values($aReplacement),
                $this->getGlParam($sAlbum));
        }
