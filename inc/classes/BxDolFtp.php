...
class BxDolFtp
{
        ...
        if (is_file($sFilePathFrom)) {
            if ($this->_isFile($sFilePathTo)) {
                ...
                if ($this->_isDirectory($sFilePathTo)) {
                    ...
                    if (isset($aFileParts[1])) {
                        $bResult = @ftp_put($this->_rStream, $this->_validatePath($sFilePathTo) . $aFileParts[1],
                            $sFilePathFrom, FTP_BINARY);
                    }
                }
            }
        } else {
            if (is_dir($sFilePathFrom) && $this->_isDirectory($sFilePathTo)) {
                ...
                foreach ($aInnerFiles as $sFile) {
                    $bResult = $this->_copyFile($this->_validatePath($sFilePathFrom) . $sFile,
                        $this->_validatePath($sFilePathTo) . $sFile);
                }
            } else {
                $bResult = false;
            }
    ...
    {
        $aParts = array();
        preg_match("/^([a-zA-Z0-9@~_\.\\\\\/:-]+[\\\\\/])([a-zA-Z0-9~_-]+\.[a-zA-Z]{2,8})$/", $sFilePath,
            $aParts) ? true : false;
    ...
    {
        return preg_match("/^([a-zA-Z0-9@~_\.\\\\\/:-]+)[\\\\\/]([a-zA-Z0-9~_-]+)[\\\\\/]?$/",
            $sFilePath) ? true : false;
    }
