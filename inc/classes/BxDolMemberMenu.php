...
class BxDolMemberMenu
{
    ...
    function getGetExtraMenuLink(&$aLinkInfo)
    {
        ...
        $aTemplateKeys = array(
            ...
            'item_img'     => $GLOBALS['oFunctions']->sysImage($aLinkInfo['item_img_src'], '', $aLinkInfo['item_title'],
                '', 'icon'),
    ...
    function _parseStructure($aMemberInfo, $aMenuStructure)
    {
                ...
                foreach ($aMenuItems as $iKey => $aItems) {
                    ...
                    foreach ($aItems AS $sMenuKey => $sValue) {
                        if ($sMenuKey != 'PopupMenu' && $sMenuKey != 'linked_items') {
                            if ($sMenuKey == 'Caption') {
                                $aReplaced[$sPosition][$iKey][$sMenuKey] = $oFunctions->markerReplace($aMemberInfo,
                                    $sValue, $aItems['Eval'], true);
                            } else {
                                $aReplaced[$sPosition][$iKey][$sMenuKey] = $oFunctions->markerReplace($aMemberInfo,
                                    $sValue, $aItems['Eval']);
                            }
                    ...
                    if ($aReplaced[$sPosition][$iKey]['Type'] == 'link'
                        ...
                        if ($aReplaced[$sPosition][$iKey]['Name'] == 'MemberBlock') {
                            ...
                            $sUserThumbnail       = $GLOBALS['oFunctions']->getMemberAvatar($aMemberInfo['ID'],
                                'small');
                            $sUserThumbnailDouble = $GLOBALS['oFunctions']->getMemberAvatar($aMemberInfo['ID'], 'small',
                                true);
                            ...
                            $sMenuImage = $GLOBALS['oSysTemplate']->parseHtmlByName('member_menu_thumbnail.html', array(
                                ...
                                'status_icon'                => $oUserStatus->getStatusIcon($aMemberInfo['ID'],
                                    'icon8'),
                            ...
                            'bx_if:reduce_element_top'    => array(
                                ...
                                'content'   => array(
                                    ...
                                    'bx_if:part_image' => array(
                                        ...
                                        'content'   => array(
                                            'item_img' => $this->getImage($sReduceImage,
                                                strip_tags($aReplaced[$sPosition][$iKey]['Caption'])),
                                        ),
                                    ),
                                ),
                            ),
        ...
        //generate data
        $aTemplateKeys = array(
            'items'               => $oSysTemplate->parseHtmlByName('extra_top_menu_items.html',
                array('bx_repeat:items' => $aDefinedMenuItems['top'])),
            'items_extra'         => $oSysTemplate->parseHtmlByName('extra_top_menu_items.html',
                array('bx_repeat:items' => $aDefinedMenuItems['top_extra'])),
    ...
    function genMemberMenu($memberID = 0)
    {
        if (isset($GLOBALS['bx_profiler'])) {
            $GLOBALS['bx_profiler']->beginMenu('Member Menu');
        }
        ...
        if (!$aMemberInfo) {
            if (isset($GLOBALS['bx_profiler'])) {
                $GLOBALS['bx_profiler']->endMenu('Member Menu');
            }
