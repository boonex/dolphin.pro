    ...
    function processAdminStats()
    {
        ...
        if (!empty($sMessage) && !empty($aAdmins)) {
            ...
            foreach ($aAdmins as $aAdmin) {
                ...
                if (sendMail($aAdmin['Email'], $aMail['subject'], $aMail['body'], $aAdmin['ID'], array(), 'html', false,
                    true)) {
                    $iCounter++;
                }
            }
                ...
                if ($aCurrentMem['ID'] != MEMBERSHIP_ID_STANDARD) {
                    ...
                    if ($aCurrentMem['ID'] != $aFurtherMem['ID'] && $aFurtherMem['ID'] == MEMBERSHIP_ID_STANDARD) {
                        if (!$bExpireNotifyOnce || abs($iFurtherTimestamp - $aCurrentMem['DateExpires']) < 24 * 3600) {
                            $bMailResult = mem_expiration_letter($aRow['ID'], $aCurrentMem['Name'],
                                (int)(($aCurrentMem['DateExpires'] - time()) / (24 * 3600)));
                            if ($bMailResult) {
                                $iExpireLetters++;
                            }
                        }
