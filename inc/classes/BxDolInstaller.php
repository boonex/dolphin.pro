...
class BxDolInstaller extends BxDolInstallerUtils
{
    ...
    function install($aParams)
    {
        ...
        if ($oModuleDb->isModuleParamsUsed($this->_aConfig['home_uri'], $this->_aConfig['home_dir'],
            $this->_aConfig['db_prefix'], $this->_aConfig['class_prefix'])
        ) {
            return array(
                'operation_title' => $sTitle,
        ...
        if (isset($this->_aConfig['compatible_with']) && is_array($this->_aConfig['compatible_with'])) {
            foreach ($this->_aConfig['compatible_with'] as $iKey => $sVersion) {
                ...
                $bCompatible = $bCompatible || (preg_match($sVersion,
                            $GLOBALS['site']['ver'] . '.' . $GLOBALS['site']['build']) > 0);
            }
        }
        if (!$bCompatible) {
            return array(
                'operation_title' => $sTitle,
                'message'         => $this->_displayResult('check_script_version', false,
    ...
    function _perform($sOperationName, $sOperationTitle)
    {
        ...
        foreach ($this->_aConfig[$sOperationName] as $sAction => $iEnabled) {
            ...
            if ($iEnabled == 0 || (isset($_COOKIE[$sCookie]) && (int)$_COOKIE[$sCookie] == 1) || !method_exists($this,
                    $sMethod)
            ) {
                continue;
            }
            ...
            if ((is_int($mixedResult) && (int)$mixedResult == BX_DOL_INSTALLER_SUCCESS) || (isset($mixedResult['code']) && (int)$mixedResult['code'] == BX_DOL_INSTALLER_SUCCESS)) {
                ...
                $sMessage .= $this->_displayResult($sAction, true,
                    isset($mixedResult['content']) ? $mixedResult['content'] : '');
                continue;
            }
            ...
            return array('message' => $this->_displayResult($sAction, false, method_exists($this,
                $sMethodFailed) ? $this->$sMethodFailed($mixedResult) : $this->actionOperationFailed($mixedResult)),
                         'result'  => false
            );
        }
        ...
        //--- Remove all cookies ---//
        foreach ($this->_aConfig[$sOperationName] as $sAction => $iEnabled) {
            setcookie((empty($this->_aConfig['name']) ? '' : $this->_aConfig['name']) . '_' . $sAction, 1,
                time() - 86400, '/');
        }
    ...
    function actionCheckDependencies($bInstall = true)
    {
        ...
        return empty($sContent) ? BX_DOL_INSTALLER_SUCCESS : array('code'    => BX_DOL_INSTALLER_FAILED,
                                                                   'content' => $sContent
        );
    }
    ...
    function actionShowIntroduction($bInstall = true)
    {
        ...
        return file_exists($sPath) ? array("code"    => BX_DOL_INSTALLER_SUCCESS,
                                           "content" => "<pre>" . file_get_contents($sPath) . "</pre>"
    ...
    function actionCheckPermissions($bInstall = true)
    {
        ...
        foreach ($aPermissions as $sPermissions => $aFiles) {
            ...
            foreach ($aFiles as $sFile) {
                if (!BxDolInstallerUtils::$sCheckFunction(bx_ltrim_str($this->_sModulePath . $sFile,
        ...
        return empty($aResult) ? BX_DOL_INSTALLER_SUCCESS : array('code'    => BX_DOL_INSTALLER_FAILED,
                                                                  'content' => $aResult
        );
    }
    ...
    function actionCheckPermissionsFailed($mixedResult)
    {
        ...
        foreach ($mixedResult['content'] as $aFile) {
            $sResult .= '-- -- ' . _t('_adm_txt_modules_wrong_permissions_msg', $aFile['path'],
                    $aFile['permissions']) . '<br />';
        }
    ...
    function actionChangePermissions($bInstall = true)
    {
        ...
        bx_import('BxDolFtp');
        $oFile = new BxDolFtp($sFtpHost, getParam('sys_ftp_login'), getParam('sys_ftp_password'),
            getParam('sys_ftp_dir'));

        if (!$oFile->connect()) {
            return array('code'         => BX_DOL_INSTALLER_FAILED,
                         'content_msg'  => '_adm_txt_modules_wrong_permissions_change_cannot_connect_to_ftp',
                         'content_data' => $aResult
            );
        }

        if (!$oFile->isDolphin()) {
            return array('code'         => BX_DOL_INSTALLER_FAILED,
                         'content_msg'  => '_adm_txt_modules_wrong_permissions_change_destination_not_valid',
                         'content_data' => $aResult
            );
        }
        ...
        foreach ($aChangeItems as $aChangeItem) {
            if (!$oFile->setPermissions($aChangeItem['path'], $aChangeItem['permissions'])) {
                $aResult[] = array('path'        => $this->_sModulePath . $aChangeItem['file'],
                                   'permissions' => $aChangeItem['permissions']
                );
            }
        }

        return empty($aResult) ? BX_DOL_INSTALLER_SUCCESS : array('code'         => BX_DOL_INSTALLER_FAILED,
                                                                  'content_msg'  => '_adm_txt_modules_wrong_permissions_change',
                                                                  'content_data' => $aResult
        );
    }
    ...
    function _addLanguage($aLanguage, $aLangInfo)
    {
        ...
        foreach ($aLanguage as $sKey => $sValue) {
            ...
            if (!$iExistedKey) { // key is missing, insert new key
                if (!$oDb->res("INSERT INTO `sys_localization_keys` VALUES (NULL, ?, ?)",
                    [BX_DOL_LANGUAGE_CATEGORY_SYSTEM, $sDbKey])
                ) {
                    continue;
                }
    ...
    function _recompileLanguageForAllModules($iLangId)
    {
        ...
        $aLanguage = $GLOBALS['MySQL']->getRow("SELECT `ID` AS `id`, `Name` AS `name`, `Title` AS `title` FROM `sys_localization_languages` WHERE `ID` = ?",
            [$iLangId]);
        if (!$aLanguage) {
            return false;
        }
