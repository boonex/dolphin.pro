...
class BxDolAdminMenu
{
    public static function getTopMenu()
    {
        ...
        $aItems = array_merge($aItems,
            $GLOBALS['MySQL']->getAll("SELECT `caption`, `url`, `target`, `icon` FROM `sys_menu_admin_top` ORDER BY `Order`"));
        foreach ($aItems as $aItem) {
            $aTmplVarsItems[] = array(
                ...
                'icon'               => false === strpos($aItem['icon'],
                    '.') ? '<i class="sys-icon ' . $aItem['icon'] . '"></i>' : '<img src="' . $GLOBALS['oAdmTemplate']->getIconUrl($aItem['icon']) . '" alt="' . _t($aItem['caption']) . '" />',
            );
        }
    ...
    public static function getMainMenu()
    {
        ...
        foreach ($aMenu as $aMenuItem) {
            $aMenuItem['url'] = str_replace(array('{siteUrl}', '{siteAdminUrl}'),
                array(BX_DOL_URL_ROOT, BX_DOL_URL_ADMIN), $aMenuItem['url']);

            $bActiveCateg = $sFile == 'index.php' && (!empty($_GET['cat'])) && $_GET['cat'] == $aMenuItem['name'];
            $aSubmenu     = $GLOBALS['MySQL']->getAll("SELECT * FROM `sys_menu_admin` WHERE `parent_id`= ? ORDER BY `order`",
                [$aMenuItem['id']]);
            ...
            foreach ($aSubmenu as $aSubmenuItem) {
                ...
                $aSubmenuItem['url'] = str_replace(array('{siteUrl}', '{siteAdminUrl}'),
                    array(BX_DOL_URL_ROOT, BX_DOL_URL_ADMIN), $aSubmenuItem['url']);

                if (!defined('BX_DOL_ADMIN_INDEX') && $aSubmenuItem['url'] != '' && (strpos($sUri,
                            $aSubmenuItem['url']) !== false || strpos($aSubmenuItem['url'], $sUri) !== false)
                ) {
