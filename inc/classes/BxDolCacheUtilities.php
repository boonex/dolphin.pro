...
class BxDolCacheUtilities
{
    ...
    function _action($sCache, $sMode = 'clear')
    {
        ...
        switch ($sCache) {
            case 'member_menu':
                $mixedResult = $this->$sFuncCacheObject ($this->oCacheMemberMenu,
                    $this->oMemberMenu->sMenuMemberKeysCache);
                break;
            ...
            case 'template':
                $mixedResult = $this->$sFuncCacheObject ($this->oCacheTemplates,
                    $GLOBALS['oSysTemplate']->_sCacheFilePrefix);
                break;

            case 'css':
                $mixedResult = $this->$sFuncCacheFile($GLOBALS['oSysTemplate']->_sCssCachePrefix,
                    BX_DIRECTORY_PATH_CACHE_PUBLIC);
                break;

            case 'js':
                $mixedResult = $this->$sFuncCacheFile($GLOBALS['oSysTemplate']->_sJsCachePrefix,
                    BX_DIRECTORY_PATH_CACHE_PUBLIC);
                break;
        }
