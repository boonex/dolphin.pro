...
class BxDolTwigModuleDb extends BxDolModuleDb
{
    ...
    function getEntryByIdAndOwner($iId, $iOwner, $isAdmin)
    {
        ...
        return $this->getRow("SELECT * FROM `" . $this->_sPrefix . $this->_sTableMain . "` WHERE `{$this->_sFieldId}` = ? $sWhere LIMIT 1",
            $aBindings);
    }
    ...
    {
        return $this->getPairs("SELECT `{$this->_sFieldId}` FROM `" . $this->_sPrefix . $this->_sTableMain . "` WHERE `{$this->_sFieldAuthorId}` = '$iProfileId'",
            $this->_sFieldId, $this->_sFieldId);
    }
    ...
    {
        return $this->getRow("SELECT * FROM `" . $this->_sPrefix . $this->_sTableMain . "` WHERE `{$this->_sFieldUri}` = ? LIMIT 1",
            [$sUri]);
    }
    ...
    function getLatestFeaturedItem()
    {
        ...
        WHERE `{$this->_sFieldStatus}` = ? AND `{$this->_sFieldAllowViewTo}` = ? $sWhere ORDER BY `{$this->_sFieldCreated}` DESC LIMIT 1",
            ['approved', BX_DOL_PG_ALL]);
    }
    ...
    {
        return $this->getPairs("SELECT `media_id` FROM `" . $this->_sPrefix . $this->_sTableMediaPrefix . "{$sMediaType}` WHERE `entry_id` = '$iEntryId'",
            'media_id', 'media_id');
    }
    ...
    {
        return $this->getRow("SELECT `entry_id`, `media_id` FROM `" . $this->_sPrefix . $this->_sTableMediaPrefix . "{$sMediaType}` WHERE `entry_id` = ? AND `media_id` = ?",
            [$iEntryId, $iMediaId]);
    }
    ...
    {
        $a = $this->getAllWithKey("SELECT `p`.`NickName`, `p`.`ID` FROM `sys_friend_list` AS `o` INNER JOIN `Profiles` AS `p` ON (`p`.`ID` = `o`.`ID` AND `o`.`Profile` = ?)",
            'NickName', [$iProfileId]);
        $a = array_merge($a,
            $this->getAllWithKey("SELECT `p`.`NickName`, `p`.`ID` FROM `sys_friend_list` AS `o` INNER JOIN `Profiles` AS `p` ON (`p`.`ID` = `o`.`Profile` AND `o`.`ID` = ?)",
                'NickName', [$iProfileId]));
        $a = array_merge($a,
            $this->getAllWithKey("SELECT `p`.`NickName`, `p`.`ID` FROM `sys_fave_list` AS `o` INNER JOIN `Profiles` AS `p` ON (`p`.`ID` = `o`.`ID` AND `o`.`Profile` = ?)",
                'NickName', [$iProfileId]));
        $a = array_merge($a,
            $this->getAllWithKey("SELECT `p`.`NickName`, `p`.`ID` FROM `sys_fave_list` AS `o` INNER JOIN `Profiles` AS `p` ON (`p`.`ID` = `o`.`Profile` AND `o`.`ID` = ?)",
                'NickName', [$iProfileId]));
        $a = array_merge($a,
            $this->getAllWithKey("SELECT `p`.`NickName`, `p`.`ID` FROM `sys_messages` AS `o` INNER JOIN `Profiles` AS `p` ON (`p`.`ID` = `o`.`Recipient` AND `o`.`Sender` = ?)",
                'NickName', [$iProfileId]));
        foreach ($a as $k => $r) {
            if ($iProfileId == $r['ID']) {
                unset($a[$k]);
