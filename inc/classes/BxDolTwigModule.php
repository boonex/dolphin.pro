...
class BxDolTwigModule extends BxDolModule
{
    ...
    function _actionFiles($sUri, $sTitle)
    {
        ...
        echo BxDolService::call('files', 'get_browse_block',
            array($aRestriction, $this->_oConfig->getBaseUri() . 'files/' . $sUri), 'Search');
    ...
    function _actionVideos($sUri, $sTitle)
    {
        ...
        echo BxDolService::call('videos', 'get_browse_block',
            array($aRestriction, $this->_oConfig->getBaseUri() . 'videos/' . $sUri), 'Search');
    ...
    function _actionSounds($sUri, $sTitle)
    {
        ...
        echo BxDolService::call('sounds', 'get_browse_block',
            array($aRestriction, $this->_oConfig->getBaseUri() . 'sounds/' . $sUri), 'Search');
    ...
    function _actionPhotos($sUri, $sTitle)
    {
        ...
        echo BxDolService::call('photos', 'get_browse_block',
            array($aRestriction, $this->_oConfig->getBaseUri() . 'photos/' . $sUri), 'Search');
    ...
    function _actionUploadMedia($sUri, $sIsAllowedFuncName, $sMedia, $aMediaFields, $sTitle)
    {
        ...
        $oForm  = new $sClass ($this, $aDataEntry[$this->_oDb->_sFieldAuthorId], $iEntryId, $aDataEntry, $sMedia,
            $aMediaFields);
    ...
    function _actionBroadcast($iEntryId, $sTitle, $sMsgNoRecipients, $sMsgSent)
    {
        ...
        if ($oForm->isSubmittedAndValid()) {
            ...
            foreach ($aRecipients as $aProfile) {
                $iSentMailsCounter += sendMail($aProfile['Email'], $aTemplate['Subject'], $aTemplate['Body'],
                    $aProfile['ID'], $aTemplateVars);
            }
        ...
        if ($oForm->isSubmittedAndValid()) {
            ...
            if (false !== bx_get('inviter_users') && is_array(bx_get('inviter_users'))) {
                ...
                foreach ($aInviteUsers as $iRecipient) {
                    ...
                    $aPlus      = array_merge(array('NickName' => ' ' . getNickName($aRecipient['ID'])),
                        $aPlusOriginal);
                    $iSuccess += sendMail(trim($aRecipient['Email']), $aTemplate['Subject'], $aTemplate['Body'], '',
                        $aPlus) ? 1 : 0;
                }
            }
    ...
    function _actionCalendar($iYear, $iMonth, $sTitle)
    {
        ...
        $oCalendar = bx_instance($this->_aModule['class_prefix'] . 'Calendar',
            array($iYear, $iMonth, $this->_oDb, $this->_oConfig, $this->_oTemplate));
    ...
    function actionBrowse($sMode = '', $sValue = '', $sValue2 = '', $sValue3 = '')
    {
        ...
        $o      = new $sClass(process_db_input($sMode, BX_TAGS_STRIP), process_db_input($sValue, BX_TAGS_STRIP),
            process_db_input($sValue2, BX_TAGS_STRIP), process_db_input($sValue3, BX_TAGS_STRIP));
    ...
    function _actionJoin(
        ...
        $isFan = $this->_oDb->isFan($iEntryId, $this->_iProfileId, true) || $this->_oDb->isFan($iEntryId,
                $this->_iProfileId, false);
        ...
        if ($isFan) {
            ...
            if ($this->_oDb->joinEntry($iEntryId, $this->_iProfileId, $isConfirmed)) {
                ...
                echo MsgBox($isConfirmed ? $sMsgJoinSuccess : $sMsgJoinSuccessPending) . genAjaxyPopupJS($iEntryId,
    ...
    function _actionManageFansPopup(
        $iEntryId,
        $sTitle,
        ...
        $sActionsUrl = bx_append_url_params(BX_DOL_URL_ROOT . $this->_oConfig->getBaseUri() . "view/" . $aDataEntry[$this->_oDb->_sFieldUri],
            'ajax_action=');
        $aButtons    = array(
            array(
                'type'    => 'submit',
        ...
        $sControl = BxTemplSearchResult::showAdminActionsPanel('sys_manage_items_manage_fans', $aButtons,
            'sys_fan_unit');
    ...
    function _actionTags($sTitle, $sTitleAllTags = '')
    {
        ...
        $oTags  = new BxTemplTagsModule($aParam, $sTitleAllTags,
    ...
    function serviceGetForumPermission($iMemberId, $iForumId)
    {
        ...
        $aTrue = array(
            'admin' => $aDataEntry[$this->_oDb->_sFieldAuthorId] == $iMemberId || $this->isAdmin() ? 1 : 0,
            // author is admin
    ...
    function _serviceGetMemberMenuItem($sTitle, $sAlt, $sIcon, $isExtraCount = true, $sLinkAppend = '')
    {
        ...
        $aLinkInfo = array(
            ...
            'item_title'   => $sTitle,
            'extra_info'   => $isExtraCount ? $this->_oDb->getCountByAuthorAndStatus($this->_iProfileId,
                    'approved') + $this->_oDb->getCountByAuthorAndStatus($this->_iProfileId, 'pending') : '',
        );
    ...
    function _serviceGetWallPost($aEvent, &$aParams)
    {
        ...
        foreach ($aObjectIds as $iId) {
            ...
            if ($aItem[$this->_oDb->_sFieldStatus] == 'approved' && $aParams['obj_privacy']->check($aParams['txt_privacy_view_event'],
    ...
    function _serviceGetWallPostOutline($aEvent, $sIcon, $aParams = array())
    {
            ...
            if (empty($aItem)) {
                ...
                if ($aItem[$this->_oDb->_sFieldStatus] == 'approved' && $aParams['obj_privacy']->check($aParams['txt_privacy_view_event'],
                    ...
                    if ($aItem[$this->_oDb->_sFieldThumb]) {
                        ...
                        $aImage                 = BxDolService::call('photos', 'get_entry',
                            array($aItem[$this->_oDb->_sFieldThumb], 'browse2x'), 'Search');
        ...
        $aResult['content'] = $sCss . $this->_oTemplate->parseHtmlByName($sTmplName, array_merge($aTmplItem, array(
                ...
                'item_comments'      => (int)$aItem[$this->_oDb->_sFieldCommentCount] > 0 ? _t('_wall_n_comments',
                    $aItem[$this->_oDb->_sFieldCommentCount]) : _t('_wall_no_comments'),
    ...
    function serviceGetShoutboxData()
    {
        ...
        return array(
            array(
                ...
                'code_allow_use'    => method_exists($this,
                    'isFan') ? 'return BxDolService::call("' . $this->_aModule['uri'] . '", "is_fan", array($iHandler));' : '',
                'code_allow_delete' => method_exists($this,
                    'isEntryAdmin') ? 'return BxDolService::call("' . $this->_aModule['uri'] . '", "is_entry_admin", array($iHandler));' : '',
                'code_allow_block'  => '',
            ),
        );
    }
    ...
    {
        if (!($aDataEntry = $this->_oDb->getEntryByIdAndOwner($iEntryId, 0,
                true)) || $aDataEntry[$this->_oDb->_sFieldStatus] != 'approved'
        ) {
    ...
    function onEventChanged($iEntryId, $sStatus)
    {
        ...
        $oAlert = new BxDolAlerts($this->_sPrefix, 'change', $iEntryId, $this->_iProfileId,
    ...
    {
        // arise alert
        $oAlert = new BxDolAlerts($this->_sPrefix, 'mark_as_featured', $iEntryId, $this->_iProfileId,
            array('Featured' => $aDataEntry[$this->_oDb->_sFieldFeatured]));
        $oAlert->alert();
    }
    ...
    function _manageEntries(
        ...
        $aVars = array(
            ...
            'filter_panel'  => $isFilter ? $o->showAdminFilterPanel(false !== bx_get($this->_sFilterName) ? bx_get($this->_sFilterName) : '',
                'filter_input_id', 'filter_checkbox_id', $this->_sFilterName) : '',
    ...
    function _preProductTabs($sUri, $sSubTab = '')
    {
        ...
        if ((!$this->_iProfileId || $aDataEntry[$this->_oDb->_sFieldAuthorId] != $this->_iProfileId) && !$this->isAllowedView($aDataEntry,
                true)
        ) {
            $this->_oTemplate->displayAccessDenied();
    ...
    function _processFansActions($aDataEntry, $iMaxFans = 1000)
    {
        ...
        if (false !== bx_get('ajax_action') && $this->isAllowedManageFans($aDataEntry) && 0 == strcasecmp($_SERVER['REQUEST_METHOD'],
                'POST')
        ) {
