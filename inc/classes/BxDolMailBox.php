...
class BxDolMailBox extends BxDolPageView
{
        ...
        if ($this->aMailBoxSettings['messageID']) {
            ...
            if ($iSecondPersonID) {
                ...
                $sQuery =
                    "
                              SELECT
                                   DATE_FORMAT(`Date`, '" . getLocaleFormat(BX_DOL_LOCALE_DATE_SHORT,
                        BX_DOL_LOCALE_DB) . "') AS `When`,
    ...
    function getFavesList($sSqlLimit)
    {
        ...
        $sQuery =
            "
                    SELECT
                         `Profiles`.*,
                         DATE_FORMAT(`sys_fave_list`.`When`, '" . getLocaleFormat(BX_DOL_LOCALE_DATE_SHORT,
                BX_DOL_LOCALE_DB) . "') AS `When`
                    FROM
                         `sys_fave_list`
                    INNER JOIN
    ...
    function getContactedList($sSqlLimit)
    {
        ...
        $sQuery =
            "
                    SELECT
                         DISTINCT `Profiles`.*,
                         DATE_FORMAT(`sys_messages`.`Date`, '" . getLocaleFormat(BX_DOL_LOCALE_DATE_SHORT,
                BX_DOL_LOCALE_DB) . "') AS `When`
                    FROM
                         `sys_messages`
                    INNER JOIN
        ...
        if ($sWhereParameter) {
            ...
            if ($this->iTotalMessageCount) {
                ...
                $sQuery =
                    "
                              SELECT
                                   `sys_messages`.`ID`, DATE_FORMAT(`sys_messages`.`Date`, '" . getLocaleFormat(BX_DOL_LOCALE_DATE,
                        BX_DOL_LOCALE_DB) . "') AS `Date`,
        ...
        while (true == ($aRow = $rResult->fetch())) {
            if ($aRow['Recipient'] == $aRow['Sender']) {
                ...
                if ($aRow['Sender'] == $this->aMailBoxSettings['member_id']) {
                    ...
                    if ($aRow['Recipient'] == $this->aMailBoxSettings['member_id']) {
    ...
    function sendMessage($sMessageSubject, $sMessageBody, $vRecipientID, &$aComposeSettings, $isSimulateSending = false)
    {
        ...
        if (db_arr($sQuery)) {
            $sReturnMessage               = MsgBox(_t('_You have to wait for PERIOD minutes before you can write another message!',
                $this->iWaitMinutes, $sComposeUrl));
        ...
        if (!$isSimulateSending && $bAllowToSend) {
            ...
            if (db_res($sQuery)) {
                $sReturnMessage               = MsgBox(_t('_MESSAGE_SENT', $sComposeUrl, getProfileLink($iRecipientID),
                    $aRecipientInfo['NickName']));
                ...
                if ($aComposeSettings['notification']) {
                    ...
                    $aPlus['ProfileReference'] = getNickName($this->aMailBoxSettings['member_id']);
                    $aPlus['ProfileUrl']       = getProfileLink($this->aMailBoxSettings['member_id']);
                    sendMail($aRecipientInfo['Email'], $aTemplate['Subject'], $aTemplate['Body'], $iRecipientID,
                        $aPlus);
                }
