...
class BxDolSubscription
{
    ...
    function getMySubscriptions()
    {
        ...
        if ((int)$aUserInfo['EmailNotify'] == 0) {
            $sContent .= MsgBox(_t('_sbs_wrn_email_notify_disabled',
                BX_DOL_URL_ROOT . 'pedit.php?ID=' . (int)$aUserInfo['ID']));
        }
    ...
    function getData($bDynamic = false)
    {
        ...
        if (!$this->_bDataAdded) {
            ...
            $oForm = new BxTemplFormView($aForm);
            $sContent .= PopupBox($this->_sVisitorPopup, _t('_sys_bcpt_subscribe'),
                $oSysTemplate->parseHtmlByName('default_margin.html', array(
    ...
    {
        if ($this->_oDb->isSubscribed(array('user_id'   => $iUserId,
                                            'unit'      => $sUnit,
                                            'action'    => $sAction,
                                            'object_id' => $iObjectId
        ))
