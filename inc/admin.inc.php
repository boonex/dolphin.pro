...
function getID($str, $with_email = 1)
{
    if ($with_email) {
        if (filter_var($str,
            FILTER_VALIDATE_EMAIL)) { //eregi("^[_.0-9a-z-]+@([0-9a-z][0-9a-z-]+.)+[a-z]{2,4}$", $str) ) {
...
function check_login($ID, $passwd, $iRole = BX_DOL_ROLE_MEMBER, $error_handle = true)
{
    ...
    // if IP is banned
    if ((2 == getParam('ipBlacklistMode') && bx_is_ip_blocked()) || ('on' == getParam('sys_dnsbl_enable') && 'block' == getParam('sys_dnsbl_behaviour') && bx_is_ip_dns_blacklisted('',
                'login'))
    ) {
        if ($error_handle) {
...
function member_auth($member = 0, $error_handle = true, $bAjx = false)
{
    ...
    return check_login(process_pass_data($_COOKIE['memberID']), process_pass_data($_COOKIE['memberPassword']), $iRole,
        $error_handle);
}
...
function profile_delete($ID, $isDeleteSpammer = false)
{
    ...
    if ($isDeleteSpammer) {
        ...
        $oBxDolStopForumSpam->submitSpammer(array(
            'username' => $aProfileInfo['NickName'],
            'email'    => $aProfileInfo['Email'],
            'ip'       => bx_member_ip_get_last($ID)
        ));
    }

    // delete moxiemanager files
    $sMoxieFilesPath = BX_DIRECTORY_PATH_ROOT . 'media/moxie/files/' . substr($aProfileInfo['NickName'], 0,
            1) . '/' . substr($aProfileInfo['NickName'], 0, 2) . '/' . substr($aProfileInfo['NickName'], 0,
            3) . '/' . $aProfileInfo['NickName'];
    bx_rrmdir($sMoxieFilesPath);

    // create system event
    $oZ = new BxDolAlerts('profile', 'delete', $ID, 0,
        array('profile_info' => $aProfileInfo, 'logged_in' => $iLoggedInId, 'delete_spammer' => $isDeleteSpammer));
    $oZ->alert();
...
function get_user_online_status($ID)
{
    ...
    if ($ID && is_numeric($ID)) {
        ...
        if ($aMemberInfo['UserStatus'] != 'offline') {
            ...
            $iOnline = $GLOBALS['MySQL']->fromMemory("member_online_status.$ID.$min", 'getOne',
