...
function getCurrentLangName($isSetCookie = true)
{
    ...
    setlocale(LC_TIME, $sLang . '_' . strtoupper($sLang) . '.utf-8', $sLang . '_' . strtoupper($sLang) . '.utf8',
        $sLang . '.utf-8', $sLang . '.utf8', $sLang);
...
function checkLangExists($sLang)
{
    ...
    $iLangID = (int)$GLOBALS['MySQL']->fromCache('checkLangExists_' . $sLang, 'getOne',
...
{
    return (int)db_value("SELECT `ID` FROM `sys_localization_languages` WHERE `Name` = '" . process_db_input($sLangName,
...
function getProfileLangName($iProfile = false)
{
    ...
    return $GLOBALS['MySQL']->fromMemory('profile_lang_' . $iProfile, 'getOne',
...
{
    return $GLOBALS['MySQL']->getAllWithKey('SELECT * FROM `sys_localization_languages` ORDER BY `Title` ASC',
        $bRetIDs ? 'ID' : 'Name');
}
...
function compileLanguage($langID = 0)
{
    ...
    while ($arrLanguage = $resLangs->fetch()) {
        ...
        if ($arrLanguage['Name'] != BX_DOL_LANGUAGE_DEFAULT && getParam('lang_subst_from_en') == 'on') {
            $aKeysAll = getLocalizationKeysBy(array('type'  => 'by_language_name_key_key',
                                                    'value' => BX_DOL_LANGUAGE_DEFAULT
            ));
        ...
        $fileContent = "<" . "?PHP{$newLine}\$LANG_INFO=" . var_export($arrLanguage,
                true) . ";{$newLine}\$LANG = array(";
