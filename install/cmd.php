...
class BxDolInstallCmd
{
    ...
    protected function init()
    {
        ...
        if (file_exists($this->_sHeaderPath)) {
            $this->finish($this->_aReturnCodes['already installed']['code'],
                $this->_aReturnCodes['already installed']['msg']);
        }
    }
    ...
    public function checkRequirements()
    {
        if (!empty($GLOBALS['aErrors'])) {
            $this->finish($this->_aReturnCodes['requirements failed']['code'],
                $this->_aReturnCodes['requirements failed']['msg']);
        }
    }
    ...
    public function checkPermissions()
    {
        ...
        if ($sErrorMsg) {
            $this->finish($this->_aReturnCodes['permissions failed']['code'],
                $this->_aReturnCodes['permissions failed']['msg']);
        }
    }
    ...
    public function createSiteConfig()
    {
        ...
        if (!file_exists($this->_sHeaderPath)) {
            $this->finish($this->_aReturnCodes['create config failed']['code'],
                $this->_aReturnCodes['create config failed']['msg']);
        }
    }
    ...
    public function compileLanguage()
    {
        ...
        if (empty($sLang) || !file_exists($GLOBALS['aPathInfo']['dirname'] . '/../langs/lang-' . $sLang . '.php')) {
            $this->finish($this->_aReturnCodes['lang compile failed']['code'],
                $this->_aReturnCodes['lang compile failed']['msg']);
        }
    }
