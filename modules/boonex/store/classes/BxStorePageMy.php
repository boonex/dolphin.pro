...
class BxStorePageMy extends BxDolPageView
{
    ...
    function getBlockCode_Owner()
    {
        ...
        $aMenu    = array(
            _t('_bx_store_block_submenu_main')             => array(
                'href'   => $sBaseUrl,
                'active' => !bx_get('bx_store_filter')
            ),
            _t('_bx_store_block_submenu_add_product')      => array(
                'href'   => $sBaseUrl . '&bx_store_filter=add_product',
                'active' => 'add_product' == bx_get('bx_store_filter')
            ),
            _t('_bx_store_block_submenu_manage_products')  => array(
                'href'   => $sBaseUrl . '&bx_store_filter=manage_products',
                'active' => 'manage_products' == bx_get('bx_store_filter')
            ),
            _t('_bx_store_block_submenu_pending_products') => array('href'   => $sBaseUrl . '&bx_store_filter=pending_products',
                                                                    'active' => 'pending_products' == bx_get('bx_store_filter')
            ),
        );
    ...
    function getBlockCode_Browse()
    {
        ...
        $o                  = new BxStoreSearchResult('user',
            process_db_input($this->_aProfile['NickName'], BX_TAGS_NO_ACTION, BX_SLASHES_NO_ACTION));
        $o->aCurrent['rss'] = 0;
    ...
    function getBlockCode_Main()
    {
        ...
        $aVars    = array('msg' => '');
        if ($iPending) {
            $aVars['msg'] = sprintf(_t('_bx_store_msg_you_have_pending_approval_products'),
                $sBaseUrl . '&bx_store_filter=pending_products', $iPending);
        } elseif (!$iActive) {
            $aVars['msg'] = sprintf(_t('_bx_store_msg_you_have_no_products'),
                $sBaseUrl . '&bx_store_filter=add_product');
        } else {
            $aVars['msg'] = sprintf(_t('_bx_store_msg_you_have_some_products'),
                $sBaseUrl . '&bx_store_filter=manage_products', $iActive, $sBaseUrl . '&bx_store_filter=add_product');
        }
    ...
    {
        $sForm = $this->_oMain->_manageEntries('user',
            process_db_input($this->_aProfile['NickName'], BX_TAGS_NO_ACTION, BX_SLASHES_NO_ACTION), false,
            'bx_store_user_form', array(
                'action_delete' => '_bx_store_admin_delete',
