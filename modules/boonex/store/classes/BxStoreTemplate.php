        ...
        $aVars = array(
            ...
            'bx_if:full'    => array(
                ...
                'content'   => array(
                    ...
                    'rate'       => $oVotingView ? $oVotingView->getJustVotingElement(0, $aData['id'],
                        $aData['rate']) : '&#160;',
                ),
            ),
        );
    ...
    function blockFiles(&$aData)
    {
        ...
        foreach ($aReadyMedia as $r) {
            ...
            $aVars['bx_repeat:files'][] = array(
                ...
                'for_group'      => sprintf(_t('_bx_store_for_group'),
                    $GLOBALS['oBxStoreModule']->getGroupName($r['allow_purchase_to'])),
                'date'           => defineTimeInterval($a['date']),
                'bx_if:purchase' => array(
                    'condition' => $GLOBALS['oBxStoreModule']->isAllowedPurchase($r),
                    'content'   => array(
                        'btn_purchase' => $oPayment->getAddToCartLink($r['author_id'], $this->_oConfig->getId(),
                            $r['id'], 1),
                    ),
                ),
