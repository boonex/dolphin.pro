...
class BxAvaModule extends BxDolModule
{
    ...
    function actionHome()
    {
        ...
        if ('bx_photos_thumb' == getParam('sys_member_info_thumb') && 'bx_photos_icon' == getParam('sys_member_info_thumb_icon')) {
            $sProfilePhotosUrl = BxDolService::call('photos', 'get_manage_profile_photo_url',
                array($this->_iProfileId, 'profile_album_name'));
    ...
    function serviceSetImageForCropping($iProfileId, $sOrigPath)
    {
        ...
        if (@copy($sOrigPath, $sImagePath)) {
            return IMAGE_ERROR_SUCCESS == imageResize($sImagePath, '', BX_AVA_PRE_RESIZE_W, BX_AVA_PRE_RESIZE_H,
                true) ? true : false;
        }
    ...
    function serviceGetSiteAvatars($iPage)
    {
        ...
        return $sAjaxPaginate . '<div class="bx-def-padding-thd">' . $GLOBALS['oFunctions']->centerContent($this->_oTemplate->parseHtmlByName('avatars_site',
            $aVars), '.bx_ava_tar') . '</div>' . $sAjaxPaginate . $sScript;
    }
    ...
    function serviceGetMyAvatars($iProfileId)
    {
        ...
        foreach ($aAvatars as $iAvatar) {
            $aFiles[] = array(
                ...
                'bx_if:is_not_main'        => array(
                    'condition' => $iAvatarCurrent != $iAvatar,
                    'content'   => array('name' => $iAvatar . BX_AVA_EXT)
                ),
                'bx_if:is_not_main_couple' => array(
        ...
        return $GLOBALS['oFunctions']->centerContent($this->_oTemplate->parseHtmlByName('avatars_my', $aVars),
            '.bx_ava_tar') . $sScript;
    }
            ...
            if ($_POST['copy_to_profile_photos']) {
                if (BxDolRequest::serviceExists('photos', 'perform_photo_upload', 'Uploader')) {
                    ...
                    $aFileInfo = array(
                        ...
                        'album'        => str_replace('{nickname}', getUsername($iProfileId),
                            getParam('bx_photos_profile_album_name')),
                    ...
                    BxDolService::call('photos', 'perform_photo_upload', array($sImagePath, $aFileInfo, false),
                        'Uploader');
                }
            }
    ...
    function _cropAvatar()
    {
        ...
        if ((int)$_POST['w'] && (int)$_POST['h']) {
            ...
            $bRet  = $aSize ? $o->crop($aSize['w'], $aSize['h'], (int)$_POST['x1'], (int)$_POST['y1'], (int)$_POST['w'],
                (int)$_POST['h'], 0, $sSrcImagePath) : IMAGE_ERROR_WRONG_TYPE;
        } else {
            $o->setSquareResize(true);
