...
class BxPageACDb extends BxDolModuleDb
{
    ...
    {
        if (empty($aVisibleTo)) {
            $this->query("DELETE FROM `{$this->_sPrefix}{$sType}_menu_visibility` WHERE `MenuItemID` = {$iMenuItemID}");
        } else {
            $this->query("REPLACE `{$this->_sPrefix}{$sType}_menu_visibility` SET `MemLevels` = '" . serialize($aVisibleTo) . "', `MenuItemID` = {$iMenuItemID}");
        }
    ...
    {
        if (empty($aMemLevels)) {
            $this->query("DELETE FROM `{$this->_sPrefix}page_blocks_visibility` WHERE `PageBlockID` = {$iID}");
        } else {
            $this->query("REPLACE `{$this->_sPrefix}page_blocks_visibility` SET `MemLevels` = '" . serialize($aMemLevels) . "', `PageBlockID` = {$iID}");
        }
    ...
    {
        $aCache = $this->fromCache($sType . '_menu', 'getAllWithKey',
            "SELECT `MenuItemID`, `MemLevels` FROM `{$this->_sPrefix}{$sType}_menu_visibility`", 'MenuItemID');
        if (!empty($aCache)) {
    ...
    {
        $aCache = $this->fromCache('page_blocks', 'getAllWithKey',
            "SELECT `PageBlockID`, `MemLevels` FROM `{$this->_sPrefix}page_blocks_visibility`", 'PageBlockID');
        if (!empty($aCache)) {
            foreach ($aCache as $iID => $aItem) {
                $aCache[$iID]['MemLevels'] = empty($aCache[$iID]['MemLevels']) ? array() : unserialize($aCache[$iID]['MemLevels']);
            }
