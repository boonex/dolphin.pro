...
class BxPageACModule extends BxDolModule
{
    ...
    function actionGetPageRules()
    {
        ...
        $sRet .= DesignBoxAdmin(_t('_bx_pageac_note'),
            $GLOBALS['oAdmTemplate']->parseHtmlByName('design_box_content.html',
                array('content' => _t('_bx_pageac_note_text'))));
    ...
    function actionGetRulesList($bAddWrapper = false)
    {
        ...
        if ($bAddWrapper) {
            $sRet = '<div id="rules_list">' . $sRet . '</div>';
        }
    ...
    function actionTopMenu($sAction, $iMenuItemID)
    {
        ...
        if ($sAction == 'edit') {
            ...
            return PopupBox('pageac_popup_edit_form', _t('_bx_pageac_visible_for'),
                $GLOBALS['oAdmTemplate']->parseHtmlByName('design_box_content.html', array(
                    'content' => $this->_oTemplate->getMenuItemEditForm('top', $iMenuItemID,
                            $aMenuItemVisibility) . LoadingBox('formItemEditLoading')
                )));
        } elseif ($sAction == 'save') {
    ...
    function actionMemberMenu($sAction, $iMenuItemID)
    {
        ...
        if ($sAction == 'edit') {
            ...
            return PopupBox('pageac_popup_edit_form', _t('_bx_pageac_visible_for'),
                $GLOBALS['oAdmTemplate']->parseHtmlByName('design_box_content.html', array(
                    'content' => $this->_oTemplate->getMenuItemEditForm('member', $iMenuItemID,
                            $aMenuItemVisibility) . LoadingBox('formItemEditLoading')
                )));
        } elseif ($sAction == 'save') {
    ...
    function actionPageBlock($sAction, $iID)
    {
        ...
        if ($sAction == 'edit') {
            ...
            return PopupBox('pageac_popup_edit_form', _t('_bx_pageac_visible_for'),
                $GLOBALS['oAdmTemplate']->parseHtmlByName('design_box_content.html', array(
                    'content' => $this->_oTemplate->getPageBlockEditForm($iID,
                            $aVisibility) . LoadingBox('formItemEditLoading')
                )));
        } elseif ($sAction == 'save') {
            $aVisibleTo = array();
            if (is_array($_POST['mlv_visible_to'])) {
    ...
    function _validateRule($sRule, $bIsAdvanced)
    {
        ...
        if (strpos($sRule, BX_DOL_URL_ROOT) === 0) {
            $sRule = substr($sRule, strlen(BX_DOL_URL_ROOT));
        } //if URL starts from http://www.site.com
        elseif (strpos($sRule, $sBaseURL) === 0) {
            $sRule = substr($sRule, strlen($sBaseURL) + 1);
        } //if URL starts from www.site.com
        else {
            if (strpos($sBaseURL, 'www.') === 0) {
                $sBaseURL = substr($sBaseURL, 4);
                if (strpos($sRule, $sBaseURL) === 0) {
                    $sRule = substr($sRule, strlen($sBaseURL) + 1);
                } //if URL starts from site.com
            }
        }
    ...
    function serviceMenuItemsFilter($sType, &$aItems)
    {
        ...
        define('BX_DOL_ROLE_MEMBER',
            1);    //this code is required here because at the time of BxDolMenu::load function call profiles.inc.php isn't fully included yet,
        define('BX_DOL_ROLE_ADMIN',
            2);    //thus all defines and function calls located in profiles.inc.php aren't executed at this moment
        ...
        if (!isLogged()) {
            return;
        } //non-members visibility controlled by default in builders
    ...
    function servicePageBlocksFilter(&$oBxDolPageView)
    {
        ...
        foreach ($oBxDolPageView->aPage['Columns'] as $iColumn => $aColumn) {
            foreach ($aColumn['Blocks'] as $iBlockID => $aBlock) {
                if (!empty($aPageBlocksCache[$iBlockID]) && !$aPageBlocksCache[$iBlockID]['MemLevels'][$iMemLevel]) {
                    unset($oBxDolPageView->aPage['Columns'][$iColumn]['Blocks'][$iBlockID]);
                }
            }
        }
    }
