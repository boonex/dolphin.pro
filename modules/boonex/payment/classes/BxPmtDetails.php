...
class BxPmtDetails
{
    ...
    function getForm($iUserId)
    {
        ...
        foreach ($aInputs as $aInput) {
            ...
            if (!empty($aAddon) && is_array($aAddon)) {
                $this->_aForm['inputs'][$aInput['name']] = array_merge($this->_aForm['inputs'][$aInput['name']],
                    $aAddon);
            }
        }
        ...
        if ($oForm->isSubmittedAndValid()) {
            ...
            foreach ($aOptions as $aOption) {
                $this->_oDb->updateOption($iUserId, $aOption['id'],
                    process_db_input(isset($_POST[$aOption['name']]) ? $_POST[$aOption['name']] : "", BX_TAGS_STRIP));
            }
    ...
    function getFormBlock($iUserId = -1)
    {
        ...
        $aTopMenu = array(
            'pmt-orders-processed-lnk' => array(
                'href'  => $sBaseUrl . 'orders/processed/',
                'title' => _t($sLangsPrefix . 'btn_orders_processed')
            ),
            'pmt-orders-pending-lnk'   => array(
                'href'  => $sBaseUrl . 'orders/pending/',
                'title' => _t($sLangsPrefix . 'btn_orders_pending')
            ),
            'pmt-payment-settings-lnk' => array(
                'href'   => $sBaseUrl . 'details/',
                'title'  => _t($sLangsPrefix . 'btn_settings'),
                'active' => 1
            )
        );
        ...
        return array(
            (!empty($sResult) ? $sResult : MsgBox(_t($sLangsPrefix . 'msg_no_results'))),
            $aTopMenu,
            array(),
            false,
            'getBlockCaptionMenu'
