...
class BxPmtModule extends BxDolModule
{
    ...
    function actionManualOrderSubmit()
    {
        ...
        if (!$this->isLogged()) {
            return $this->_onResultInline(array_merge($aResult,
                array('code' => 1, 'message' => $this->_sLangsPrefix . 'err_required_login')));
        }
    ...
    function actionOrdersSubmit($sType)
    {
        ...
        if (!isset($aData['orders']) || !is_array($aData['orders']) || empty($aData['orders'])) {
            return $this->_onResultInline(array_merge($aResult,
                array('code' => 2, 'message' => $this->_sLangsPrefix . 'err_nothing_selected')));
        }
        ...
        if (isset($aData['pmt-report']) && !empty($aData['pmt-report'])) {
            ...
            if (isset($aData['pmt-cancel']) && !empty($aData['pmt-cancel'])) {
                $mixedResult = $this->_oOrders->cancel($sType, $aData['orders']);
            } else {
                if (isset($aData['pmt-process']) && !empty($aData['pmt-process']) && $sType == BX_PMT_ORDERS_TYPE_PENDING) {
    ...
    function getCartHistory($iVendorId)
    {
        $aTopMenu = array(
            'pmt-cart'         => array(
                'href'  => BX_DOL_URL_ROOT . $this->_oConfig->getBaseUri() . 'cart/',
                'title' => _t($this->_sLangsPrefix . 'btn_cart')
            ),
            'pmt-cart-history' => array(
                'href'   => BX_DOL_URL_ROOT . $this->_oConfig->getBaseUri() . 'history/' . ($iVendorId == BX_PMT_ADMINISTRATOR_ID ? 'site/' : ''),
                'title'  => _t($this->_sLangsPrefix . 'btn_history'),
                'active' => 1
            )
        );
    ...
    function getCartContent($mixedVendor = null)
    {
        ...
        $sResult = !empty($aCartInfo) ? $this->_oTemplate->displayCartContent($aCartInfo,
            $iVendorId) : MsgBox(_t($this->_sLangsPrefix . 'msg_no_results'));
    ...
    function serviceGetAddToCartJs(
        $iVendorId,
        $mixedModuleId,
        $iItemId,
        $iItemCount,
    ...
    function actionCartSubmit()
    {
        ...
        if (isset($aData['pmt-delete']) && !empty($aData['items'])) {
            ...
            if (isset($aData['pmt-checkout']) && !empty($aData['items'])) {
    ...
    function initializeCheckout($iVendorId, $sProvider, $aItems = array())
    {
        ...
        foreach ($aInfo['items'] as $iIndex => $aItem) {
            if ((int)$aInfo['client_id'] != 0 && (float)$aItem['price'] == 0) {
                $aItemInfo = BxDolService::call((int)$aItem['module_id'], 'register_cart_item', array(
                    $aInfo['client_id'],
                    $aInfo['vendor_id'],
                    $aItem['id'],
                    $aItem['quantity'],
                    $this->_oConfig->generateLicense()
                ));
                ...
                $sCartItems = trim(preg_replace("'" . implode(BxPmtCart::$DESCRIPTOR_DIVIDER,
                        array($aInfo['vendor_id'], $aItem['module_id'], $aItem['id'], $aItem['quantity'])) . ":?'", "",
                    $sCartItems), ":");
    ...
    function actionFinalizeCheckout($sProvider, $mixedVendorId = "")
    {
        ...
        $aProvider  = is_numeric($mixedVendorId) && (int)$mixedVendorId != BX_PMT_EMPTY_ID ? $this->_oDb->getVendorInfoProviders((int)$mixedVendorId,
            $sProvider) : $this->_oDb->getProviders($sProvider);
        $sClassPath = !empty($aProvider['class_file']) ? BX_DIRECTORY_PATH_ROOT . $aProvider['class_file'] : $this->_oConfig->getClassPath() . $aProvider['class_name'] . '.php';
        if (empty($aProvider) || !file_exists($sClassPath)) {
    ...
    function performJoin($iPendingId, $aPayment = array())
    {
        ...
        if (!empty($aPayment['payer_name']) && !empty($aPayment['payer_email'])) {
            ...
            $aTemplate = $oEmailTemplates->parseTemplate($this->_sEmailTemplatesPrefix . 'paid_need_join', array(
                ...
                'JoinLink' => bx_append_url_params($this->_oConfig->getJoinUrl(),
                    array($this->_sRequestKeyPending => (int)$iPendingId))
            ));

            sendMail($aPayment['payer_email'], $aTemplate['Subject'], $aTemplate['Body'], 0, array(), 'html', false,
                true);
        }
    ...
    function actionJoin()
    {
        ...
        $oAlert = new BxDolAlerts('system', 'join_after_payment', 0, 0,
            array('override' => &$bOverride, 'override_error' => &$sOverrideError));
        $oAlert->alert();
