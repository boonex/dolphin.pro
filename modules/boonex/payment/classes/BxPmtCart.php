...
class BxPmtCart
{
    ...
    {
        return $this->_oTemplate->displayAddToCartJs($iVendorId, $iModuleId, $iItemId, $iItemCount, $bNeedRedirect,
            $bWrapped);
    }
    ...
    function addToCart($iClientId, $iVendorId, $iModuleId, $iItemId, $iItemCount)
    {
        ...
        if (strpos($sCartItems, $iVendorId . $sDd . $iModuleId . $sDd . $iItemId . $sDd) !== false) {
            $sCartItems = preg_replace("'" . $iVendorId . $sDd . $iModuleId . $sDd . $iItemId . $sDd . "([0-9])+'e",
                "'" . $iVendorId . $sDd . $iModuleId . $sDd . $iItemId . $sDd . "' . (\\1 + " . $iItemCount . ")",
                $sCartItems);
        } else {
        ...
        return array(
            'code'           => 0,
            'message'        => _t($this->_sLangsPrefix . 'inf_successfully_added'),
            'total_quantity' => $iTotalQuantity,
            'content'        => $this->_oTemplate->displayToolbarSubmenu($aInfo)
        );
    }
    ...
    function getInfo($iUserId, $iVendorId = BX_PMT_EMPTY_ID, $aItems = array())
    {
        ...
        if ($iVendorId != BX_PMT_EMPTY_ID) {
            return isset($aContent[$iVendorId]) ? $this->_getInfo($iUserId, $iVendorId,
                $aContent[$iVendorId]) : array();
        }
    ...
    {
        $aPending = is_array($mixedPending) ? $mixedPending : $this->_oDb->getPending(array(
            'type' => 'id',
            'id'   => (int)$mixedPending
        ));
    ...
    function _getInfo($iClientId, $iVendorId, $aItems)
    {
        ...
        $aItemsInfo  = array();
        foreach ($aItems as $aItem) {
            $aItemInfo              = BxDolService::call((int)$aItem['module_id'], 'get_cart_item',
                array($iClientId, $aItem['item_id']));
