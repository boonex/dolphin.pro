...
class BxPmtTemplate extends BxDolModuleTemplate
{
    ...
    function displayOrder($sType, $iId)
    {
        ...
        foreach ($aItems as $aItem) {
            $aInfo = BxDolService::call((int)$aItem['module_id'], 'get_cart_item',
                array($aOrder['client_id'], $aItem['item_id']));
    ...
    function displayOrdersBlock($sType, $iVendorId)
    {
        ...
        //--- Get Filter Panel ---//
        $sFilterPanel = BxTemplSearchResult::showAdminFilterPanel('', 'pmt-filter-text-' . $sType,
            'pmt-filter-enable-' . $sType, 'filter', $sJsObject . ".applyFilter('" . $sType . "', this)");
        ...
        if ($sType == BX_PMT_ORDERS_TYPE_PROCESSED) {
            $aButtons['pmt-manual'] = array('type'    => 'button',
                                            'name'    => 'pmt-manual',
                                            'value'   => _t($this->_sLangsPrefix . 'btn_manual_order'),
                                            'onclick' => 'onclick="javascript:' . $sJsObject . '.addManually(this);"'
            );
        }
    ...
    function displayHistoryBlock($iUserId, $iVendorId)
    {
        ...
        //--- Get Filter Panel ---//
        $sFilterPanel = BxTemplSearchResult::showAdminFilterPanel('', 'pmt-filter-text-history',
            'pmt-filter-enable-history', 'filter', $sJsObject . ".applyFilter('history', this)");
        ...
        return $this->parseHtmlByName('history_orders_block.html', array(
            ...
            'txt_license'  => _t($this->_sLangsPrefix . 'txt_license'),
            'txt_action'   => _t($this->_sLangsPrefix . 'txt_action'),
            'orders'       => $this->displayOrders('history',
                array('user_id' => $iUserId, 'seller_id' => $iVendorId, 'start' => 0)),
            'filter_panel' => $sFilterPanel,
    ...
    function displayToolbarSubmenu($aInfo)
    {
        ...
        return $this->addCss('toolbar.css', true) . $this->parseHtmlByName('toolbar_submenu.html',
            array('bx_repeat:carts' => $aCarts));
    }
    ...
    function displayCartContent($aCartInfo, $iVendorId = BX_PMT_EMPTY_ID)
    {
        ...
        foreach ($aCartInfo as $aVendor) {
            ...
            $sControlPanel = BxTemplSearchResult::showAdminActionsPanel('items_from_' . $aVendor['vendor_id'],
                $aButtons, 'items', true, true);
            ...
            $aVendors[] = array(
                ...
                'bx_if:show_link'     => array(
                    ...
                    'content'   => array(
                        'txt_shopping_cart'    => _t($this->_sLangsPrefix . 'txt_shopping_cart',
                            $this->parseHtmlByName('vendor_link.html', array(
        ...
        return $this->parseHtmlByName('cart.html',
            array_merge($this->_getJsContentCart(), array('bx_repeat:vendors' => $aVendors)));
    }
