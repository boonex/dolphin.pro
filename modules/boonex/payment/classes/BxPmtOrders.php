...
class BxPmtOrders
{
    ...
    function getOrdersBlock($sType, $iUserId = BX_PMT_EMPTY_ID)
    {
        ...
        $sJsObject = $this->_oConfig->getJsObject('orders');
        $aTopMenu  = array(
            'pmt-orders-processed-lnk' => array('href'   => $sBaseUrl . 'orders/processed/',
                                                'title'  => _t($sLangsPrefix . 'btn_orders_processed'),
                                                'active' => $sType == BX_PMT_ORDERS_TYPE_PROCESSED ? 1 : 0
            ),
            'pmt-orders-pending-lnk'   => array('href'   => $sBaseUrl . 'orders/pending/',
                                                'title'  => _t($sLangsPrefix . 'btn_orders_pending'),
                                                'active' => $sType == BX_PMT_ORDERS_TYPE_PENDING ? 1 : 0
            ),
            'pmt-payment-settings-lnk' => array('href'  => $sBaseUrl . 'details/',
                                                'title' => _t($sLangsPrefix . 'btn_settings')
            )
        );

        $sTitle   = $this->_sLangsPrefix . ($sType == 'processed' ? 'bcpt_processed_orders' : 'bcpt_pending_orders');
        $sContent = $this->_oTemplate->displayOrdersBlock($sType,
            $iUserId != BX_PMT_EMPTY_ID ? $iUserId : $this->_iUserId);
