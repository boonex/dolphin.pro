...
class BxPmtBitPay extends BxPmtProvider
{
    /**
     * Constructor
     */
    ...
    function initializeCheckout($iPendingId, $aCartInfo, $bRecurring = false, $iRecurringDays = 0)
    {
        ...
        $this->aBpOptions['notificationURL'] = bx_append_url_params($this->aBpOptions['notificationURL'],
            array('bxssl' => 1));
    ...
    function finalizeCheckout(&$aData)
    {
        ...
        if ((in_array($sSpeed, array(
                    BP_SPEED_HIGH,
                    BP_SPEED_MEDIUM
                )) && $sStatus == BP_STATUS_CONFIRMED) || ($sSpeed == BP_SPEED_LOW && $sStatus == BP_STATUS_COMPLETE)
        ) {
    ...
    public function createInvoice($orderId, $price, $posData, $options = array())
    {
        ...
        try {
            $options = array_merge($this->aBpOptions,
                $options);  // $options override any options found in Bitpay::aOptions
            $pos     = array('d' => $posData);
    ...
    protected function _initializeOptions()
    {
        ...
        $this->aBpOptions['redirectURL'] = BX_DOL_URL_ROOT . $this->_oConfig->getBaseUri() . 'act_checkout_finished/' . $this->_sName . '/';
        ...
        try {
            ...
            if ($uname) {
                ...
                curl_setopt($curl, CURLOPT_SSL_VERIFYHOST,
                    2); // check existence of CN and verify that it matches hostname
