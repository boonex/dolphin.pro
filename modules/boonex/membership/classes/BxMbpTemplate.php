...
class BxMbpTemplate extends BxDolModuleTemplate
{
    ...
    function displayAvailableLevels($aValues)
    {
        ...
        foreach ($aValues as $aValue) {
            list($sJsCode, $sJsMethod) = $oPayment->getAddToCartJs(0, $this->_oConfig->getId(), $aValue['price_id'], 1,
                true);
    ...
    function displaySelectLevelBlock($aLevels, $bDynamic = false)
    {
        ...
        foreach ($aLevels as $iIndex => $aLevel) {
            ...
            $aTmplVarsLevels[] = array(
                'level' => $this->parseHtmlByName('select_level.html', array(
                    ...
                    'id'                     => $aLevel['mem_id'],
                    'descriptor'             => (int)$aLevel['mem_id'] == MEMBERSHIP_ID_STANDARD ? $this->_oConfig->getStandardDescriptor() : $oPayment->getCartItemDescriptor(0,
                        $iModuleId, $aLevel['price_id'], 1),
        ...
        return array(
            ($bDynamic ? $sCssJs : '') . $this->parseHtmlByName('select_level_block.html', $aTmplParams),
            array(),
            array(),
            false
        );
