...
class BxDolOrcaForumsHome extends BxDolPageView
{
    ...
    function getBlockCode_ShortIndex()
    {
        ...
        foreach ($ac as $c) {
            $s .= '<div class="forums_index_short_cat bx-def-font-large"><a href="' . $gConf['url']['base'] . sprintf($gConf['rewrite']['cat'],
                    $c['cat_uri'],
                    0) . '" onclick="return f.selectForumIndex(\'' . $c['cat_uri'] . '\')">' . $c['cat_name'] . '</a></div>';
            $af = $GLOBALS['f']->fdb->getForumsByCatUri(filter_to_db($c['cat_uri']));
            foreach ($af as $ff) {
                $s .= '<div class="forums_index_short_forum bx-def-padding-sec-left"><a href="' . $gConf['url']['base'] . sprintf($gConf['rewrite']['forum'],
                        $ff['forum_uri'],
                        0) . '" onclick="return f.selectForum(\'' . $ff['forum_uri'] . '\', 0)">' . $ff['forum_title'] . '</a></div>';
            }
        }
        $s .= '</div>';
...
$GLOBALS['BxDolTemplateInjections']['page_' . $_ni]['injection_body'][] = array(
    'type' => 'text',
    'data' => 'id="body" onload="if(!document.body) { document.body = document.getElementById(\'body\'); }; h = new BxHistory(\'' . $gConf['url']['base'] . '\'); document.h = h; return h.init(\'h\'); "'
);
...
function(ed) {
    ed.on("init", function(e) {
        ...
        $sEditor .= $oEditor->attachEditor($sEditorId,
                BX_EDITOR_FULL) . '<div id="fakeEditor" style="display:none;"></div>';
    }
}
