...
class Forum extends ThingPage
{
    ...
    function getSearchResultsXML($text, $type, $forum, $u, $disp, $start = 0, $isWholePage = false)
    {
        ...
        switch ($type) {
            case 'msgs':
            case 'tlts':
                $a = $this->fdb->searchMessages(filter_to_db($text), filter_to_db($u), $forum, $type,
                    ...
                    if ($text) {
                        ...
                        while (list (, $w) = each($ws)) {
                            if ($w) {
                                ...
                                $r['topic_title'] = preg_replace("/($wreg)/i",
                                    "<span style=\"background-color:yellow\">$w</span>", $r['topic_title']);
                            }
                        }
                    }
                    ...
                    if ($text) {
                        ...
                        while (list (, $w) = each($ws)) {
                            if ($w) {
                                ...
                                if ($ind) {
                                    $r['post_text'] = preg_replace("/($wreg)(?=[^>]*<)/i",
                                        "<span style=\"background-color:yellow\">$w</span>",
                                        "<div>{$r['post_text']}</div>");
                                } else {
                                    $r['post_text'] = preg_replace("/($wreg)/i",
                                        "<span style=\"background-color:yellow\">$w</span>", $r['post_text']);
                                }
                            }
                        }
                    }
...
EOS;
        ...
        if ($isWholePage) {
            ...
            return $this->addHeaderFooter($li,
    ...
    function getHiddenPostXML($post_id, $force_show)
    {
        ...
        if (!$allow_edit && $r['user'] == $this->_getLoginUserName()) {
            if ($this->_checkUserPerm($r['user'], 'own', 'edit',
                    $forum_id) && !$this->_isEditTimeout($r['post_id']) && !$t['topic_locked']
            ) {
                $allow_edit = 1;
            }
        }
...
EOF;
    ...
    function isNewTopic($topic_id, $topic_last_time, $user_last_time)
    {
        ...
        if ($topic_last_time > $user_last_time) {
            return 1;
        }
    ...
    function getForumsShortXML($cat, $root, $sCheckPermission = false)
    {
        ...
        while (list (, $rr) = each($aa)) {
            if ($sCheckPermission && !$this->_checkUserPerm('', $rr['forum_type'], $sCheckPermission,
                    $rr['forum_id'])
            ) {
                continue;
            }
...
EOF;
    ...
    function postReplyXML(&$p)
    {
        ...
        if (!$this->_checkUserPerm('', $f['forum_type'], 'post',
                (int)$p['forum_id']) || $t['topic_locked'] || bx_is_spam($p['topic_text'])
        ) {
            return <<<EOF
<html>
<body>
...
EOF;
    ...
    function topicAutoContinue($t)
    {
        ...
        $sNewTopicSubj = ($aTopicFirst ? $aTopicFirst['topic_title'] : $t['topic_title']) . sprintf('[L[continue_topic_suffix - %d]]',
                $iTopicIndex);
        $sOldTopicUrl  = $gConf['url']['base'] . sprintf($gConf['rewrite']['topic'], $t['topic_uri']);
        $sNewTopicText = sprintf('[L[continue_topic_first %s]]',
            '<a href="' . $sOldTopicUrl . '">' . $t['topic_title'] . '</a>');
        prepare_to_db($sNewTopicSubj, -1);
        prepare_to_db($sNewTopicText, 1);
        $iNewTopicReply = $this->fdb->newTopic((int)$t['forum_id'], $sNewTopicSubj, $sNewTopicText, false,
            $gConf['robot'], $sNewTopicUri);
        ...
        $sOldTopicText = sprintf('[L[continue_topic_last %s]]',
            '<a href="' . $sNewTopicUrl . '">' . $aNewTopic['topic_title'] . '</a>');
    ...
    function editPost($post_id, $topic_id, $text)
    {
        ...
        if ($no_access && $user == $this->_getLoginUserName()) {
            if ($this->_checkUserPerm('', 'own', 'edit',
                    $t['forum_id']) && !$this->_isEditTimeout((int)$post_id) && !$t['topic_locked']
            ) {
                $no_access = false;
            }
        }
    ...
    function deletePostXML($post_id, $topic_id, $forum_id)
    {
        ...
        if ($no_access && $sPostUser == $sUser) {
            if ($this->_checkUserPerm('', 'own', 'del',
                    $f['forum_id']) && !$this->_isEditTimeout((int)$post_id) && !$aTopic['topic_locked']
            ) {
                $no_access = false;
            }
        }
...
EOF;
    ...
    function hidePost($is_hide, $post_id)
    {
        ...
        $user = $this->_getLoginUserName();
        if (0 != strcasecmp($_SERVER['REQUEST_METHOD'], 'POST') || !$this->_checkUserPerm($user, '', 'hide_posts',
                $f ? $f['forum_id'] : 0)
        ) {
            return '<ret>0</ret>';
        }
...
EOS;
    ...
    {
        $user = $this->_getLoginUserName();
        if (0 != strcasecmp($_SERVER['REQUEST_METHOD'], 'POST') || !$this->_checkUserPerm($user, '', 'move_topics',
    ...
    function delTopic($topic_id)
    {
        ...
        $user = $this->_getLoginUserName();
        if (0 != strcasecmp($_SERVER['REQUEST_METHOD'], 'POST') || !$this->_checkUserPerm($user, '', 'del_topics',
                $f ? $f['forum_id'] : 0)
        ) {
            return '<ret>0</ret>';
        }
    ...
    function hideTopic($is_hide, $topic_id)
    {
        $f    = $this->fdb->getForumByTopicId((int)$topic_id);
        $user = $this->_getLoginUserName();
        if (0 != strcasecmp($_SERVER['REQUEST_METHOD'], 'POST') || !$this->_checkUserPerm($user, '',
                $is_hide ? 'hide_topics' : 'unhide_topics', $f ? $f['forum_id'] : 0)
        ) {
            return '<ret>0</ret>';
        }
    ...
    function stick($topic_id)
    {
        $f    = $this->fdb->getForumByTopicId((int)$topic_id);
        $user = $this->_getLoginUserName();
        if (0 != strcasecmp($_SERVER['REQUEST_METHOD'], 'POST') || !$this->_checkUserPerm($user, '', 'sticky',
                $f ? $f['forum_id'] : 0)
        ) {
            return '<ret>0</ret>';
        }
    ...
    function lock($topic_id)
    {
        $f    = $this->fdb->getForumByTopicId((int)$topic_id);
        $user = $this->_getLoginUserName();
        if (0 != strcasecmp($_SERVER['REQUEST_METHOD'], 'POST') || !$this->_checkUserPerm($user, '', 'lock',
                $f ? $f['forum_id'] : 0)
        ) {
            return '<ret>0</ret>';
        }
...
EOF;
        ...
        $post_id   = $this->fdb->newTopic((int)$p['forum'], $p['topic_subject'], $p['topic_text'],
            ($p['topic_sticky'] == 'on'), $user, $topic_uri);
        ...
        if (is_callable($this->onNewTopic)) {
            call_user_func_array($this->onNewTopic, array(
                (int)$p['forum'],
                $p['topic_subject'],
                $p['topic_text'],
                ($p['topic_sticky'] == 'on'),
...
EOF;
    ...
    function getRssForum($forum_uri)
    {
        ...
        while (list (, $r) = each($a)) {
            ...
            $items .= $this->_rssItem($r['topic_title'], sprintf($gConf['rewrite']['topic'], $r['topic_uri']), $td,
                $lp['when']);
        }

        return $this->_rssFeed($f['forum_title'], sprintf($gConf['rewrite']['forum'], $f['forum_uri'], 0),
            $f['forum_title'], $lastBuildDate, $items);
    }
    ...
    function getRssUpdatedTopics()
    {
        ...
        while (list (, $r) = each($a)) {
            ...
            $td = orca_mb_replace('/#/', $r['count_posts'], '[L[# posts]]') . ' &#183; ' . orca_mb_replace('/#/',
                    $ui[$r['last_post_user']]['title'],
                    '[L[last reply by #]]') . ' &#183; ' . $r['cat_name'] . ' &#187; ' . $r['forum_title'];
    ...
    function getRssTopic($topic_uri)
    {
        ...
        while (list (, $r) = each($a)) {
            ...
            if (!$lastBuildDate) {
                $lastBuildDate = $a['when'];
            }

            $items .= $this->_rssItem($tt, sprintf($gConf['rewrite']['topic'], $t['topic_uri']), $td, $r['when']);
        }

        return $this->_rssFeed($t['topic_title'], sprintf($gConf['rewrite']['topic'], $t['topic_uri']),
            $t['topic_title'], $lastBuildDate, $items);
    }
    ...
    function getRssUser($user, $sort)
    {
        ...
        return $this->_rssFeed($sTitle,
            '?action=goto&amp;search=1#action=goto&amp;search_result=1&amp;&amp;msgs&amp;0&amp;' . $user . '&amp;posts&amp;0',
    ...
    function getRssAll($sort)
    {
        ...
        while (list (, $r) = each($a)) {
            ...
            $items .= $this->_rssItem($r['topic_title'], sprintf($gConf['rewrite']['topic'], $r['topic_uri']),
                $ui[$r['user']]['profile_title'] . ': ' . $td, $r['when']);
        }
...
EOF;
    ...
    function _getUserInfoReadyArray($user, $bWrapWithCdata = true)
    {
        ...
        return array(
            'avatar'   => $aa['avatar'],
            'avatar64' => $aa['avatar64'],
            'url'      => $aa['profile_url'],
            'title'    => $aa['profile_title'],
            'onclick'  => $aa['profile_onclick'],
            'role'     => $aa['role']
        );
    }
