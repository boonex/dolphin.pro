...
EOS;
    ...
    function editCategorySubmit($cat_id, $cat_name, $cat_order, $cat_expanded)
    {
        ...
        $db = new DbAdmin ();
        if ($cat_id) {
            return $db->editCategory((int)$cat_id, $cat_name, (int)$cat_order,
                $cat_expanded ? 1 : 0) ? '<ret>1</ret>' : '<ret>0</ret>';
        } else {
            global $f;
            $cat_uri = $f->uriGenerate($cat_name, TF_FORUM_CAT, 'cat_uri');

            return $db->insertCategory($cat_name, $cat_uri, (int)$cat_order,
                $cat_expanded ? 1 : 0) ? '<ret>1</ret>' : '<ret>0</ret>';
        }
    }
...
OES;
    ...
    function editFormSubmit($cat_id, $forum_id, $title, $desc, $type, $order)
    {
        ...
        if ($forum_id > 0) {
            ...
            return $db->insertForum((int)$cat_id, $title, $desc, $type, $forum_uri,
                (int)$order) ? '<ret>1</ret>' : '<ret>0</ret>';
        }
    ...
    {
        return $this->getXxxPostsXML($wp, 'getReportedPosts', '[L[Reported Posts]]',
