 ...
 */
class BxSoundsSiteMapsSounds extends BxDolSiteMaps
{
    protected $_oModule;
    ...
    protected function __construct($aSystem)
    {
        ...
        $this->_aQueryParts = array(
            ...
            // date field type
            'table'           => "`RayMp3Files` AS `e`",
            // table name
            'join'            => " INNER JOIN `sys_albums_objects` AS `o` ON (`o`.`id_object` = `e`.`ID`)
                        INNER JOIN `sys_albums` AS `a` ON (`a`.`Type` = 'bx_sounds' AND `a`.`Status` = 'active' AND `a`.`AllowAlbumView` = '" . BX_DOL_PG_ALL . "' AND `a`.`ID` = `o`.`id_album`)",
            // join SQL part
