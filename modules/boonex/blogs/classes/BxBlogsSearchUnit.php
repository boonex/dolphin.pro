...
class BxBlogsSearchUnit extends BxTemplSearchResultText
{
    ...
    var $aCurrent = array(
        ...
        'restriction'  => array(
            ...
            'category_uri'   => array('value'     => '',
                                      'field'     => 'Categories',
                                      'operator'  => 'against',
                                      'paramName' => 'uri'
            ),
            'allow_view'     => array('value'    => '',
                                      'field'    => 'allowView',
                                      'operator' => 'in',
                                      'table'    => 'bx_blogs_posts'
            ),
    ...
    function displaySearchBox($sCode, $sPaginate = '')
    {
        $sCode = DesignBoxContent(_t($this->aCurrent['title']),
            '<div class="bx-def-bc-padding">' . $sCode . '<div class="clear_both"></div></div>' . $sPaginate, 1);
    ...
    function displaySearchUnit($aResSQL)
    {
        ...
        if (!$bPossibleToView) {
            if ($this->sMobileWrapper) {
                return $this->_wrapMobileUnit($oMain->_oTemplate->parseHtmlByTemplateName('browse_unit_private_mobile',
                    array()), $iPostID, $oMain);
            } else {
        ...
        if (count($aCategories) > 0) {
            foreach ($aCategories as $iKey => $sCatValue) {
                $sCatLink              = $this->getCurrentUrl('category', title2uri($sCatValue), title2uri($sCatValue),
                    array('ownerId' => $aResSQL['ownerId'], 'blogOwnerName' => $sAuthorUsername));
...
EOF;
        ...
        if ($this->sMobileWrapper) {
            return $this->_wrapMobileUnit($oMain->_oTemplate->parseHtmlByTemplateName('blogpost_unit_mobile',
                $aUnitReplace), $iPostID, $oMain);
        } else {
    ...
    function showPagination3($bAdmin = false)
    {
        ...
        $sPgnAdd   = false === strpos($this->aCurrent['paginate']['page_url'],
            '{page}') ? 'per_page={per_page}&page={page}' : '';
        $oPaginate = new BxDolPaginate(array(
