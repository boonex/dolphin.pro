...
class BxPhotosSiteMapsPhotos extends BxDolSiteMaps
{
    ...
    protected function __construct($aSystem)
    {
        ...
        $this->_aQueryParts = array(
            'fields'          => "`e`.`ID`, `e`.`Uri`, `e`.`Date`",
            // fields list
            'field_date'      => "Date",
            // date field name
            'field_date_type' => "timestamp",
            // date field type
            'table'           => "`bx_photos_main` AS `e`",
            // table name
            'join'            => " INNER JOIN `sys_albums_objects` AS `o` ON (`o`.`id_object` = `e`.`ID`)
                        INNER JOIN `sys_albums` AS `a` ON (`a`.`Type` = 'bx_photos' AND `a`.`Status` = 'active' AND `a`.`AllowAlbumView` = '" . BX_DOL_PG_ALL . "' AND `a`.`ID` = `o`.`id_album`)",
            // join SQL part
