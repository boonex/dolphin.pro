 ...
 */
class BxPhotosSiteMapsPhotos extends BxDolSiteMaps
{
    protected $_oModule;
    ...
    protected function __construct($aSystem)
    {
        ...
        $this->_aQueryParts = array (
            ...
            'table' => "`bx_photos_main` AS `e`", // table name
            'join' => " INNER JOIN `sys_albums_objects` AS `o` ON (`o`.`id_object` = `e`.`ID`)
                        INNER JOIN `sys_albums` AS `a` ON (`a`.`Type` = 'bx_photos' AND `a`.`Status` = 'active' AND `a`.`AllowAlbumView` = '" . BX_DOL_PG_ALL . "' AND `a`.`ID` = `o`.`id_album`)", // join SQL part
