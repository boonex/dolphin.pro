...
class BxPhotosUploader extends BxDolFilesUploader
{
    ...
    {
        $sCustomRecorderObject = getApplicationContent('photo', 'shooter',
            array('id' => $this->_getAuthorId(), 'password' => $this->_getAuthorPassword(), 'extra' => ''), true);
    ...
    function serviceAcceptFileInfo()
    {
        ...
        switch ($_POST['type']) {
            ...
            case 'record':
                global $sFilesPath;
                $iPhotoID = (int)$this->performPhotoUpload($sFilesPath . $iAuthorId . IMAGE_EXTENSION, array(), false,
                    false);
            ...
            if (file_exists($sTempFileName) && filesize($sTempFileName) > 0) {
                ...
                if ($iChangingPhotoID == 0) {
                    if (is_array($aFileInfo) && count($aFileInfo) > 0) {
                        ...
                        $iLastID = $this->oModule->_oDb->insertData(array(
                            'medProfId'  => $iAuthorId,
                            'medExt'     => $sExtDb,
                            ...
                            'medTags'    => '',
                            'Categories' => PROFILE_PHOTO_CATEGORY,
                            'medSize'    => $sDimension,
                            'Approved'   => $sStatus,
                            'medDate'    => $sCurTime
                        ));
                ...
                foreach ($this->oModule->_oConfig->aFilesConfig as $sKey => $aValue) {
                    ...
                    $iRes         = imageResize($sFile, $sNewFilePath, $iWidth, $iHeight, true,
                        isset($aValue['square']) && $aValue['square']);
                    if ($iRes != 0) {
                        return false;
                    } //resizing was failed
    ...
    {
        $sMedUri = uriGenerate($aFileInfo['medTitle'], $this->oModule->_oDb->sFileTable,
            $this->oModule->_oDb->aFileFields['medUri']);

        return $this->oModule->_oDb->updateData($iMediaID, array(
            'medTitle' => $aFileInfo['medTitle'],
            'medUri'   => $sMedUri,
            'medDesc'  => $aFileInfo['medDesc'],
            'medDate'  => time()
        ));
    }
}
