...
class BxPhotosModule extends BxDolFilesModule
{
    ...
    function actionGetImage($sParamValue, $sParamValue1)
    {
        ...
        if ($iId > 0) {
            ...
            $sPath = $this->_oConfig->getFilesPath() . $iId . str_replace('{ext}', $sExt,
                    $this->_oConfig->aFilePostfix[$sParamValue]);
            $sAdd  = '';
            if ($this->iHeaderCacheTime > 0) {
                $iLastModTime = filemtime($sPath);
                $sAdd         = ", max-age={$this->iHeaderCacheTime}, Last-Modified: " . gmdate("D, d M Y H:i:s",
                        $iLastModTime) . " GMT";
            }
    ...
    function actionCropPerform($iPhotoID)
    {
        ...
        $sSrcFileName = $this->_oConfig->getFilesPath() . $aInfo['medID'] . str_replace('{ext}', $aInfo['medExt'],
                $this->_oConfig->aFilesConfig['original']['postfix']);
        ...
        $aInfo['Categories']        = preg_split('/[' . CATEGORIES_DIVIDER . ']/', $aInfo['Categories'], 0,
            PREG_SPLIT_NO_EMPTY);
        bx_import('Uploader', $this->_aModule);
        $sClassName = $this->_oConfig->getClassPrefix() . 'Uploader';
        $oUploader  = new $sClassName();
        $a          = $oUploader->performUpload($sTmpFileName, pathinfo($sSrcFileName, PATHINFO_BASENAME), $aInfo,
            false);
        @unlink($sTmpFileName);
    ...
    function actionCrop($iPhotoID)
    {
        ...
        $GLOBALS['oTopMenu']->setCustomSubHeader(_t('_sys_album_x_photo_x', $aInfo['albumCaption'],
            $aInfo['medTitle']));
    ...
    function serviceGetBlockFavorited($iBlockId)
    {
        ...
        bx_import('Search', $this->_aModule);
        $oMedia = new BxPhotosSearch();
        $oMedia->clearFilters(array('activeStatus', 'allow_view', 'album_status', 'albumType', 'ownerStatus'),
