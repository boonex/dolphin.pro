...
class BxPhotosSearch extends BxTemplSearchResultSharedMedia
{
    function __construct($sParamName = '', $sParamValue = '', $sParamValue1 = '', $sParamValue2 = '')
    {
        ...
        switch ($sParamName) {
            case 'calendar':
                $this->aCurrent['restriction']['calendar-min'] = array(
                    'value'          => "UNIX_TIMESTAMP('{$sParamValue}-{$sParamValue1}-{$sParamValue2} 00:00:00')",
                    'field'          => 'Date',
                    'operator'       => '>=',
                    'no_quote_value' => true
                );
                $this->aCurrent['restriction']['calendar-max'] = array(
                    'value'          => "UNIX_TIMESTAMP('{$sParamValue}-{$sParamValue1}-{$sParamValue2} 23:59:59')",
                    'field'          => 'Date',
                    'operator'       => '<=',
                    'no_quote_value' => true
                );
                $this->aCurrent['title']                       = _t('_bx_photos_caption_browse_by_day') . sprintf("%04u-%02u-%02u",
    ...
    function getAlbumCovers($iAlbumId, $aParams = array())
    {
        ...
        $aPics    = $this->oModule->oAlbums->getAlbumCoverFiles($iAlbumId,
            array('table' => $this->aCurrent['table'], 'field' => 'ID', 'fields_list' => array('Uri', 'Hash')),
    ...
    function _getAlbumUnitItem($iIndex, $aPicture, $aParams = array())
    {
        ...
        if (empty($aPicture)) {
            ...
            if (!empty($aPicture['Uri'])) {
                $aResult['bx_if:exist']['content']['url'] = $this->getCurrentUrl('file', $aPicture['id_object'],
                    $aPicture['Uri']);
            }
    ...
    function _getImageFullUrl($aImageInfo, $sType = 'thumb')
    {
        ...
        $sImageUrl = !empty($aImageInfo['id']) && extFileExists($this->oModule->_oConfig->getFilesPath() . $sName) ? $this->getImgUrl($aImageInfo['Hash'],
            $sType) : '';
    ...
    {
        $sCaption = str_replace('{nickname}', getUsername($iProfileId),
            $this->oModule->_oConfig->getGlParam('profile_album_name'));
    ...
    function serviceProfilePhotoBlock($aParams)
    {
        ...
        if ((int)$aParams['PID'] == getLoggedId()) {
            $aParams['LinkUnitTo'] = $this->getCurrentUrl('manageProfilePhoto', 0,
                    uriFilter($sCaption)) . '/owner/' . $sOwner;
        }
    ...
    {
        $sCaption   = str_replace('{nickname}', getUsername($aParams['PID']),
            $this->oModule->_oConfig->getGlParam('profile_album_name'));
        $sUri       = uriFilter($sCaption);
        $oAlbum     = new BxDolAlbums('bx_photos');
    ...
    function getPhotoBlock($aParams = array())
    {
        ...
        if ($iCnt) {
            ...
            bx_import('BxDolAlbums');
            $sCaption = BxDolAlbums::getAbumUri($this->oModule->_oConfig->getGlParam('profile_album_name'),
    ...
    function getSwitcherUnit(&$aData, $aShowParams = array(), $aAddElems = array())
    {
        ...
        $aUnit       = array(
            ...
            'bx_if:rate'  => array(
                ...
                'content'   => array(
                    'rate' => $this->oRate && $this->oRate->isEnabled() ? $this->oRate->getJustVotingElement(1,
                        $aData['id'], $aData['Rate']) : $this->oRate->getJustVotingElement(0, 0, $aData['Rate'])
                )
            ),
            'bx_if:date'  => array(
