...
class BxFilesModule extends BxDolFilesModule
{
    ...
    function actionGetFile($sFileName)
    {
        ...
        $aInfo   = $this->_oDb->getFileInfo(array('fileId' => (int)$iFileId), false,
            array('medID', 'medProfId', 'medExt', 'medDate', 'Type', 'medUri'));
    ...
    function actionAlbumsViewMy($sParamValue = '', $sParamValue1 = '', $sParamValue2 = '', $sParamValue3 = '')
    {
        ...
        if ($bNotAllowView) {
            ...
            $this->_oTemplate->pageCode($this->aPageTmpl,
                array('page_main_code' => DesignBoxContent($sKey, MsgBox($sKey), 1)), '', '', false);
        ...
        switch ($sParamName) {
            ...
            case 'browse':
                if ('owner' == $sParamValue) {
                    ...
                    $this->aPageTmpl['header'] = _t('_' . $this->_oConfig->getMainPrefix() . '_browse_by_owner',
                        $sParamValue1);
                }
            ...
            default:
                ...
                $oPage = new $sClassName($this,
                    array($sParamName, $sParamValue, $sParamValue1, $sParamValue2, $sParamValue3));
    ...
    function actionBrowse(
        ...
        if ($sParamName == 'album' && $sParamValue1 == 'owner') {
            ...
            ), array('ID', 'Caption', 'Owner', 'AllowAlbumView', 'Description'));
            if (empty($aAlbumInfo)) {
                $this->_oTemplate->displayPageNotFound();
                exit;
            } else {
                if ($aAlbumInfo['Owner'] == $this->_iProfileId && $sParamValue2 === getUsername($this->_iProfileId)) {
                    ...
                    return;
                } elseif (!empty($aAlbumInfo['AllowAlbumView']) && !$this->oAlbumPrivacy->check('album_view',
                        $aAlbumInfo['ID'], $this->_iProfileId)
                ) {
                    $sKey                      = _t('_' . $this->_oConfig->getMainPrefix() . '_access_denied');
    ...
    function actionEdit($iFileId)
    {
        ...
        if (!$this->isAllowedEdit($aInfo)) {
            ...
            $aAllowDownload          = $this->oPrivacy->getGroupChooser($this->_iProfileId, $this->_oConfig->getUri(),
                'download');
    ...
    function getInstanceUploadFormArray(
        ...
        return parent::getInstanceUploadFormArray($aAlbums, $aPrivFieldView, _t($sLangPref . '_album'),
            _t($sLangPref . '_album_title'), _t($sLangPref . '_albums_add_new'));
    }
