...
class BxFilesUploader extends BxDolFilesUploader
{
    ...
    function serviceAcceptFileInfo()
    {
        ...
        if ($iFileID && $iAuthorId) {
            ...
            if ($this->initFile($iFileID, $sTitle, $sCategories, $sTags, $sDescription,
                array('AllowDownload' => $iAllowDownload))
            ) {
    ...
    function performUpload(
        ...
        if (!$iFileSize || $iFileSize > $this->iMaxFilesize) {
            return array(
                'error' => _t('_' . $this->oModule->_oConfig->getMainPrefix() . '_size_error',
                    _t_format_size($this->iMaxFilesize))
            );
        }

        $this->sTempFilename = pathinfo($sRealFilename, PATHINFO_FILENAME);
        if (!($iMID = $this->performFileUpload($sFilePath, array(), $isMoveUploadedFile, $sRealFilename,
            isset($aExtraParams['file_type']) ? $aExtraParams['file_type'] : ''))
        ) {
        ...
        if (($isMoveUploadedFile && is_uploaded_file($sTmpFile)) || !$isMoveUploadedFile) {
            ...
            if (file_exists($sTempFileName)) {
                ...
                if (is_array($aFileInfo) && count($aFileInfo) > 0) {
                    ...
                    $iLastID              = $this->insertSharedMediaToDb($sExtension, $aFileInfo, $this->_iOwnerId,
                        array(
                    ...
                    $this->addObjectToAlbum($this->oModule->oAlbums, $_POST['extra_param_album'], $iLastID,
                        $bAutoActivate);
    ...
    function getUploadFormArray(&$aForm, $aAddObjects = array())
    {
        ...
        $aForm['AllowView'] = $this->oModule->oPrivacy->getGroupChooser($this->_iOwnerId,
            $this->oModule->_oConfig->getUri(), 'download');
