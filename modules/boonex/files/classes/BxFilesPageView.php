...
class BxFilesPageView extends BxDolPageView
{
    ...
    function getBlockCode_ActionList()
    {
        ...
        if (BxDolRequest::serviceExists('wall', 'get_repost_js_click')) {
            ...
            $aReplacement['repostScript'] = BxDolService::call('wall', 'get_repost_js_click',
                array($this->iProfileId, $sMainPrefix, 'add', (int)$this->aFileInfo['medID']));
        }
    ...
    function getBlockCode_LastAlbums()
    {
        ...
        return $this->oSearch->getAlbumsBlock(array('owner' => $this->aFileInfo['medProfId']), array(),
            array('paginate_url' => $sPaginateUrl, 'simple_paginate_url' => $sSimpleUrl));
    }

    function getBlockCode_RelatedFiles()
    {
        $this->oSearch->clearFilters(array('activeStatus', 'albumType', 'allow_view', 'album_status'),
            array('albumsObjects', 'albums'));
        $bLike = getParam('useLikeOperator');
        if ($bLike != 'on') {
            $aRel      = array(
                $this->aFileInfo['medTitle'],
                $this->aFileInfo['medDesc'],
                $this->aFileInfo['medTags'],
                $this->aFileInfo['Categories']
            );
    ...
    function getBlockCode_ViewFile()
    {
        ...
        if ($this->aFileInfo['prevItem'] > 0) {
            $aPrev = $this->oDb->getFileInfo(array('fileId' => $this->aFileInfo['prevItem']), true,
                array('medUri', 'medTitle'));
        }
        if ($this->aFileInfo['nextItem'] > 0) {
            $aNext = $this->oDb->getFileInfo(array('fileId' => $this->aFileInfo['nextItem']), true,
                array('medUri', 'medTitle'));
        }
        //icon
        ...
        $aUnit = array(
            ...
            'rate'            => $oVotingView->isEnabled() ? $oVotingView->getBigVoting(1,
                $this->aFileInfo['Rate']) : '',
