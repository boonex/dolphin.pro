...
class BxPollSearch extends BxTemplSearchResultText
{
        ...
        */
        ...
        $this->aCurrent = array(
            ...
            'restriction' => array(
                ...
                'calendar-min'   => array('value'          => "",
                                          'field'          => 'poll_date',
                                          'operator'       => '>=',
                                          'no_quote_value' => true
                ),
                'calendar-max'   => array('value'          => "",
                                          'field'          => 'poll_date',
                                          'operator'       => '<=',
                                          'no_quote_value' => true
                ),
    ...
    function displayResultBlock()
    {
        ...
        return !empty($sCode) ? $this->oPollObject->_oTemplate->parseHtmlByName('default_margin.html',
    ...
    function displaySearchBox($sCode, $sPaginate = '')
    {
        ...
        if (isset($this->aCurrent['rss']) && $this->aCurrent['rss']['link']) {
            $aCaptionMenu = '<div class="dbTopMenu"><div class="notActive notActiveIcon" style="background-image:url(' . getTemplateIcon('rss.png') . ')"><a target="_blank" class="top_members_menu" href="' . $this->aCurrent['rss']['link'] . (false === strpos($this->aCurrent['rss']['link'],
                    '?') ? '?' : '&') . 'rss=1">' . _t('RSS') . '</a></div></div>';
        }
