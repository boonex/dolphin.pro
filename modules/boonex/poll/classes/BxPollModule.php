...
class BxPollModule extends BxDolModule
{
    ...
    function getOwnerBlock($iPollOwner, $aPollInfo)
    {
        ...
        $aTemplateKeys = array(
            ...
            'tags'            => getLinkSet($aPollInfo['poll_tags'], $this->getModulePath() . '&action=tag&tag=',
                BX_DOL_TAGS_DIVIDER),
            'fields'          => null,
            'cats'            => getLinkSet($aPollInfo['poll_categories'],
                $this->getModulePath() . '&action=category&category=', CATEGORIES_DIVIDER),
        );
    ...
    function genToggleElements()
    {
        ...
        foreach ($aToggleItems as $sKey => $sValue) {
            $aItems[$sValue] = array(
                'href'   => $this->sPathToModule . '&mode=' . $sKey,
                'active' => $this->aPollSettings['mode'] == $sKey ? 1 : 0
            );
        }
    ...
    function getManagePollsPage($bApproval = true)
    {
        ...
        if ($bApproval) {
            ...
            $sOutputCode                                                     = $this->showSearchResult(null,
                $sExtraParam, 0, true, 0, false);
        } else {
    ...
    function showAdminActionsPanel(
        $sWrapperId,
        $aButtons,
        $sCheckboxName = 'entry',
        $bSelectAll = true,
    ...
    function getPollBlock(&$aPollInfo, $bAjaxQuery = false, $bViewMode = false, $bAdminMode = false)
    {
        ...
        $aTemplateKeys = array(
            ...
            'bx_if:manage_page' => array(
                'condition' => $bAdminMode || (in_array($this->aPollSettings['mode'], array(
                            'manage',
                            'pending'
                        )) && $this->aPollSettings['member_id'] && $this->aPollSettings['member_id'] == $aPollInfo['id_profile']),
                'content'   => array(),
            ),
    ...
    function proccesData()
    {
        ...
        if (isset($_POST['question'])
            ...
            if ($_GET['mode'] == 'add' && $sPollAnsers) {
                ...
                if ($_GET['mode'] == 'edit_poll' && $sPollAnsers) {
            ...
            'inputs' => array(
                'question' => array(
                    ...
                    'checker'  => array(
                        'func'   => 'length',
                        'params' => array(
                            $this->aPollSettings['question_min_length'],
                            $this->aPollSettings['question_max_length']
                        ),
                ...
                'answers' => array(
                    ...
                    'checker' => array(
                        'func'   => 'answers',
                        'params' => array(
                            $this->aPollSettings['answer_min_length'],
                            $this->aPollSettings['answer_max_length']
                        ),
    ...
    function getEditForm()
    {
        if ($this->aPollSettings['edit_poll_id']) {
            ...
            if ($this->aPollSettings['admin_mode']
                ...
                $aForm['inputs']['category']          = $oCategories->getGroupChooser('bx_poll',
                    $this->aPollSettings['member_id'], true, $aPollInfo['poll_categories']);
        ...
        if ((int)$iVoteNumber >= 0) {
            ...
            if (!isset($_COOKIE['profile_polls_question_' . $iPollID])) {
                if ($this->_oDb->setVotes((int)$iPollID, $sVotes, $iPoll_total_votes)) {
                    ...
                    // create system event
                    $oZ = new BxDolAlerts('bx_poll', 'answered', $aPoll[0]['id_profile'],
                        $this->aPollSettings['member_id'], array('poll_id' => $iPollID, 'vote' => $iVoteNumber));
                    $oZ->alert();
                }
            }
        }
    ...
    function actionAdministration()
    {
        ...
        $aMenu = array(
            'bx_poll_main'     => array(
                'title' => _t('_bx_poll_main'),
                'href'  => $this->sPathToModule . 'administration&action=main'
            ),
            'bx_poll_settings' => array(
                'title' => _t('_bx_poll_settings'),
                'href'  => $this->sPathToModule . 'administration&action=settings'
            ),
        );
    ...
    function showSearchResult(
        ...
        return ($bUseDesignBox && $sOutputCode) ? DesignBoxContent($sBlockCaption, $sOutputCode, 1, '',
            $sPaginate) : $sOutputCode . $sPaginate;
    }
    ...
    function serviceGetPolls($sAction, $iProfileId = 0)
    {
        ...
        switch ($sAction) {
            ...
            case 'get_polls' :
                ...
                $sOutputCode                                                   = $this->searchAll($iRowsLimit, 1, true,
                    '_bx_polls_public', false);
                break;
        }
    ...
    function serviceGetWallPost($aEvent)
    {
        ...
        if (empty($aProfile) || (int)$aItem['poll_approval'] != 1 || !$this->oPrivacy->check('view',
                (int)$aEvent['object_id'], getLoggedId())
        ) {
            return '';
        }
    ...
    function serviceGetWallPostOutline($aEvent)
    {
        ...
        return array(
            'content' => $sCss . $this->_oTemplate->parseHtmlByName('modules/boonex/wall/|outline_item_text.html',
                    array(
                        'mod_prefix'         => $sPrefix,
                        'mod_icon'           => 'tasks',
                        'user_name'          => $sOwner,
                        ...
                        'item_comments'      => (int)$aItem['poll_comments_count'] > 0 ? _t('_wall_n_comments',
                            $aItem['poll_comments_count']) : _t('_wall_no_comments'),
    ...
    function _getWallContent($aItem, $bAsArray = false)
    {
        ...
        return $bAsArray ? $aResult : $this->_oTemplate->parseHtmlByName('poll_answers.html',
            array('bx_repeat:items' => $aResult));
    }
    ...
    function serviceGetSpyPost($sAction, $iObjectId = 0, $iSenderId = 0, $aExtraParams = array())
    {
        ...
        switch ($sAction) {
            ...
            case 'answered' :
                $aRet = $this->_getSpyArray($aExtraParams['poll_id'], $iSenderId, '_bx_poll_answered',
                    'content_activity', $iObjectId);
                break;
