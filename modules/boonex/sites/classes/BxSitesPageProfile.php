...
class BxSitesPageProfile extends BxDolPageView
{
    ...
    function getBlockCode_Administration()
    {
        ...
        $aMenu = array(
            ...
            _t('_bx_sites_block_submenu_add_site')      => array(
                'href'   => $sBaseUrl . '/add',
                'active' => $this->_sSubMenu == 'add'
            ),
            _t('_bx_sites_block_submenu_manage_sites')  => array(
                'href'   => $sBaseUrl . '/manage',
                'active' => $this->_sSubMenu == 'manage'
            ),
            _t('_bx_sites_block_submenu_pending_sites') => array('href'   => $sBaseUrl . '/pending',
                                                                 'active' => $this->_sSubMenu == 'pending'
            ),
        );
    ...
    function getBlockCode_Owner()
    {
        ...
        $oSearchResult = new BxSitesSearchResult('user',
    ...
    function getBlockCode_Main()
    {
        ...
        if ($iPending) {
            $sPending = ($iActive ? ', ' : '') . sprintf(_t('_bx_sites_msg_you_have_pending_sites'),
                    $sBaseUrl . '/pending', $iPending);
        }
        ...
        if ($this->_oSites->isAllowedAdd()) {
            $aVars['msg'] .= (strlen($aVars['msg']) ? ' ' : '') . sprintf(_t('_bx_sites_msg_add_more_sites'),
                    $sBaseUrl . '/add');
        }
