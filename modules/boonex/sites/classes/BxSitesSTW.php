...
define('INSIDE_PAGES',
    getParam('bx_sites_inside_pages') == 'on' ? true : false); // set to true if inside capturing should be allowed
    ...
    if ($iForceUpdate || _cacheFileExpired($sFile)) {
        ...
        if (_checkLimitReached(THUMBNAIL_DIR . QUOTA_IMAGE)) {
            ...
            if (_checkLimitReached(THUMBNAIL_DIR . BANDWIDTH_IMAGE)) {
                ...
                if (_checkLimitReached(THUMBNAIL_DIR . NO_RESPONSE_IMAGE)) {
...
function _checkWebsiteThumbnailCaptured($aArgs)
{
    ...
    if ($sRemoteData != "") {
        ...
        if ($aResponse['exists'] && $aResponse['thumbnail'] != '') {
            ...
            if ($aResponse['stw_bandwidth_remaining'] == 0 && !$aResponse['locked'] && !$aResponse['invalid'] && !$aResponse['exists']) {
                ...
                if ($aResponse['stw_quota_remaining'] == 0 && !$aResponse['locked'] && !$aResponse['invalid'] && !$aResponse['exists']) {
...
function _getAccXMLResponse($sResponse)
{
    if (extension_loaded('simplexml')) { // If simplexml is available, we can do more stuff!
        ...
        $aResponse['stw_custom_resolution'] = _getLegacyResponse('Custom_Resolution',
            $sRemoteData); // Custom Resolution
