...
class BxEventsSearchResult extends BxDolTwigSearchResult
{
        ...
        switch ($sMode) {
            ...
            case 'pending':
                if (false !== bx_get('bx_events_filter')) {
                    $this->aCurrent['restriction']['keyword'] = array('value'    => process_db_input(bx_get('bx_events_filter'),
                        BX_TAGS_STRIP),
                                                                      'field'    => '',
                                                                      'operator' => 'against'
                    );
            ...
            case 'search':
                if ($sValue) {
                    $this->aCurrent['restriction']['keyword'] = array('value'    => $sValue,
                                                                      'field'    => '',
                                                                      'operator' => 'against'
                    );
                }
                ...
                if ($sValue2) {
                    if (is_array($sValue2)) {
                        $this->aCurrent['restriction']['country'] = array('value'    => $sValue2,
                                                                          'field'    => 'Country',
                                                                          'operator' => 'in'
                        );
                    } else {
                ...
                $this->aCurrent['title'] = _t('_bx_events_caption_search_results') . ' ' . (is_array($sValue2) ? implode(', ',
            ...
            case 'user':
                ...
                $this->aCurrent['title'] = _t('_bx_events_caption_browse_by_author',
                    $iProfileId ? getNickName($iProfileId) : $sValue);
            ...
            case 'joined':
                ...
                $iProfileId              = getID($sValue);
                $this->aCurrent['title'] = _t('_bx_events_caption_browse_by_author_joined_events',
                    $iProfileId ? getNickName($iProfileId) : $sValue);
            ...
            case 'category':
                ...
                $this->aCurrent['title']                                 = _t('_bx_events_caption_browse_by_category',
                    $sValue);
                break;
            ...
            case 'upcoming':
                $this->aCurrent['restriction']['upcoming'] = array('value'    => time(),
                                                                   'field'    => 'EventEnd',
                                                                   'operator' => '>'
                );
            ...
            case 'past':
                $this->aCurrent['restriction']['past'] = array('value'    => time(),
                                                               'field'    => 'EventEnd',
                                                               'operator' => '<'
                );
            ...
            case 'featured':
                $this->aCurrent['restriction']['featured'] = array('value'    => 1,
                                                                   'field'    => 'Featured',
                                                                   'operator' => '='
                );
            ...
            case 'calendar':
                $this->aCurrent['restriction']['calendar-min'] = array('value'          => "UNIX_TIMESTAMP('{$sValue}-{$sValue2}-{$sValue3} 00:00:00')",
                                                                       'field'          => 'EventEnd',
                                                                       'operator'       => '>=',
                                                                       'no_quote_value' => true
                );
                $this->aCurrent['restriction']['calendar-max'] = array('value'          => "UNIX_TIMESTAMP('{$sValue}-{$sValue2}-{$sValue3} 23:59:59')",
                                                                       'field'          => 'EventStart',
                                                                       'operator'       => '<=',
                                                                       'no_quote_value' => true
                );
    ...
    function displayResultBlock()
    {
        ...
        if ($s) {
            $oMain = $this->getMain();
            $GLOBALS['oSysTemplate']->addDynamicLocation($oMain->_oConfig->getHomePath(),
                $oMain->_oConfig->getHomeUrl());
