    ...
    protected function __construct($aSystem)
    {
        ...
        $this->_aTables = array(
            ...
            'bx_news_views_track' => array(
                'query' => "SELECT `t`.`id`, 0, 0, `t`.`ts` FROM `bx_news_views_track` AS `t` INNER JOIN `bx_news_entries` AS `m` ON (`m`.`id` = `t`.`id`) WHERE `m`.`author_id` = {profile_id}"), // anonymize some data
