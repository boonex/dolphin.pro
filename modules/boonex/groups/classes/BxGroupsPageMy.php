...
class BxGroupsPageMy extends BxDolPageView
{
    ...
    function __construct(&$oMain, &$aProfile)
    {
        ...
        parent::__construct('bx_groups_my');
    }
    ...
    function getBlockCode_Owner()
    {
        ...
        $aMenu    = array(
            _t('_bx_groups_block_submenu_main')           => array(
                'href'   => $sBaseUrl,
                'active' => !bx_get('bx_groups_filter')
            ),
            _t('_bx_groups_block_submenu_add_group')      => array(
                'href'   => $sBaseUrl . '&bx_groups_filter=add_group',
                'active' => 'add_group' == bx_get('bx_groups_filter')
            ),
            _t('_bx_groups_block_submenu_manage_groups')  => array(
                'href'   => $sBaseUrl . '&bx_groups_filter=manage_groups',
                'active' => 'manage_groups' == bx_get('bx_groups_filter')
            ),
            _t('_bx_groups_block_submenu_pending_groups') => array('href'   => $sBaseUrl . '&bx_groups_filter=pending_groups',
                                                                   'active' => 'pending_groups' == bx_get('bx_groups_filter')
            ),
        );
    ...
    function getBlockCode_Browse()
    {
        ...
        $o                  = new BxGroupsSearchResult('user',
    ...
    function getBlockCode_Main()
    {
        ...
        $aVars    = array('msg' => '');
        if ($iPending) {
            $aVars['msg'] = sprintf(_t('_bx_groups_msg_you_have_pending_approval_groups'),
                $sBaseUrl . '&bx_groups_filter=pending_groups', $iPending);
        } elseif (!$iActive) {
            $aVars['msg'] = sprintf(_t('_bx_groups_msg_you_have_no_groups'), $sBaseUrl . '&bx_groups_filter=add_group');
        } else {
            $aVars['msg'] = sprintf(_t('_bx_groups_msg_you_have_some_groups'),
                $sBaseUrl . '&bx_groups_filter=manage_groups', $iActive, $sBaseUrl . '&bx_groups_filter=add_group');
        }
