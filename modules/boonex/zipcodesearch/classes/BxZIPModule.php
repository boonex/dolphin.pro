...
class BxZIPModule extends BxDolModule
{
    ...
    function serviceGetSqlParts($mixedCountry, $sZip, $sMetric, $iDistance, &$sJoin, &$aWhere)
    {
        ...
        if (is_array($mixedCountry)) {
            $sCountry = process_db_input($mixedCountry[0],
                BX_TAGS_STRIP); // no reason to search by zipcode in different countries
        } else {
        ...
        if ('Google' == getParam('bx_zip_mode') && $this->_oDb->isModule('wmap')) {
            ...
            do {
                ...
                if ('GB' == $sCountry) {
                    ...
                    $sWhere = " `Country` = '$sCountry' AND (UPPER(SUBSTRING(`Profiles`.`zip`,1,3)) = '" . join("' OR UPPER(SUBSTRING(`Profiles`.`zip`,1,3)) = '",
                            $aZips) . "') ";
                } else {
                    $sWhere = " `Country` = '$sCountry' AND (UPPER(`Profiles`.`zip`) = '" . join("' OR UPPER(`Profiles`.`zip`) = '",
                            $aZips) . "') ";
                }
            } while (0);
