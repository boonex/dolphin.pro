...
class BxWallTemplate extends BxDolModuleTemplate
{
        ...
        switch ($sDisplayType) {
            ...
            case BX_WALL_VIEW_OUTLINE:
                ...
                if ($this->_oModule->_isRepostAllowed($aEvent)) {
                    ...
                    $iObjectId = $this->_oModule->_oConfig->isSystem($aEvent['type'],
                        $aEvent['action']) ? $aEvent['object_id'] : $aEvent['id'];

                    $sRepost = $this->_oModule->serviceGetRepostElementBlock($iOwnerId, $aEvent['type'],
                        $aEvent['action'], $iObjectId, array(
    ...
    function _getCommonData($aEvent)
    {
        ...
        switch ($sEventType) {
            ...
            case BX_WALL_PARSE_TYPE_REPOST:
                ...
                $sMethod = $this->_oConfig->isSystem($aContent['type'],
                    $aContent['action']) ? '_getSystemData' : '_getCommonData';
                ...
                $sTmplName                 = 'repost';
                $aTmplVars['cpt_reposted'] = _t($this->_oModule->getRepostedLanguageKey($aReposted['type'],
                    $aReposted['action'], $aReposted['object_id']));
                break;
        }
    ...
    function _getCommonMedia($sType, $iObject)
    {
        ...
        $aMediaInfo = BxDolService::call($sType, 'get_' . $aConverter[$sType] . '_array', array($iObject, 'browse'),
            'Search');
    ...
    {
        $aEvent = $this->_oDb->getEvents(array('browse'   => 'last',
    ...
    function getUploader($iOwnerId, $sType, $sSubType = '')
    {
        ...
        return $this->parseHtmlByName('uploader.html', array(
            ...
            'uploader'            => BxDolService::call($sModule, 'get_uploader_form', array(
                array(
                    'mode'      => $sSubType,
                    'category'  => 'wall',
    ...
    function displayProfileEdit($aEvent)
    {
        ...
        if ($aOwner['couple'] == 0 && $aOwner['sex'] == 'male') {
            ...
            if ($aOwner['couple'] == 0 && $aOwner['sex'] == 'female') {
    ...
    function getRepostElement($iOwnerId, $sType, $sAction, $iObjectId, $aParams = array())
    {
        ...
        $bDisabled = $this->_oModule->_isRepostAllowed($aReposted) !== true || $this->_oDb->isReposted($aReposted['id'],
        ...
        if (!$bDisabled) {
            ...
            if ($sType == $sCommonPrefix . BX_WALL_PARSE_TYPE_REPOST) {
                ...
                $sOnClick = $this->getRepostJsClick($iOwnerId, $aRepostedContent['type'], $aRepostedContent['action'],
                    $aRepostedContent['object_id']);
            } else {
    ...
    function getRepostCounter($aEvent, $aParams = array())
    {
        ...
        return $this->parseHtmlByName($sTmplCounter, array(
            ...
            'bx_repeat:attrs' => array(
                ...
                array('key'   => 'onclick',
                      'value' => 'javascript:' . $sJsObject . '.toggleByPopup(this, ' . $aEvent['id'] . ')'
                )
            ),
            'content'         => !empty($aEvent['reposts']) && (int)$aEvent['reposts'] > 0 ? _t($sTxtCounter,
                $aEvent['reposts']) : _t($sTxtCounterEmpty, $aEvent['reposts'])
        ));
    }
