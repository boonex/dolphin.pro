...
class BxWallModule extends BxDolModule
{
    ...
    function __construct($aModule)
    {
        ...
        defineMembershipActions(array('timeline repost', 'timeline post comment', 'timeline delete comment'),
            'ACTION_ID_');
    }
    ...
    function actionRepost()
    {
        ...
        $iId = $this->_oDb->insertEvent(array(
            ...
            'title'       => _t($this->getRepostedLanguageKey($aReposted['type'], $aReposted['action'],
                $aReposted['object_id'], true), getNickName($iAuthorId)),
            'description' => ''
        ));
    ...
    function actionGetPosts()
    {
        ...
        $sFilter   = isset($_POST['WallFilter']) && !empty($_POST['WallFilter']) ? process_db_input($_POST['WallFilter'],
            BX_TAGS_STRIP) : BX_WALL_FILTER_ALL;
        $sTimeline = isset($_POST['WallTimeline']) && !empty($_POST['WallTimeline']) ? process_db_input($_POST['WallTimeline'],
            BX_TAGS_STRIP) : '';
        $aModules  = isset($_POST['WallModules']) && !empty($_POST['WallModules']) ? process_db_input($_POST['WallModules'],
            BX_TAGS_STRIP) : array();
    ...
    function serviceViewBlockIndexTimeline(
        $iStart = -1,
        $iPerPage = -1,
        $sFilter = '',
        $sTimeline = '',
        ...
        return array(
            $this->_oTemplate->parseHtmlByName('view.html', $aVariables),
            array(),
            LoadingBox('bx-wall-view-loading'),
            false,
            'getBlockCaptionMenu'
        );
    }
    ...
    function servicePostBlockProfileTimeline($mixed, $sType = 'id')
    {
        ...
        $sContent     = $this->_oTemplate->parseHtmlByName('post.html', array(
            ...
            'class_photo'   => !empty($aTopMenu['mask']['photo']) ? $sClassActive : '',
            'content_photo' => isset($aTopMenu['mask']['photo']) ? $this->_oTemplate->getUploader($this->_iOwnerId,
                'photo') : '',
            'class_sound'   => !empty($aTopMenu['mask']['sound']) ? $sClassActive : '',
            'content_sound' => isset($aTopMenu['mask']['sound']) ? $this->_oTemplate->getUploader($this->_iOwnerId,
                'sound') : '',
            'class_video'   => !empty($aTopMenu['mask']['video']) ? $sClassActive : '',
            'content_video' => isset($aTopMenu['mask']['video']) ? $this->_oTemplate->getUploader($this->_iOwnerId,
                'video') : '',
        ));
    ...
    function _getPostTabs($sType)
    {
        ...
        if (!in_array('text', $aUploadersHidden)) {
            $aMask['text']            = 0;
            $aTabs['wall-ptype-text'] = array(
                'href'    => 'javascript:void(0)',
                'onclick' => 'javascript:' . $sJsObject . '.changePostType(this)',
                'class'   => 'wall-ptype-ctl',
                'icon'    => 'comment-o',
                'title'   => _t('_wall_write')
            );
        }

        if (!in_array('link', $aUploadersHidden)) {
            $aMask['link']            = 0;
            $aTabs['wall-ptype-link'] = array(
                'href'    => 'javascript:void(0)',
                'onclick' => 'javascript:' . $sJsObject . '.changePostType(this)',
                'class'   => 'wall-ptype-ctl',
                'icon'    => 'link',
                'title'   => _t('_wall_share_link')
            );
        }

        if (!in_array('photo', $aUploadersHidden) && $this->_oDb->isModule('photos')) {
            $aMask['photo']            = 0;
            $aTabs['wall-ptype-photo'] = array(
                'href'    => 'javascript:void(0)',
                'onclick' => 'javascript:' . $sJsObject . '.changePostType(this)',
                'class'   => 'wall-ptype-ctl',
                'icon'    => 'picture-o',
                'title'   => _t('_wall_add_photo')
            );
        }

        if (!in_array('sound', $aUploadersHidden) && $this->_oDb->isModule('sounds')) {
            $aMask['sound']            = 0;
            $aTabs['wall-ptype-sound'] = array(
                'href'    => 'javascript:void(0)',
                'onclick' => 'javascript:' . $sJsObject . '.changePostType(this)',
                'class'   => 'wall-ptype-ctl',
                'icon'    => 'music',
                'title'   => _t('_wall_add_sound')
            );
        }

        if (!in_array('video', $aUploadersHidden) && $this->_oDb->isModule('videos')) {
            $aMask['video']            = 0;
            $aTabs['wall-ptype-video'] = array(
                'href'    => 'javascript:void(0)',
                'onclick' => 'javascript:' . $sJsObject . '.changePostType(this)',
                'class'   => 'wall-ptype-ctl',
                'icon'    => 'film',
                'title'   => _t('_wall_add_video')
            );
        }
    ...
    function serviceViewBlockProfileTimeline(
        ...
        $aTopMenu = array(
            'wall-view-all'     => array(
                'href'    => 'javascript:void(0)',
                'onclick' => 'javascript:' . $sJsObject . '.changeFilter(this)',
                'title'   => _t('_wall_view_all'),
                'active'  => 1
            ),
            'wall-view-owner'   => array(
                'href'    => 'javascript:void(0)',
                'onclick' => 'javascript:' . $sJsObject . '.changeFilter(this)',
                'title'   => _t('_wall_view_owner', getNickName($aOwner['id']))
            ),
            'wall-view-other'   => array(
                'href'    => 'javascript:void(0)',
                'onclick' => 'javascript:' . $sJsObject . '.changeFilter(this)',
                'title'   => _t('_wall_view_other')
            ),
            'wall-get-rss'      => array(
                'href'   => BX_DOL_URL_ROOT . $this->_oConfig->getBaseUri() . 'rss/' . $aOwner['username'] . '/',
                'target' => '_blank',
                'title'  => _t('_wall_get_rss')
            ),
            'wall-subscription' => array(
                'href'    => 'javascript:void(0);',
                'onclick' => 'javascript:' . $aButton['script'] . '',
                'title'   => $aButton['title']
            ),
        );
        ...
        return array(
            $oSubscription->getData() . $this->_oTemplate->parseHtmlByName('view.html', $aVariables),
            $aTopMenu,
            LoadingBox('bx-wall-view-loading'),
            false,
            'getBlockCaptionMenu'
    ...
    function serviceViewBlockAccountTimeline(
        $mixed,
        $iStart = -1,
        $iPerPage = -1,
        $sFilter = '',
    ...
    function serviceViewBlockIndexOutline($iStart = -1, $iPerPage = -1, $sFilter = '', $aModules = array())
    {
        ...
        return array(
            $this->_oTemplate->parseHtmlByName('outline.html', $aTmplVars),
            array(),
            LoadingBox('bx-wall-view-loading'),
            true,
            'getBlockCaptionMenu'
    ...
    function serviceUpdateHandlers($sModuleUri = 'all', $bInstall = true)
    {
        ...
        foreach ($aModules as $aModule) {
            if (!BxDolRequest::serviceExists($aModule, 'get_wall_data')) {
                continue;
            }
    ...
    function _processLinkUpload()
    {
        ...
        return array(
            ...
            'content'     => $this->_oTemplate->parseHtmlByName('common_link.html', array(
                ...
                'title'               => $sTitle,
                'url'                 => strpos($sUrl, 'http://') === false && strpos($sUrl,
                    'https://') === false ? 'http://' . $sUrl : $sUrl,
    ...
    {
        if (in_array($aEvent['type'], array('profile', 'friend')) || in_array($aEvent['action'],
                array('commentPost', 'comment_add'))
        ) {
    ...
    function _isCommentDeleteAllowed($aEvent, $bPerform = false)
    {
        ...
        if ((int)$aEvent['owner_id'] == $iUserId || (strpos($aEvent['type'],
                    $sCommonPostPrefix) === 0 && (int)$aEvent['object_id'] == $iUserId)
        ) {
            return true;
        }
