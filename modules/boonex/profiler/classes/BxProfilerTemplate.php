...
class BxProfilerTemplate extends BxDolTwigTemplate
{
    ...
    function _isAjaxRequest()
    {
        ...
        if (!empty($_GET['r']) && (preg_match('/^poll\/set_answer/',
                    $_GET['r']) || preg_match('/^poll\/get_poll_block/',
                    $_GET['r']) || preg_match('/^poll\/get_questions/', $_GET['r']))
        ) {
            return true;
        }
        if (preg_match('/pageBuilder\.php/',
                bx_html_attribute($_SERVER['PHP_SELF'])) && $_REQUEST['action'] == 'load'
        ) {
            return true;
        }
