...
class BxProfiler
{
    ...
    function __construct($iTimeStart)
    {
        ...
        $aCss = array(
            'modules/boonex/profiler/templates/base/css/|profiler.css',
            'modules/boonex/profiler/plugins/jush/|jush.css',
        );
        $aJs  = array(
            'profiler.js',
            'plugins/jquery/|jquery.tablesorter.js',
            'modules/boonex/profiler/plugins/jush/|jush.js',
        );
    ...
    function _plankInjections()
    {
        ...
        $sInjections = count($GLOBALS['bx_profiler']->_aInjections) . ' injection (' . $this->_formatTime($iTimeInjections,
                3) . ')';
    ...
    function _plankSql()
    {
        ...
        $sQueries = count($GLOBALS['bx_profiler']->_aQueries) . ' queries (' . $this->_formatTime($iTimeQueries,
                3) . ')';
    ...
    function _plankModules()
    {
        ...
        $sModulesQueries = count($GLOBALS['bx_profiler']->_aModules) . ' modules queries (' . $this->_formatTime($iTimeModules,
                3) . ')';
    ...
    {
        if (is_string) {
            return "'" . (strlen($mixed) > $this->_sLogMaxArgLength ? substr($mixed, 0,
                    $this->_sLogMaxArgLength) . '...' : $mixed) . "'";
        } else {
            return $mixed;
        }
    }
