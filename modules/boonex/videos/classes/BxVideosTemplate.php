...
class BxVideosTemplate extends BxDolFilesTemplate
{
    ...
    {
        $sOverride = false;
        $oAlert    = new BxDolAlerts($this->_oConfig->getMainPrefix(), 'display_player', $iFileId, getLoggedId(),
            array('extra' => $aExtra, 'override' => &$sOverride));
        $oAlert->alert();
        if ($sOverride) {
            return $sOverride;
        }
        ...
        $iFileId = (int)$iFileId;
        if (empty($aExtra['ext'])) {
            $sPlayer = getApplicationContent('video', 'player',
                array('id' => $iFileId, 'user' => $this->iViewer, 'password' => clear_xss($_COOKIE['memberPassword'])),
                true);
        } else {
            $sPlayer = str_replace("#video#", $aExtra['ext'], YOUTUBE_VIDEO_PLAYER);
            $sPlayer = str_replace("#wmode#", getWMode(), $sPlayer);
            $sPlayer = str_replace("#autoplay#", (getSettingValue("video",
                "autoPlay") == TRUE_VAL && class_exists('BxVideosPageView') ? "&autoplay=1" : ""), $sPlayer);
        }
    ...
    {
        $sOverride = false;
        $oAlert    = new BxDolAlerts($this->_oConfig->getMainPrefix(), 'embed_code', $iFileId, getLoggedId(),
            array('override' => &$sOverride));
        $oAlert->alert();
        if ($sOverride) {
            return $sOverride;
        }
        ...
        switch ($aExtra["source"]) {
            ...
            case "youtube":
                ...
                $sEmbedCode = str_replace("#autoplay#",
                    (getSettingValue("video", "autoPlay") == TRUE_VAL ? "&autoplay=1" : ""), $sEmbedCode);
                break;
            default:
            ...
            $aAdd  = array(
                'embed' => array(
                    ...
                    'value'   => $this->getEmbedCode($aInfo['medID'],
                        array('video' => $aInfo['medExt'], 'source' => $aInfo['medSource'])),
                    'attrs'   => array(
