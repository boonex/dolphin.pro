...
require_once('BxVideosCmtsAlbums.php');
...
class BxVideosPageAlbumView extends BxDolPageView
{
    ...
    {
        parent::__construct('bx_videos_album_view');
        $this->aInfo      = $aInfo;
    ...
    function getBlockCode_Objects($iBlockId)
    {
        if (empty($this->sBrowseCode)) {
            ...
            return DesignBoxContent(_t('_' . $this->oConfig->getMainPrefix() . '_browse_by_album',
                $this->aInfo['Caption']), $aHtml['code'] . $sPaginate, 1);
        } else {
    ...
    function getBlockCode_Comments()
    {
        ...
        $oCmtsView = new BxVideosCmtsAlbums($this->oConfig->getMainPrefix() . '_albums', $this->aInfo['ID']);
