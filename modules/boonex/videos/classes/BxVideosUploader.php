...
class BxVideosUploader extends BxDolFilesUploader
{
    ...
    {
        $sCustomRecorderObject = getApplicationContent('video', 'recorder',
            array('user' => $this->_getAuthorId(), 'password' => $this->_getAuthorPassword(), 'extra' => ''), true);
        ...
        switch ($_POST['type']) {
            case 'embed':
                $iVideoID       = (int)embedVideo($iAuthorId, $_POST['video'], $_POST['duration'],
                    $this->oModule->_oConfig->aFilesConfig);
    ...
    function performUpload($sFilePath, $sRealFilename = '', $aInfo = array(), $isMoveUploadedFile = true)
    {
        ...
        if (!($iMID = uploadVideo(process_db_input($sFilePath), $iOwner, $isMoveUploadedFile, '',
            process_db_input($sRealFilename), $this->oModule->_oConfig->aFilesConfig))
        ) {
        ...
        $this->addObjectToAlbum($this->oModule->oAlbums, !empty($aInfo['album']) ? $aInfo['album'] : $sAlbum, $iMID,
            false, $iOwner, $aAlbumParams);
