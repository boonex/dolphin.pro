...
class BxVideosPageAlbumsMy extends BxDolPageView
{
    ...
    {
        parent::__construct('bx_videos_albums_my');
    ...
    function getBlockCode_my($iBoxId)
    {
        ...
        $this->oSearch->clearFilters(array('activeStatus', 'allow_view', 'album_status'),
            array('albumsObjects', 'albums'));
        ...
        $sCode                           = $this->oSearch->getAlbumList($iPage, $iPerPage,
            array('owner' => $this->iOwnerId, 'show_empty' => true, 'hide_default' => true));
    ...
    function getBlockCode_adminShort()
    {
        if (in_array('adminShort', $this->aCurrentBlocks['blocks'])) {
            $iNumber = $this->oAlbum->getAlbumCount(array(
                'owner'        => $this->iOwnerId,
                'show_empty'   => true,
                'hide_default' => true
            ));
    ...
    function getBlockCode_adminAlbumShort()
    {
        if (in_array('adminAlbumShort', $this->aCurrentBlocks['blocks'])) {
            ...
            $aInfo   = $this->oAlbum->getAlbumInfo(array(
            ...
            if ($aInfo['AllowAlbumView'] == BX_DOL_PG_NOBODY) {
                $aDraw = array(
                    ...
                    'fileStatAdd'   => _t('_sys_album_edit_info',
                        BX_DOL_URL_ROOT . $this->oConfig->getBaseUri() . 'albums/my/edit/' . $this->aAddParams[1] . '/owner/' . $this->aAddParams[3]),
    ...
    function getBlockCode_adminFull($iBoxId)
    {
        if (in_array('adminFull', $this->aCurrentBlocks['blocks'])) {
            return array(
                $this->getAdminPart(array(), array('section' => 'manage', 'page_block_id' => $iBoxId)),
                $this->getTopMenu('manage'),
                array(),
                ''
            );
        }
    ...
    function getBlockCode_add()
    {
        ...
        $aPrivFieldView = $this->oAlbumPrivacy->getGroupChooser($this->iOwnerId, $this->oConfig->getUri(),
            'album_view');
        ...
        if ($oForm->isSubmittedAndValid()) {
            ...
            if ($iNewId > 0) {
                ...
                $sCode   = MsgBox(_t('_' . $this->oConfig->getMainPrefix() . '_album_save_redirect')) . $this->oTemplate->getJsTimeOutRedirect($sUrlAdd,
                        3);
            }
        } else {
            $sCode = $oForm->getCode();
    ...
    function getBlockCode_addObjects($iBoxId)
    {
        ...
        if (!$this->oSearch->oModule->isAllowedAdd()) {
            ...
            if (count($aMenu) > 1) {
                $sSubMenu = $this->oTemplate->getExtraSwitcher($aMenu,
                    '_' . $this->oConfig->getMainPrefix() . '_choose_uploader', $iBoxId);
            }
            $oUploader               = new BxVideosUploader();
            $oUploader->sWorkingFile = $sLink;
            $sCode                   = $this->oTemplate->parseHtmlByName('default_margin.html',
                array('content' => $oUploader->GenMainAddFilesForm(array('album' => $this->aAddParams[1]))));
        }
    ...
    function getBlockCode_manageObjects($iBoxId)
    {
        ...
        return array(
            $this->getAdminObjectPart(array('activeStatus' => 'approved'), true, $iBoxId, true),
            $this->getTopMenu('manage_objects'),
            array(),
            ''
        );
    }
    ...
    function getBlockCode_manageObjectsDisapproved($iBoxId)
    {
        ...
        return array(
            $this->getAdminObjectPart(array('activeStatus' => 'disapproved'), true, $iBoxId),
            $this->getTopMenu('manage_objects_disapproved'),
            array(),
            ''
        );
    }
    ...
    function getBlockCode_manageObjectsNotProcessed($iBoxId)
    {
        ...
        return array(
            $this->getAdminObjectPart(array('activeStatus' => array('pending', 'processing')), false, $iBoxId),
            $this->getTopMenu('manage_objects_not_processed'),
            array(),
            ''
        );
    }
    ...
    function getBlockCode_manageObjectsFailed($iBoxId)
    {
        ...
        return array(
            $this->getAdminObjectPart(array('activeStatus' => 'failed'), true, $iBoxId),
            $this->getTopMenu('manage_objects_failed'),
            array(),
            ''
        );
    }
    ...
    function getAdminObjectPart($aCondition = array(), $bAdminMode = true, $iBoxId = 0, $bShowMove = false)
    {
        ...
        $sManage = '';
        if ($bAdminMode) {
            $aUserAlbums = $this->oAlbum->getAlbumList(array(
                'owner'        => $this->iOwnerId,
                'show_empty'   => true,
                'hide_default' => true
            ), 0, 0, true);
            ...
            if (count($aUserAlbums) > 1 && $bShowMove) {
                ...
                $sMoveToAlbum            = $this->oTemplate->parseHtmlByName('albums_select.html',
                    array('bx_repeat:choose' => $aAlbums));
            }
            $sManage = $this->oSearch->showAdminActionsPanel($this->oSearch->aCurrent['name'] . '_admin_form', $aBtns,
                'entry', true, false, $sMoveToAlbum);
        }
        $sCode     = $this->oSearch->displayResultBlock();
        $iCount    = $this->oSearch->aCurrent['paginate']['totalNum'];
        $sPaginate = '';
        if ($iCount == 0) {
            ...
            if ($iCount > $this->oSearch->aCurrent['paginate']['perPage']) {
                $sViewAllUrl = BX_DOL_URL_ROOT . $this->oConfig->getBaseUri() . 'albums/my/' . implode('/',
                        array_slice($this->aAddParams, 0, 4));
                $sViewAllUrl .= ($this->oConfig->isPermalinkEnabled) ? '?' : '&amp;';
                $sPaginate = $this->oSearch->getBottomMenu($sViewAllUrl . 'per_page=' . $iCount);
                if ($iBoxId > 0) {
