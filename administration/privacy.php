...
{
    $aTopItems = array(
        'adm-pvc-btn-defaults' => array('href'    => 'javascript:void(0)',
                                        'onclick' => 'javascript:onChangeType(this)',
                                        'title'   => _t('_adm_txt_pvc_defaults'),
                                        'active'  => empty($aResults) ? 1 : 0
        ),
        'adm-pvc-btn-settings' => array('href'    => 'javascript:void(0)',
                                        'onclick' => 'javascript:onChangeType(this)',
                                        'title'   => _t('_adm_txt_pvc_settings'),
                                        'active'  => isset($aResults['settings']) ? 1 : 0
        )
    );

    $sResult = $GLOBALS['oAdmTemplate']->parseHtmlByName('privacy.html', array(
        'content_defaults' => _getDefaults(isset($aResults['defaults']) ? $aResults['defaults'] : true,
            empty($aResults)),
...
function _getDefaults($mixedResult, $bActive = false)
{
    ...
    $aGroupIds = $GLOBALS['MySQL']->getColumn("SELECT `id` FROM `sys_privacy_groups` WHERE `owner_id`='0' AND `id` NOT IN('" . implode("','",
            array(BX_DOL_PG_DEFAULT, BX_DOL_PG_HIDDEN)) . "')");
