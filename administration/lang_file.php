        ...
        if (isset($_POST['adm-lang-compile']) && !empty($_POST['langs'])) {
            ...
            if (isset($_POST['adm-lang-delete']) && !empty($_POST['langs'])) {
                ...
                if (isset($_GET['action']) && $_GET['action'] == 'export' && isset($_GET['id'])) {
                    $aLanguage = $GLOBALS['MySQL']->getRow("SELECT `Name`, `Flag`, `Title`, `Direction`, `LanguageCountry` FROM `sys_localization_languages` WHERE `ID`= ? LIMIT 1",
                        [$_GET['id']]);

                    $aContent = array();
                    $aItems   = $GLOBALS['MySQL']->getAll("SELECT `tlk`.`Key` AS `key`, `tls`.`String` AS `string` FROM `sys_localization_keys` AS `tlk` 
              LEFT JOIN `sys_localization_strings` AS `tls` ON `tlk`.`ID`=`tls`.`IDKey` WHERE `tls`.`IDLanguage`= ? ",
                        [$_GET['id']]);
                    ...
                    if (isset($_POST['action']) && $_POST['action'] == 'get_edit_form_language') {
                        echo json_encode(array(
                            'code' => PopupBox('adm-langs-wnd-edit', _t('_adm_box_cpt_lang_edit_language'),
                                _getLanguageCreateForm(true))
                        ));
                        exit;
                    }
                }
...
if (isset($_POST['create_key'])) {
    ...
    $mixedResult = $GLOBALS['MySQL']->query("INSERT INTO `sys_localization_keys`(`IDCategory`, `Key`) VALUES('" . $iCategoryId . "', '" . $sName . "')",
        false);
    if ($mixedResult !== false) {
        ...
        $aResult = $bCompiled ? array('code' => 0, 'message' => '_adm_txt_langs_success_key_save') : array(
            'code'    => 1,
            'message' => '_adm_txt_langs_cannot_compile'
        );
    } else {
...
$sLangRssFeed = 'on' == getParam('feeds_enable') ? DesignBoxAdmin(_t('_adm_box_cpt_lang_files'),
    '<div class="RSSAggrCont" rssid="boonex_unity_lang_files" rssnum="5" member="0">' . $GLOBALS['oFunctions']->loadingBoxInline() . '</div>') : '';
...
{
    $aTopItems = array(
        'adm-langs-btn-keys'         => array(
            'href'    => 'javascript:void(0)',
            'onclick' => 'javascript:onChangeType(this)',
            'title'   => _t('_adm_txt_langs_keys'),
            'active'  => empty($aResults) ? 1 : 0
        ),
        'adm-langs-btn-keys-add'     => array(
            'href'    => 'javascript:void(0)',
            'onclick' => 'javascript:onCreate()',
            'title'   => _t('_adm_txt_langs_add_key'),
            'active'  => 0
        ),
        'adm-langs-btn-langs'        => array(
            'href'    => 'javascript:void(0)',
            'onclick' => 'javascript:onChangeType(this)',
            'title'   => _t('_adm_txt_langs_languages'),
            'active'  => isset($aResults['langs']) ? 1 : 0
        ),
        'adm-langs-btn-langs-add'    => array(
            'href'    => 'javascript:void(0)',
            'onclick' => 'javascript:onChangeType(this)',
            'title'   => _t('_adm_txt_langs_languages_add'),
            'active'  => isset($aResults['langs-add']) ? 1 : 0
        ),
        'adm-langs-btn-langs-import' => array(
            'href'    => 'javascript:void(0)',
            'onclick' => 'javascript:onChangeType(this)',
            'title'   => _t('_adm_txt_langs_languages_import'),
            'active'  => isset($aResults['langs-import']) ? 1 : 0
        ),
        'adm-langs-btn-settings'     => array(
            'href'    => 'javascript:void(0)',
            'onclick' => 'javascript:onChangeType(this)',
            'title'   => _t('_adm_txt_langs_settings'),
...
function _getKeysList($mixedResult, $bActive = false)
{
    ...
    if (isset($_GET[$sFilterName])) {
        ...
        $aKeys = $GLOBALS['MySQL']->getAll("SELECT `tk`.`ID` AS `id`, `tk`.`Key` AS `key`, `tc`.`Name` AS `category` FROM `sys_localization_keys` AS `tk` 
                 ...
                 WHERE `tk`.`Key` LIKE ? COLLATE utf8_general_ci OR `ts`.`String` LIKE ? GROUP BY `tk`.`ID`",
            ["%{$sFilter}%", "%{$sFilter}%"]);
    ...
    $sFilter = BxTemplSearchResult::showAdminFilterPanel(false !== bx_get($sFilterName) ? bx_get($sFilterName) : '',
        'adm-langs-look-for', 'adm-langs-apply', $sFilterName);
...
function _getLanguageCreateForm($mixedResult, $bActive = false)
{
    ...
    foreach ($aLangs as $iId => $sName) {
        $aFormCreate['inputs']['CopyLanguage_SourceLangID']['values'][] = array(
            'key'   => $iId,
            'value' => htmlspecialchars_adv($sName)
        );
    }
...
function PageCodeKeyCreate()
{
    ...
    $sContent = $GLOBALS['oAdmTemplate']->parseHtmlByName('langs_key.html',
        array('type' => 'add', 'content' => $oForm->getCode()));
...
function PageCodeKeyEdit($iId)
{
    ...
    $aStrings   = $GLOBALS['MySQL']->getAllWithKey("SELECT CONCAT('string_for_', `IDLanguage`) AS `key`, `String` AS `value` FROM `sys_localization_strings` WHERE `IDKey`= ?",
        "key", [$iId]);
    $aLanguages = $GLOBALS['MySQL']->getAll("SELECT `ID` AS `id`, `Title` AS `title` FROM `sys_localization_languages`");
    foreach ($aLanguages as $aLanguage) {
    ...
    $oForm    = new BxTemplFormView($aForm);
    $sContent = $GLOBALS['oAdmTemplate']->parseHtmlByName('langs_key.html',
        array('type' => 'edit', 'content' => $oForm->getCode()));
...
function createLanguage(&$aData)
{
    ...
    $aStrings = $MySQL->getAll("SELECT `IDKey`, `String` FROM `sys_localization_strings` WHERE `IDLanguage` = ?",
        [$iSourceId]);
...
function importLanguage(&$aData, &$aFiles)
{
    ...
    if (!file_exists($aFiles['ImportLanguage_File']['tmp_name']) || !move_uploaded_file($aFiles['ImportLanguage_File']['tmp_name'],
            $sTmpPath)
    ) {
