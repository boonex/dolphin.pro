...
function PageCodeManageSubscribers($oSubscription)
{
    ...
    return array(
        'page_main_code' => DesignBoxAdmin(_t('_adm_page_cpt_manage_subscribers'), $s, $GLOBALS['aTopItems'], '', 1)
    );
}
    ...
    <center>
        <table cellspacing=2 cellpadding=2 class=text border=0>
            <tr class=header align="center">
                <td><?= _t('_adm_mmail_Queue_status'); ?>:</td>
            </tr>
            <?= $sSingleEmailsTRs; ?>
        </table>
        <?= $sEmptyQueueTable; ?>
...
function getEmailMessage($sAction)
{
    ...
    if (isset($_POST['body']) && getPostFieldIfSet('action') != 'delete') {
        ...
        list($sSubject, $sBody) = $GLOBALS['MySQL']->getRow("SELECT `subject`, `body` FROM `sys_sbs_messages` WHERE `id`= ? LIMIT 1",
            [$sMessageID], PDO::FETCH_NUM);
    }
...
function QueueMessage()
{
    ...
    $aOriginalMessage = $MySQL->getRow("SELECT `id`, `subject`, `body` FROM `sys_sbs_messages` WHERE `id`= ? LIMIT 1",
        [$iMsgId]);
    ...
    if ($_POST['send_to_subscribers'] == 'non') {
        ...
        foreach ($aSubscribers as $aSubscriber) {
            ...
            $mixedResult = $MySQL->query("INSERT INTO `sys_sbs_queue`(`email`, `subject`, `body`) VALUES('" . $aSubscriber['user_email'] . "', '" . process_db_input($aMessage['subject'],
                    BX_TAGS_STRIP) . "', '" . process_db_input($aMessage['body'], BX_TAGS_VALIDATE) . "')");
    ...
    if ($_POST['send_to_members'] == 'memb') {
        ...
        foreach ($aMembers as $aMember) {
            ...
            $mixedResult = $MySQL->query("INSERT INTO `sys_sbs_queue`(`email`, `subject`, `body`) VALUES('" . $aMember['email'] . "', '" . process_db_input($aMessage['subject'],
                    BX_TAGS_STRIP) . "', '" . process_db_input($aMessage['body'], BX_TAGS_VALIDATE) . "')");
            if ($mixedResult === false) {
