...
while (($aTopItem = $rTopItems->fetch()) !== false) {
    $sComposerInit .= "
        ...
        aTopItems[$iIndex] = [{$aTopItem['ID']}, '" . bx_js_string($aTopItem['Name'],
            BX_ESCAPE_STR_APOS) . "', " . $aTopItem['Movable'] . "];
        aCustomItems[$iIndex] = {};";
    ...
    while (($aCustomItem = $rCustomItems->fetch()) !== false) {
        $sComposerInit .= "
        aCustomItems[$iIndex][" . ($iSubIndex++) . "] = [{$aCustomItem['ID']}, '" . bx_js_string($aCustomItem['Name'],
                BX_ESCAPE_STR_APOS) . "', " . $aCustomItem['Movable'] . "];";
    }
...
while (($aSystemItem = $rSysItems->fetch()) !== false) {
    $sComposerInit .= "
        ...
        aSystemItems[$iIndex] = [{$aSystemItem['ID']}, '" . bx_js_string($aSystemItem['Name'],
            BX_ESCAPE_STR_APOS) . "', " . $aSystemItem['Movable'] . "];
        aCustomItems[$iIndex] = {};";
...
function showEditForm($aItem)
{
    ...
    return PopupBox('tmc_edit_popup', _t('_adm_mbuilder_edit_item'),
