...
if (isset($_GET['popup'])) {
    ...
    switch ($_GET['popup']) {
        ...
        case 'dnsbl_recheck':
            ...
            $sPopupContent = PageCodeRecheckPopup($aChains, _t('_sys_adm_fld_dnsbl_recheck'), 'sys-adm-dnsbl-test',
                'dnsbl-recheck-ip');
            break;
        ...
        case 'dnsbluri_recheck':
            ...
            $sPopupContent = PageCodeRecheckPopup($aChains, _t('_sys_adm_fld_dnsbluri_recheck'), 'sys-adm-dnsbl-test',
                'dnsbl-recheck-uri');
            break;

        case 'dnsbl_help':
            $sPopupTitle   = _t('_sys_adm_btn_dnsbl_help');
            $sPopupContent = $GLOBALS['oAdmTemplate']->parseHtmlByName('antispam_dnsbl_help.html',
                array('text' => _t('_sys_adm_btn_dnsbl_help_text')));
            break;
        case 'dnsbluri_help':
            $sPopupTitle   = _t('_sys_adm_btn_dnsbl_help');
            $sPopupContent = $GLOBALS['oAdmTemplate']->parseHtmlByName('antispam_dnsbl_help.html',
                array('text' => _t('_sys_adm_btn_dnsbluri_help_text')));
            break;
        case 'dnsbl_add':
            $sPopupTitle   = _t('_sys_adm_btn_dnsbl_add');
            $oForm         = new BxDolAdmFormDnsblAdd(array('spammers' => 'spammers', 'whitelist' => 'whitelist'),
                'dnsbl');
...
switch (true) {
    ...
    case (isset($_POST['action']) && isset($_POST['id']) && isset($_POST['test'])):
        ...
        $aChain = $GLOBALS['MySQL']->getAll("SELECT `zonedomain`, `postvresp` FROM `sys_dnsbl_rules` WHERE `id` = ? AND `active` = 1",
            [$_POST['id']]);
...
$_page_cont[$iNameIndex]['page_main_code'] = call_user_func($aPages[$sMode]['func'], $aPages[$sMode]['func_params'][0],
    $aPages[$sMode]['func_params'][1]);
...
function PageCodeDNSBL($aChains, $sMode)
{
    ...
    $aRules        = $GLOBALS['MySQL']->getAll("SELECT * FROM `sys_dnsbl_rules` WHERE `chain` IN($sPlaceholders) ORDER BY `chain`, `added` ",
        $aChains);
...
{
    $sPlaceholders = implode(',', array_fill(0, count($aChains), '?'));
    $aRules        = $GLOBALS['MySQL']->getAll("SELECT * FROM `sys_dnsbl_rules` WHERE `chain` IN($sPlaceholders) AND `active` = 1 ORDER BY `chain`, `added` ",
        $aChains);
...
function PageCodeLog($sMode)
{
    ...
    $aLog   = $GLOBALS['MySQL']->getAll("SELECT SQL_CALC_FOUND_ROWS * FROM `sys_antispam_block_log` WHERE `type` = ? ORDER BY `added` DESC LIMIT $iStart, $iPerPage",
        [$sMode]);
