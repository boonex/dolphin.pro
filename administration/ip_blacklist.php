...
$_page_cont[$iNameIndex]['page_main_code'] = call_user_func_array($aPages[$sMode]['func'],
    $aPages[$sMode]['func_params']);
...
function PageCodeManage()
{
    ...
    $s = DesignBoxAdmin(_t('_adm_ipbl_manage'), $oBxDolAdminIpBlockList->getManagingForm(), $GLOBALS['aTopItems'], '',
        11);

    $s .= DesignBoxAdmin(_t('_adm_ipbl_Type' . (int)getParam('ipListGlobalType') . '_desc'),
        $oBxDolAdminIpBlockList->GenIPBlackListTable(), '', '', 11);
