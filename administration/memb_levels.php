...
if (isset($_POST['save']) && isset($_POST['cat'])) {
    ...
    if ((isset($_POST['adm-mlevels-enable']) || isset($_POST['adm-mlevels-disable'])) && !empty($_POST['levels'])) {
        ...
        $GLOBALS['MySQL']->query("UPDATE `sys_acl_levels` SET `Active`='" . $sValue . "' WHERE `ID` IN ('" . implode("','",
                $_POST['levels']) . "')");
    } else {
        if (isset($_POST['adm-mlevels-delete']) && !empty($_POST['levels'])) {
            ...
            if (isset($_POST['adm-mlevels-actions-enable']) || isset($_POST['adm-mlevels-actions-disable'])) {
                ...
                if (isset($_POST['adm-mlevels-prices-add'])) {
                    $iLevelId = (int)$_POST['level'];
                    ...
                    if (isset($_POST['adm-mlevels-prices-delete'])) {
                        $GLOBALS['MySQL']->query("DELETE FROM `sys_acl_level_prices` WHERE `id` IN ('" . implode("','",
                                $_POST['prices']) . "')");
                    } else {
                        if (isset($_POST['adm-mlevels-action-save'])) {
                            ...
                            $aResult = $GLOBALS['MySQL']->query($sQuery) > 0 ? array(
                                'code'    => 0,
                                'message' => MsgBox(_t('_adm_txt_mlevels_action_saved'))
                            ) : array('code' => 1, 'message' => MsgBox(_t('_adm_txt_mlevels_action_cannot_save')));
                            ...
                            if (isset($_POST['action']) && $_POST['action'] == 'get_edit_form_action') {
                                echo json_encode(array(
                                    'code' => PageCodeAction((int)$_POST['level_id'], (int)$_POST['action_id'],
                                        $mixedResultAction)
                                ));
                                exit;
                            }
...
$_page_cont[$iNameIndex] = array(
    ...
    'page_code_actions' => bx_get('action') !== false && bx_get('action') == 'actions' && $iLevelId > 0 ? PageCodeActions($iLevelId,
        $mixedResultActions) : "",
    'page_code_prices'  => bx_get('action') !== false && bx_get('action') == 'prices' && $iLevelId > 0 ? PageCodePrices($iLevelId,
        $mixedResultPrices) : "",
);
...
function PageCodeMain($aResults, $iLevelId)
{
    ...
    $aTopItems = array(
        'adm-mlevels-btn-levels'     => array(
            'href'    => 'javascript:void(0)',
            'onclick' => 'javascript:onChangeType(this)',
            'title'   => _t('_adm_txt_mlevels_levels'),
            'active'  => $sTab == 'levels' ? 1 : 0
        ),
        'adm-mlevels-btn-levels-add' => array(
            'href'    => 'javascript:void(0)',
            'onclick' => 'javascript:onChangeType(this)',
            'title'   => _t('_adm_txt_mlevels_levels_add'),
            'active'  => $sTab == 'levels_add' ? 1 : 0
        ),
        'adm-mlevels-btn-settings'   => array(
            'href'    => 'javascript:void(0)',
            'onclick' => 'javascript:onChangeType(this)',
            'title'   => _t('_adm_txt_mlevels_settings'),
            'active'  => $sTab == 'settings' ? 1 : 0
        )
    );

    $sResult = $GLOBALS['oAdmTemplate']->parseHtmlByName('mlevels.html', array(
        'content_levels'   => _getLevelsList(isset($aResults['levels']) ? $aResults['levels'] : true,
            $sTab == 'levels'),
        'content_create'   => _getLevelsCreateForm($bEdit ? $iLevelId : 0, $sTab == 'levels_add'),
        'content_settings' => _getLevelsSettingsForm(isset($aResults['settings']) ? $aResults['settings'] : true,
            $sTab == 'settings'),
    ));
...
function _getLevelsCreateForm($iLevelId, $bActive = false)
{
    ...
    if (($bEdit = $iLevelId != 0) === true) {
        $aLevel = $GLOBALS['MySQL']->getRow("SELECT `Name` AS `Name`, `Description` AS `Description`, `Order` AS `Order` FROM `sys_acl_levels` WHERE `ID`= ? LIMIT 1",
            [$iLevelId]);
    }
    ...
    if ($oForm->isSubmittedAndValid()) {
        ...
        if (!$bEdit) {
            ...
            } elseif (isImage($_FILES['Icon']['type'],
                    $sFileExt) && !empty($_FILES['Icon']['tmp_name']) && move_uploaded_file($_FILES['Icon']['tmp_name'],
                    $sFilePath . $sFileName . '.' . $sFileExt)
            ) {
...
function PageCodeActions($iId, $mixedResult)
{
    ...
    $aActionsActive = $GLOBALS['MySQL']->getAllWithKey("SELECT `ta`.`ID` AS `id`, `ta`.`Name` AS `title` FROM `sys_acl_actions` AS `ta` 
                      LEFT JOIN `sys_acl_matrix` AS `tm` ON `ta`.`ID`=`tm`.`IDAction` LEFT JOIN `sys_acl_levels` AS `tl` ON `tm`.`IDLevel`=`tl`.`ID` WHERE `tl`.`ID`= ?",
        "id", [$iId]);
...
{
    $aAction = $GLOBALS['MySQL']->getRow("SELECT * FROM `sys_acl_matrix` WHERE `IDLevel`='" . $iLevelId . "' AND `IDAction`= ?",
        [$iActionId]);
    ...
    $aForm = array(
        ...
        'inputs'     => array(
            ...
            'dateStart'               => array(
                ...
                'value'   => isset($aAction['AllowedPeriodStart']) ? preg_replace('/(\d+):(\d+):(\d+)/', '$1:$2',
            ...
            'dateEnd'                 => array(
                ...
                'info'    => _t('_adm_txt_mlevels_actions_avail_desc'),
                'value'   => isset($aAction['AllowedPeriodEnd']) ? preg_replace('/(\d+):(\d+):(\d+)/', '$1:$2',
                    $aAction['AllowedPeriodEnd']) : "",
                'attrs'   => array(
                    'allow_input' => 'true',
                ),
                'db'      => array(
...
function PageCodePrices($iId, $mixedResult)
{
    ...
    $aItems  = array();
    $aPrices = $GLOBALS['MySQL']->getAll("SELECT `id` AS `id`, `Days` AS `days`, `Price` AS `price` FROM `sys_acl_level_prices` WHERE `IDLevel`= ? ORDER BY `id`",
        [$iId]);
    foreach ($aPrices as $aPrice) {
        $aItems[] = array(
            'id'    => $aPrice['id'],
            'title' => (int)$aPrice['days'] == 0 ? _t('_adm_txt_mlevels_price_info_lifetime', $sCurrencySign,
                $aPrice['price']) : _t('_adm_txt_mlevels_price_info', $aPrice['days'], $sCurrencySign,
                $aPrice['price']),
        );
    }
...
function deleteMembership($iId)
{
    ...
    $aLevel = $GLOBALS['MySQL']->getRow("SELECT `Icon` AS `icon`, `Removable` AS `removable` FROM `sys_acl_levels` WHERE `ID`= ?",
        [$iId]);
    if (empty($aLevel)) {
