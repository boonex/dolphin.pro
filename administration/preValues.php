        ...
        <div class="adm-pv-cp-selector bx-def-margin-bottom">
            <div class="adm-pv-cp-item">
                ...
                <div class="input_wrapper input_wrapper_select bx-def-margin-sec-leftright clearfix">
                    <select class="form_input_select bx-def-font-inputs" name="list"
                            onchange="if( this.value != '' ) window.location = '<?= $GLOBALS['site']['url_admin'] . 'preValues.php'; ?>' + '?list=' + encodeURIComponent( this.value ) + '<?= $sPopupAdd ?>';"><?= genListOptions($aLists,
                            $sList) ?></select>
                </div>
            </div>
            <div class="input_wrapper input_wrapper_submit clearfix">
                <input class="form_input_submit bx-btn" type="button"
                       value="<?php echo bx_html_attribute(_t('_adm_pvalues_txt_create_new')); ?>"
                       onclick="createNewList();" />
            </div>
        </div>
        <table class="bx-def-table" id="listEdit" cellpadding="0"
               cellspacing="1"><?php $iNextInd = $sList !== '' ? genListRows($sList_db) : 0; ?></table>
        <div class="adm-pv-submit bx-def-margin-top">
            <input type="hidden" name="popup" value="<?= $iAmInPopup ?>" />
            <input type="hidden" name="action" value="Save" />
            <div class="input_wrapper input_wrapper_submit clearfix">
                <input class="form_input_submit bx-btn" type="submit" name="submit"
                       value="<?= bx_html_attribute(_t('_Save')) ?>" />
            </div>
        </div>
        <script type="text/javascript">
            iNextInd = <?= $iNextInd ?>;
...
function genListRows($sList)
{
    ...
    <tr class="headers">
        ...
        foreach ($aFields as $sField => $sHelp) {
            ?>
            <th>
                <span class="tableLabel" onmouseover="showFloatDesc( '<?= bx_js_string($sHelp) ?>' );"
                      onmousemove="moveFloatDesc( event );" onmouseout="hideFloatDesc();"><?= $sField ?></span>
            </th>
            <?php
        }
        ?>
    ...
    foreach ($aRows as $aRow) {
        ...
        <tr>
            ...
            ?>
            <th class="row_controls"><a class="row_control bx-def-margin-thd-left-auto" href="javascript:void(0)"
                                        onclick="javascript:delRow(this);"
                                        title="<?php echo bx_html_attribute(_t('_Delete')); ?>"><i
                        class="sys-icon times"></i></a><a class="row_control bx-def-margin-thd-left-auto"
                                                          href="javascript:void(0)"
                                                          onclick="javascript:moveUpRow(this);"
                                                          title="<?php echo bx_html_attribute(_t('_adm_pvalues_txt_move_up')); ?>"><i
                        class="sys-icon arrow-up"></i></a><a class="row_control bx-def-margin-thd-left-auto"
                                                             href="javascript:void(0)"
                                                             onclick="javascript:moveDownRow(this);"
                                                             title="<?php echo bx_html_attribute(_t('_adm_pvalues_txt_move_down')); ?>"><i
                        class="sys-icon arrow-down"></i></a></th>
        </tr>
        <?php
        $iCounter++;
    }
    ...
    <tr class="headers">
        ...
        <th class="row_controls">
            <a class="row_control" href="javascript:void(0)" onclick="javascript:addRow(this);"
               title="<?php echo bx_html_attribute(_t('_adm_pvalues_txt_add_record')); ?>"><i class="sys-icon plus"></i></a>
        </th>
    </tr>
    <?php
