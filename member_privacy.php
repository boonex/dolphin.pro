...
if (isset($_POST['ps_action']) && $_POST['ps_action'] == 'get_chooser' && $iId != 0) {
    ...
    if (isset($_POST['ps_action']) && $_POST['ps_action'] == 'view_block' && $iId != 0) {
        ...
        if (($bResult = (int)$GLOBALS['MySQL']->query($sSql)) > 0) {
            $aGroup      = $GLOBALS['MySQL']->getRow("SELECT `id`, `title` FROM `sys_privacy_groups` WHERE `id`= ? LIMIT ?",
                [$iGroupId, 1]);
            ...
            if (isset($_POST['ps_action']) && $_POST['ps_action'] == 'members') {
                ...
                if (isset($_POST['ps-add-members-add']) && !empty($_POST['ps-add-members-add'])) {
                    ...
                    if (isset($_POST['ps-del-members-delete']) && !empty($_POST['ps-del-members-delete'])) {
                        ...
                        if (isset($_POST['ps-my-groups-delete']) && !empty($_POST['ps-my-groups-delete'])) {
                            $aIds = !empty($_POST['ps-my-groups-ids']) ? $_POST['ps-my-groups-ids'] : array();
                            $oPrivacyView->deleteGroups($aIds);
                        } else {
                            if (isset($_POST['ps-default-group-save']) && !empty($_POST['ps-default-group-save'])) {
                                $iId = !empty($_POST['ps-default-group-ids']) ? (int)$_POST['ps-default-group-ids'] : 0;
                                $oPrivacyView->setDefaultGroup($iId);
                            } else {
                                if (isset($_POST['ps-default-values-save']) && !empty($_POST['ps-default-values-save'])) {
